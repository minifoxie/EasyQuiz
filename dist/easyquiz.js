...
- Em 'rationale', seja estritamente telegr\xE1fico e ultra-curto (M\xC1XIMO 10 PALAVRAS no total, ex: "Afirma\xE7\xF5es I e III verdadeiras" ou "Op\xE7\xF5es B e D corretas" ou "x = 42").
...
[RESPOSTAS]:
${(()=>{if(c.length===0)return"Nenhuma";let p=c.filter(v=>v.type==="checkbox"||v.type==="chk"),f=new Set(c.filter(v=>v.type==="radio").map(v=>v.name).filter(Boolean)),y=p.filter(v=>!v.name||!f.has(v.name)),x=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(o.questionText),m=y.length>=2||x,b=c.every(v=>v.type==="radio"||v.type==="chk")&&f.size>=1&&!m;return(m?`[MULTI-SELE\xC7\xC3O: marque TODOS os corretos, pode ser 2 ou mais]
`:b?`[ESCOLHA-\xDAnica: marque APENAS 1 op\xE7\xE3o]
`:"")+JSON.stringify(c.map(v=>({id:v.id,t:v.type,n:v.name||void 0,txt:v.label?v.label.length>160?v.label.slice(0,160)+"...":v.label:void 0,v:v.value||void 0,opt:v.options&&v.options.length?v.options.slice(0,20).map(T=>T.label||T.value):void 0})))})()}
...
`;function Kt(){try{if(typeof document>"u"||!document.head)return;if(!document.getElementById("eq-image-pulse-style")){let o=document.createElement("style");o.id="eq-image-pulse-style",o.textContent=bo,document.head.appendChild(o)}}catch{}}function le(){ge&&(ge.style.removeProperty("outline"),ge.style.removeProperty("outline-offset"),ge.style.removeProperty("position"),ge=null);for(let o of re)o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("background-color"),o.style.removeProperty("box-shadow"),o.removeAttribute("data-easyquiz-highlight");re=[];for(let o of lt)o.style.removeProperty("animation"),o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("box-shadow"),o.removeAttribute("data-easyquiz-image-highlight");lt=[];for(let o of ct)try{o.remove()}catch{}if(ct=[],fe){try{fe.remove()}catch{}fe=null}}function dt(o){Kt();for(let e of o){if(!e||!(e instanceof(typeof HTMLElement<"u"?HTMLElement:e.constructor)))continue;let t=e;t.style.outline="3px solid #ffd600",t.style.outlineOffset="3px",t.style.animation="eq-image-pulse-yellow-white 1.2s ease-in-out infinite",t.setAttribute("data-easyquiz-image-highlight","true"),lt.push(t);try{let n=t.parentElement;if(n&&!n.querySelector("[data-easyquiz-capture-badge]")){window.getComputedStyle(n).position==="static"&&(n.style.position="relative");let a=document.createElement("div");a.setAttribute("data-easyquiz-capture-badge","true"),a.textContent="\u{1F4F7} Capturado pela IA",a.style.cssText=`
    ...
    `,this.shadow.appendChild(this.element),this.element.querySelector("#eq-fah-pill").addEventListener("click",()=>this.restore()),this.element.querySelector("#eq-fah-min-btn").addEventListener("click",()=>this.minimize()),this.element.querySelector("#eq-fah-close-btn").addEventListener("click",()=>this.hide());let i=this.element.querySelector("#eq-fah-copy-md-btn");i.addEventListener("click",()=>this.copyMarkdownToClipboard(i));let a=this.element.querySelector("#eq-fah-copy-all-btn");a.addEventListener("click",()=>this.copyMarkdownToClipboard(a));let l=this.element.querySelector("#eq-fah-header");this.setupDraggable(l)}setupDraggable(e){let t=n=>{if(n.target.closest(".eq-fah-btn"))return;n.preventDefault(),this.isDragging=!0,this.dragStartX=n.clientX,this.dragStartY=n.clientY;let i=this.element.getBoundingClientRect();this.initialLeft=i.left,this.initialTop=i.top;let a=s=>{if(!this.isDragging||!this.element)return;let r=s.clientX-this.dragStartX,c=s.clientY-this.dragStartY,d=Math.max(10,window.innerWidth-this.element.offsetWidth-10),h=Math.max(10,window.innerHeight-this.element.offsetHeight-10),u=Math.min(Math.max(10,this.initialLeft+r),d),p=Math.min(Math.max(10,this.initialTop+c),h);this.element.style.left=`${u}px`,this.element.style.top=`${p}px`},l=()=>{this.isDragging=!1,window.removeEventListener("mousemove",a),window.removeEventListener("mouseup",l)};window.addEventListener("mousemove",a),window.addEventListener("mouseup",l)};e.addEventListener("mousedown",t)}renderContent(){if(!this.element||!this.currentPlan)return;let e=this.element.querySelector("#eq-fah-body"),t=this.element.querySelector("#eq-fah-pill-text"),n=this.element.querySelector("#eq-fah-pill-badge");e.innerHTML="";let i=this.currentPlan,a=i.actions.filter(p=>p.t==="drag"),l=i.actions.filter(p=>{if(p.t!=="val")return!1;let f=q(p.id||"").toLowerCase();return!/continu|avan[cç]|pr[oó]xim|submet|enviar|check|verific/i.test(f)}),s=i.actions.filter(p=>p.t==="clk"||p.t==="chk"),r=i.actions.filter(p=>p.t==="sel"),c=a.length||l.length||s.length||r.length,d=document.createElement("div");d.className="eq-fah-meta";let h=document.createElement("span");h.textContent=`Modo: ${i.mode.replace("_"," ")}`;let u=document.createElement("span");if(u.className="eq-fah-meta-badge",u.textContent=`${Math.round(i.confidence*100)}% Confian\xE7a`,d.append(h,u),e.appendChild(d),a.length>0||i.mode==="categorizacao"||i.mode==="arrastar_soltar"){t.textContent=`Categoriza\xE7\xE3o (${a.length} itens)`,n.textContent=String(a.length);let p={};for(let f of a){let y=q(f.to)||"Geral";p[y]||(p[y]=[]),p[y].push(q(f.from))}for(let[f,y]of Object.entries(p)){let x=document.createElement("div"),m=/fato|true|verdadeiro|sim/i.test(f),b=/opini[aã]o|false|falso|n[aã]o/i.test(f);x.className=`eq-fah-group ${m?"group-fato":b?"group-opiniao":""}`;let g=document.createElement("div");g.className="eq-fah-group-title",g.textContent=`\u{1F4C1} ${f} (${y.length})`,x.appendChild(g);let v=document.createElement("div");v.className="eq-fah-group-items";for(let T of y){let A=document.createElement("div");A.className="eq-fah-item";let w=document.createElement("span");w.className="eq-fah-item-text",w.textContent=T,A.appendChild(w);let E=document.createElement("button");E.className="eq-fah-copy-inline",E.textContent="Copiar",E.addEventListener("click",()=>{navigator.clipboard.writeText(T),E.textContent="\u2713 Copiado",setTimeout(()=>E.textContent="Copiar",1200)}),A.appendChild(E),v.appendChild(A)}x.appendChild(v),e.appendChild(x)}}else if(l.length>0){t.textContent=`Preenchimento (${l.length} campos)`,n.textContent=String(l.length);let p=document.createElement("div");p.className="eq-fah-group";let f=document.createElement("div");f.className="eq-fah-group-title",f.textContent="\u{1F4DD} Respostas para os Campos de Texto:",p.appendChild(f);let y=document.createElement("div");y.className="eq-fah-group-items";for(let x=0;x<l.length;x++){let m=l[x],b=document.createElement("div");b.className="eq-fah-item";let g=Me(m.id);(!g||/^[#\.\$]|input|mat-|cell|field|q[0-9]|eq-/i.test(g))&&(g=`Campo ${x+1}`);let v=String(m.v??""),T=document.createElement("div");T.className="eq-fah-field-box";let A=document.createElement("div");A.className="eq-fah-field-label",A.textContent=g,T.appendChild(A);let w=document.createElement("div");w.className="eq-fah-field-val",w.textContent=v,T.appendChild(w),b.appendChild(T);let E=document.createElement("button");E.className="eq-fah-copy-inline",E.textContent="Copiar",E.addEventListener("click",()=>{navigator.clipboard.writeText(v),E.textContent="\u2713 Copiado",setTimeout(()=>E.textContent="Copiar",1200)}),b.appendChild(E),y.appendChild(b)}p.appendChild(y),e.appendChild(p)}else if(s.length>0){t.textContent=`Op\xE7\xF5es (${s.length} marcadas)`,n.textContent=String(s.length);let p=document.createElement("div");p.className="eq-fah-group";let f=document.createElement("div");f.className="eq-fah-group-title",f.textContent="\u{1F3AF} Alternativa(s) Correta(s):",p.appendChild(f);let y=document.createElement("div");y.className="eq-fah-group-items";for(let x=0;x<s.length;x++){let m=s[x],b=document.createElement("div");b.className="eq-fah-item";let g=Me(m.id);(!g||/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(g))&&m.v&&(g=String(m.v)),g=q(g),/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(g)&&(g="");let v="",T=g.match(/^(\([A-Za-z0-9]\)|[A-Za-z0-9][\)\.\:\-])\s*(.*)$/);T?(v=T[1].replace(/[\(\)\.\:\-\s]/g,"").toUpperCase(),g=T[2].trim()||g):s.length>1&&(v=String.fromCharCode(65+x));let A=document.createElement("div");if(A.style.display="flex",A.style.alignItems="center",A.style.gap="8px",A.style.flex="1",v){let H=document.createElement("span");H.className="eq-fah-letter-badge",H.textContent=v,A.appendChild(H)}let w=document.createElement("span");w.className="eq-fah-item-text",w.textContent=g||(v?`Alternativa ${v}`:"Alternativa Selecionada"),A.appendChild(w),b.appendChild(A);let E=document.createElement("button");E.className="eq-fah-copy-inline",E.textContent="Copiar",E.addEventListener("click",()=>{navigator.clipboard.writeText(g||v),E.textContent="\u2713 Copiado",setTimeout(()=>E.textContent="Copiar",1200)}),b.appendChild(E),y.appendChild(b)}p.appendChild(y),e.appendChild(p)}else if(r.length>0){t.textContent=`Sele\xE7\xE3o (${r.length} listas)`,n.textContent=String(r.length);let p=document.createElement("div");p.className="eq-fah-group";let f=document.createElement("div");f.className="eq-fah-group-title",f.textContent="\u{1F4CB} Op\xE7\xF5es para Selecionar na Lista:",p.appendChild(f);let y=document.createElement("div");y.className="eq-fah-group-items";for(let x=0;x<r.length;x++){let m=r[x],b=document.createElement("div");b.className="eq-fah-item";let g=Me(m.id);(!g||/^[#\.\$]|select|input|mat-|cell|field|q[0-9]|eq-/i.test(g))&&(g=`Lista ${x+1}`);let A=(Array.isArray(m.v)?m.v:[String(m.v??"")]).map(V=>{let B=L(m.id,void 0,!0)||L(q(m.id),void 0,!0),_=B instanceof HTMLSelectElement?B:B?.querySelector("select");if(_){let ce=q(V).toLowerCase();for(let S=0;S<_.options.length;S++){let I=_.options[S];if(I.value.toLowerCase()===ce||q(I.textContent).toLowerCase()===ce){let $=q(I.textContent);if($&&!$.toLowerCase().includes("selecione"))return $}}}return V}).join(", "),w=document.createElement("div");w.className="eq-fah-field-box";let E=document.createElement("div");E.className="eq-fah-field-label",E.textContent=g,w.appendChild(E);let H=document.createElement("div");H.className="eq-fah-field-val",H.textContent=A,w.appendChild(H),b.appendChild(w);let R=document.createElement("button");R.className="eq-fah-copy-inline",R.textContent="Copiar",R.addEventListener("click",()=>{navigator.clipboard.writeText(A),R.textContent="\u2713 Copiado",setTimeout(()=>R.textContent="Copiar",1200)}),b.appendChild(R),y.appendChild(b)}p.appendChild(y),e.appendChild(p)}else{t.textContent="Gabarito",n.textContent="0";let p=document.createElement("div");p.style.padding="10px",p.style.color="#888",p.textContent="Nenhuma resposta direta para exibir.",e.appendChild(p)}if(i.rationale){let p=document.createElement("div");p.className="eq-fah-rationale",p.textContent=`\u{1F4A1} Racioc\xEDnio da IA: ${i.rationale}`,e.appendChild(p)}}generateMarkdown(){if(!this.currentPlan)return"";let e=this.currentPlan,t=[];t.push("# Gabarito da Quest\xE3o \u2014 EasyQuiz Pro"),t.push(`- **Modo:** ${e.mode}`),t.push(`- **Confian\xE7a:** ${(e.confidence*100).toFixed(0)}%`),t.push("");let n=e.actions.filter(s=>s.t==="drag"),i=e.actions.filter(s=>s.t==="val"),a=e.actions.filter(s=>s.t==="clk"||s.t==="chk"),l=e.actions.filter(s=>s.t==="sel");if(n.length>0){t.push("## \u{1F4C2} Categoriza\xE7\xE3o:");let s={};for(let r of n){let c=q(r.to)||"Geral";s[c]||(s[c]=[]),s[c].push(q(r.from))}for(let[r,c]of Object.entries(s)){t.push(`### Categoria: ${r}`);for(let d of c)t.push(`- ${d}`);t.push("")}}else if(i.length>0){t.push("## \u270F\uFE0F Respostas para Preenchimento:");for(let s of i){let r=q(s.id);t.push(`- **${r||"Campo"}:** \`${s.v}\``)}t.push("")}else if(a.length>0){t.push("## \u2705 Alternativas Corretas:");for(let s=0;s<a.length;s++){let r=a[s],c=Me(r.id);(!c||/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(c))&&r.v&&(c=String(r.v)),c=q(c),/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(c)&&(c="");let d=a.length>1?`${String.fromCharCode(65+s)}) `:"";t.push(`- [x] ${d}${c||"Alternativa "+String.fromCharCode(65+s)}`)}t.push("")}else if(l.length>0){t.push("## \u{1F4CB} Op\xE7\xF5es Selecionadas em Lista:");for(let s of l){let r=q(s.id)||"Lista",c=Array.isArray(s.v)?s.v.join(", "):String(s.v??"");t.push(`- **${r}:** \`${c}\``)}t.push("")}return e.rationale&&(t.push("---"),t.push(`**\u{1F4A1} Racioc\xEDnio:** ${e.rationale}`)),t.join(`
