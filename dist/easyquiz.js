...
`;function K(o,e,t){let n=o.htmlSnippet.includes("draggable")||o.htmlSnippet.includes("perseus")||o.htmlSnippet.includes("category")||o.htmlSnippet.includes("dropzone")||o.controls.some(p=>p.type==="draggable"||p.type==="dropzone"),a=new Set(["navigation"]);o.controls.some(p=>["text","number","textarea","contenteditable"].some(d=>p.type.includes(d)))&&a.add("text"),o.controls.some(p=>["radio","checkbox"].includes(p.type)||p.tag==="button")&&a.add("choice"),o.controls.some(p=>p.tag==="select")&&a.add("select"),o.controls.some(p=>/combobox|dropdown/i.test(p.type))&&a.add("combobox"),(o.controls.some(p=>["draggable","dropzone"].includes(p.type))||n)&&a.add("drag"),t.engine==="javascript"&&a.add("javascript");let i=o.questionText.length<120||n||o.controls.length<3?`
[HTML]:
${o.htmlSnippet.slice(0,3e3).replace(/\s+/g," ")}`:`
[HTML]: Omitido.`,r=te(),c=r.length>0?`
[MEM\xD3RIA]:
${r.join(" | ")}
`:"",l=o.controls.filter(p=>p.role!=="navigation"),u=o.controls.filter(p=>p.role==="navigation");return`--- AN\xC1LISE ---
...
[RESPOSTAS]:
${l.length>0?JSON.stringify(l.map(p=>({id:p.id,t:p.type,n:p.name||void 0,txt:p.label,v:p.value||void 0,opt:p.options.length?p.options:void 0}))):"Nenhuma"}
...
[IMAGENS]: ${e.length}
[/DADOS]
Sa\xEDda em JSON v\xE1lido.`}var ct=new Set(["question","info","start","conclusion"]),dt=new Set(["texto_livre","escolha_unica","escolha_multipla","verdadeiro_falso","preenchimento","acao_sem_resposta","categorizacao","ordenacao","arrastar_soltar"]),ut=new Set(["val","chk","sel","clk","adv","js","drag"]),pt=150,_e=2e3;function H(o,e=""){return typeof o=="string"?o.trim().slice(0,_e):e}function ht(o,e){if(!o||typeof o!="object")return null;let t=o,n=t.t;if(typeof n!="string"||!ut.has(n))return null;if(n==="adv")return{t:"adv",...H(t.id)?{id:H(t.id,"").slice(0,500)}:{}};if(n==="drag"){let i=H(t.from),r=H(t.to);return!i||!r?null:{t:"drag",from:i.slice(0,500),to:r.slice(0,500)}}if(n==="js"){let i=H(t.v);return!i||i.length>8e3?null:{t:"js",v:i}}let a=H(t.id).slice(0,500);if(!a)return null;if(n==="val")return{t:"val",id:a,v:H(t.v).slice(0,_e)};if(n==="sel"){let r=(Array.isArray(t.v)?t.v:[t.v]).map(c=>H(c).slice(0,500)).filter(Boolean);return{t:"sel",id:a,v:r}}if(n==="chk")return{t:"chk",id:a,c:t.c!==!1};let s={t:"clk",id:a};return Array.isArray(t.co)&&t.co.length===2&&t.co.every(i=>typeof i=="number"&&Number.isFinite(i))&&(s.co=[t.co[0],t.co[1]]),s}function je(o){if(!o||typeof o!="object")return{pageType:"info",mode:"acao_sem_resposta",confidence:.5,rationale:"Resposta estruturada n\xE3o identificada; avan\xE7ando como informativo.",actions:[{t:"adv"}]};let e=o,t=e.pageType,n=e.mode;(typeof t!="string"||!ct.has(t))&&(t="question"),(typeof n!="string"||!dt.has(n))&&(n="escolha_unica");let a=Array.isArray(e.actions)?e.actions:[],s=[];for(let l=0;l<Math.min(a.length,pt);l++){let u=ht(a[l],l);u&&s.push(u)}let i=s.filter(l=>l.t!=="adv"),r=s.some(l=>l.t==="adv");t==="question"&&i.length===0&&(t="info"),t==="conclusion"?s.length=0:t==="info"||t==="start"?r||s.push({t:"adv"}):t==="question"&&!r&&s.push({t:"adv"});let c=typeof e.confidence=="number"&&Number.isFinite(e.confidence)?Math.min(1,Math.max(0,e.confidence)):.85;return{pageType:t,mode:n,confidence:c,rationale:H(e.rationale,"Plano validado e auto-recuperado."),actions:s,...H(e.memoryToStore)?{memoryToStore:H(e.memoryToStore)}:{},...e.needsMoreContext?{needsMoreContext:!!e.needsMoreContext}:{}}}var V=[{id:"gemini-3.8-flash",name:"Gemini 3.8 Flash (R\xE1pido e atual)",description:"Modelo est\xE1vel multimodal para baixa lat\xEAncia e tarefas agentivas.",stable:!0},{id:"gemini-3.6-flash",name:"Gemini 3.6 Flash (Recomendado Google)",description:"Modelo recomendado oficial do Google AI Studio com alt\xEDssima disponibilidade.",stable:!0},{id:"gemini-3.5-flash-lite",name:"Gemini 3.5 Flash-Lite (Econ\xF4mico e r\xE1pido)",description:"Modelo est\xE1vel de menor custo e menor taxa de fila.",stable:!0},{id:"gemini-3.5-flash",name:"Gemini 3.5 Flash (Equilibrado)",description:"Modelo balanceado para resolu\xE7\xE3o de exerc\xEDcios.",stable:!0},{id:"gemini-2.0-flash",name:"Gemini 2.0 Flash (Alta Disponibilidade)",description:"Modelo consolidado para conting\xEAncia.",stable:!0},{id:"gemini-2.5-flash",name:"Gemini 2.5 Flash (Legado)",description:"Modelo da gera\xE7\xE3o anterior para contas existentes.",stable:!0}],mt={type:"OBJECT",properties:{pageType:{type:"STRING",enum:["question","info","start","conclusion"]},mode:{type:"STRING",enum:["texto_livre","escolha_unica","escolha_multipla","verdadeiro_falso","preenchimento","acao_sem_resposta","categorizacao","ordenacao","arrastar_soltar"]},confidence:{type:"NUMBER"},rationale:{type:"STRING"},memoryToStore:{type:"STRING"},actions:{type:"ARRAY",items:{type:"OBJECT",properties:{t:{type:"STRING",enum:["val","chk","sel","clk","adv","js","drag"]},id:{type:"STRING"},v:{},c:{type:"BOOLEAN"},co:{type:"ARRAY",items:{type:"NUMBER"}},from:{type:"STRING"},to:{type:"STRING"}},required:["t"]}}},required:["pageType","mode","confidence","rationale","actions"]};function gt(o){return o.trim().replace(/^google\//,"").replace(/^models\//,"")||"gemini-2.5-flash"}function Ge(o,e){let t="";try{let n=JSON.parse(o);t=n.error?.message||n.message||""}catch{t=o.slice(0,160)}return/API_KEY_INVALID|API key not valid|key.*invalid|unregistered/i.test(t)?"Chave de API do Gemini inv\xE1lida ou n\xE3o autorizada no Google AI Studio.":/RESOURCE_EXHAUSTED|Quota exceeded/i.test(t)||e===429?"Limite tempor\xE1rio de cota do Gemini (HTTP 429) atingido. Aguardando recupera\xE7\xE3o...":e===404?`HTTP 404: ${t||"Modelo ou endpoint n\xE3o encontrado no Google AI Studio"}`:e===503||/overloaded/i.test(t)?`Servidores Google sobrecarregados (HTTP 503): ${t||"Aguardando"}`:t?`Erro Gemini (HTTP ${e}): ${t}`:`Falha na requisi\xE7\xE3o ao Gemini (HTTP ${e}).`}function ft(o){try{return JSON.parse(o)}catch(e){throw new Error(`Falha ao decodificar JSON da IA (${e instanceof Error?e.message:"incompleto"})`)}}var oe=(()=>{try{let o=typeof localStorage<"u"?localStorage.getItem("easyquiz_cached_models"):null;return o?JSON.parse(o):null}catch{return null}})(),fe=new Set;async function ne(o){let e=o.trim().replace(/^["']|["']$/g,"");if(!e)return V;let t=[`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(e)}`,`https://generativelanguage.googleapis.com/v1/models?key=${encodeURIComponent(e)}`];for(let n of t)try{let a=await fetch(n,{headers:{"Content-Type":"application/json","x-goog-api-key":e}});if(!a.ok){let i=await a.text(),r=Ge(i,a.status);if(r.includes("inv\xE1lida")||r.includes("n\xE3o autorizada"))throw new Error(r);continue}let s=await a.json();if(Array.isArray(s.models)&&s.models.length>0){let i=s.models.filter(r=>{let c=r.supportedGenerationMethods||[],l=(r.name||"").includes("gemini"),u=c.includes("generateContent"),p=(r.name||"").includes("embedding")||(r.name||"").includes("tts")||(r.name||"").includes("imagen")||(r.name||"").includes("aqa")||(r.name||"").includes("computer-use");return l&&u&&!p}).map(r=>{let c=r.supportedGenerationMethods||[],l=r.name.replace(/^models\//,""),u=r.displayName||l;return{id:l,name:u.includes(l)?u:`${u} (${l})`,description:r.description||"",stable:!/-preview|-experimental|-latest/i.test(l),supportsVision:!/embedding|tts|transcribe|live|image/i.test(l),supportsStructuredOutput:c.includes("generateContent"),supportedGenerationMethods:c,discoveredAt:Date.now()}});if(i.length>0){i.sort((r,c)=>{let l=u=>u==="gemini-3.8-flash"?120:u==="gemini-3.5-flash-lite"?115:u==="gemini-2.5-flash"?100:u==="gemini-3.1-pro-preview"?90:u.includes("flash")?50:10;return l(c.id)-l(r.id)}),oe=i;try{typeof localStorage<"u"&&localStorage.setItem("easyquiz_cached_models",JSON.stringify(i))}catch{}return i}}}catch(a){if(a.message?.includes("Chave de API"))throw a}return V}async function Fe(o){let e=o.trim().replace(/^["']|["']$/g,"");if(!e)return{ok:!1,message:"Insira sua chave de API."};try{let n=await ne(e);if(n.length>0&&n!==V){let a=n[0];return{ok:!0,message:`Chave v\xE1lida! ${n.length} modelos Gemini dispon\xEDveis em sua conta. Recomendado: ${a.name}`,models:n}}}catch(n){return{ok:!1,message:n instanceof Error?n.message:String(n)}}let t=["gemini-3.8-flash","gemini-3.5-flash-lite","gemini-2.5-flash"];for(let n of t)for(let a of["v1beta","v1"]){let s=`https://generativelanguage.googleapis.com/${a}/models/${n}:generateContent?key=${encodeURIComponent(e)}`;try{if((await fetch(s,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":e},body:JSON.stringify({contents:[{role:"user",parts:[{text:"PING"}]}],generationConfig:{maxOutputTokens:5}})})).ok)return{ok:!0,message:`Chave validada com sucesso no ${n} (${a})!`,models:V}}catch{}}return{ok:!1,message:"Chave de API inv\xE1lida, sem cota ou sem permiss\xE3o para modelos Gemini."}}async function ve(o,e,t,n){let a=t.apiKey.trim().replace(/^["']|["']$/g,"");if(!a)throw new Error("Chave de API n\xE3o configurada.");let s=gt(t.model);if(!oe||oe.length===0)try{n?.("Verificando modelos autorizados na sua chave de API...","info"),await ne(a)}catch(h){let g=h instanceof Error?h.message:String(h);if(g.includes("inv\xE1lida")||g.includes("n\xE3o autorizada"))throw new Error(g)}let i=Date.now(),r=K(o,e,t),c=[{text:r}];for(let h of e)c.push({inline_data:{mime_type:h.mediaType,data:h.base64}});let l={temperature:.05,maxOutputTokens:2500,response_mime_type:"application/json",response_schema:mt},u=[s,...oe?.map(h=>h.id)||[],"gemini-3.6-flash","gemini-3.5-flash-lite","gemini-3.8-flash","gemini-3.5-flash","gemini-2.0-flash","gemini-2.5-flash"],p=Array.from(new Set(u)).filter(h=>!fe.has(h));p.length===0&&(fe.clear(),p.push(...V.map(h=>h.id)));let d=new Error("Nenhum modelo tentado.");for(let h=0;h<p.length;h++){let g=p[h],y=p[h+1];g.includes("2.5")||g.includes("thinking")?l.thinkingConfig={thinkingBudget:0}:delete l.thinkingConfig;let m={system_instruction:{parts:[{text:Ve}]},contents:[{role:"user",parts:c}],generationConfig:l};n?.(`Aguardando resposta da API (${g})...`,"info");let v=["v1beta","v1"];for(let S of v){let $=`https://generativelanguage.googleapis.com/${S}/models/${g}:generateContent?key=${encodeURIComponent(a)}`,k=new AbortController,Ie=setTimeout(()=>k.abort(),35e3);try{let R=await fetch($,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":a},body:JSON.stringify(m),signal:k.signal});if(clearTimeout(Ie),!R.ok){let Pe=await R.text();if(R.status===400&&l.thinkingConfig&&/thinking/i.test(Pe)){delete l.thinkingConfig,m.generationConfig=l;continue}let st=Ge(Pe,R.status);if(R.status===404&&S==="v1beta")continue;throw new Error(st)}let U=await R.json(),ue=U.candidates?.[0];if(!ue||!ue.content?.parts?.[0]?.text)throw new Error("A IA n\xE3o retornou uma resposta estruturada v\xE1lida.");let ze=ue.content.parts[0].text,D=je(ft(ze));if(D.usedModel=g,D.durationMs=Date.now()-i,D.promptSent=r,D.tokensUsed=U.usageMetadata?.totalTokenCount,D.promptTokens=U.usageMetadata?.promptTokenCount,D.candidatesTokens=U.usageMetadata?.candidatesTokenCount,D.rawResponse=ze,g!==s){n?.(`Resolvido com sucesso pelo fallback '${g}' (${S})!`,"info");try{t.model=g}catch{}}return{plan:D,rawUsage:U.usageMetadata,usedModel:g}}catch(R){if(clearTimeout(Ie),d=R,d.message.includes("inv\xE1lida")||d.message.includes("n\xE3o autorizada"))throw d}}let E=d.message.includes("429")||d.message.includes("cota"),w=d.message.includes("503")||d.message.includes("sobrecarregado");if(d.message.includes("404")&&fe.add(g),y){let S=E?3500:w?2500:900,$=`Modelo '${g}' indispon\xEDvel (${d.message}). Aguardando ${S/1e3}s antes de alternar para '${y}'...`;console.warn(`[EasyQuiz Fallback] ${$}`),n?.($,"warning"),await new Promise(k=>setTimeout(k,S))}else console.warn(`[EasyQuiz Fallback] Modelo '${g}' falhou: ${d.message}. Todos os modelos esgotados.`)}throw d}var vt=[/\bfetch\b/i,/\bXMLHttpRequest\b/i,/\bWebSocket\b/i,/\b(?:localStorage|sessionStorage|indexedDB)\b/i,/\b(?:eval|Function|AsyncFunction|GeneratorFunction)\b/i,/\bimport(?:Scripts)?\b/i,/\bnavigator\s*\.\s*credentials\b/i,/\b(?:cookie|location\s*=|history\s*\.)/i,/\bwindow\s*\[\s*['"](?:fetch|eval|Function|localStorage|sessionStorage)/i];function Y(o){let e=o?.engine||"smart",t=new Set(["dom","framework","keyboard","drag"]);return o?.autoAdvance&&t.add("navigation"),e==="javascript"&&t.add("javascript"),{engine:e,capabilities:t,maxAttemptsPerAction:e==="command"?1:2,maxActionMs:e==="command"?1500:3e3,allowJavaScript:e==="javascript",allowNavigation:!!o?.autoAdvance}}function be(o,e){if(o.t==="js"&&!e.allowJavaScript)throw new Error("A\xE7\xE3o JavaScript bloqueada pela engine atual. Use o motor JavaScript explicitamente.");if(o.t==="adv"&&!e.allowNavigation)throw new Error("Avan\xE7o autom\xE1tico bloqueado pela pol\xEDtica atual.")}function Ue(o){if(!o.trim())throw new Error("JavaScript recusado: c\xF3digo vazio.");if(o.length>8e3)throw new Error("JavaScript recusado: c\xF3digo acima do limite operacional.");if(vt.find(t=>t.test(o)))throw new Error("JavaScript recusado: acesso externo, persist\xEAncia ou avalia\xE7\xE3o din\xE2mica n\xE3o permitidos.");if(!/^\s*(?:\/\/[^\n]*\n|\/\*[\s\S]*?\*\/|[\s\S])*\$eq\./.test(o)&&!o.includes("$eq."))throw new Error("JavaScript recusado: use somente a API declarativa $eq.")}var _=['input:not([type="hidden"])',"textarea","select","button","a",'[role="button"]','[role="link"]','[role="radio"]','[role="checkbox"]','[role="option"]','[contenteditable="true"]','[draggable="true"]',"[aria-grabbed]","[aria-dropeffect]","[data-widget-type]",".perseus-drag-item",".sortable-item",'[data-testid*="drag" i]','[data-testid*="card" i]','[data-testid*="option" i]','[data-testid*="category" i]','[data-role="dropzone"]',"[data-category]"].join(","),ae=/(verificar|checar|check|conferir|validar|próxim[oa]|next|continuar|continue|avançar|prosseguir|enviar|submit|concluir|finalizar|terminar|começar|iniciar|start|vamos lá|próxima tarefa|next task|próxima pergunta|next question|marcar como concluíd[oa]|mostrar resumo|entendi|compreendi|ok|leitura concluída|seguir|ir para o exercício|fazer o teste|próximo artigo|ir para a aula)/i,bt=0;function q(o){let e=o;if(!e||typeof e.isConnected=="boolean"&&!e.isConnected)return!1;if(typeof e.checkVisibility=="function")try{if(!e.checkVisibility({checkOpacity:!0,checkVisibilityCSS:!0}))return!1}catch{}try{let n=window.getComputedStyle?window.getComputedStyle(e):e.style;if(n&&(n.display==="none"||n.visibility==="hidden"||Number(n.opacity||"1")<=0))return!1}catch{}try{let n=e.closest('[hidden], [style*="display: none"], [style*="display:none"]');if(n&&!Qe(n))return!1}catch{}try{if(typeof e.getBoundingClientRect=="function"){let n=e.getBoundingClientRect();if(n.width>0||n.height>0)return!0}}catch{}try{if(typeof e.getClientRects=="function"&&e.getClientRects().length>0)return!0}catch{}let t=e.tagName?.toLowerCase();if(["input","select","textarea","button"].includes(t)){let n=e.closest('label, .option-card, .quiz-option, [class*="option" i], [class*="choice" i], tr, div');if(n&&n!==e)return q(n)}return e.ownerDocument&&e.ownerDocument.defaultView&&/jsdom/i.test(e.ownerDocument.defaultView.navigator?.userAgent||"")?!e.closest('[style*="display: none"], [style*="display:none"], [hidden]'):(e.textContent||"").trim().length>0}function yt(o){if(o==null)return"";if(typeof o=="string")return o;if(typeof o=="number"||typeof o=="boolean")return String(o);if(o instanceof Node)return o.textContent||"";try{if(typeof o?.toString=="function"){let e=o.toString();if(typeof e=="string")return e}}catch{}return""}function T(o,e=500){return yt(o).replace(/\s+/g," ").trim().slice(0,e)}function xt(o){let e=o.dataset.easyquizId;if(e)return e;let t=`eq-${Date.now().toString(36)}-${(bt+=1).toString(36)}`;return o.dataset.easyquizId=t,t}function Qe(o){return o?!!(o.closest('#easyquiz-shadow-root, .eq-sidebar, .eq-launcher, [data-easyquiz-ignore="true"], .btn-inject-eq, #btn-inject-script')||o.getAttribute?.("data-easyquiz-ignore")==="true"):!1}function z(o){if(!o||!(o instanceof Element)||Qe(o)||o.closest("header, nav, aside"))return!1;let e=o instanceof HTMLInputElement||o instanceof HTMLButtonElement?o.value:"",t=T(o.getAttribute?.("aria-label")||o.textContent||o.getAttribute?.("value")||e),n=o.type,a=t.replace(/[\d\(\)\[\]→\>\•\-\/\\]+/g," ").trim(),s=String(o.getAttribute?.("data-testid")||o.getAttribute?.("data-test-id")||o.getAttribute?.("id")||o.getAttribute?.("href")||"").toLowerCase();return ae.test(a)||ae.test(t)||n==="submit"||s.includes("next")||s.includes("check")||s.includes("continue")||s.includes("proximo")||s.includes("forward")||!1}function qt(o){let e=o.closest("tr");if(e){let r=e.querySelector("th, td:first-child"),c=r&&r!==o.closest("td")?T(r.textContent,100):"",l=T(o.closest("label, td")?.textContent||"",50);if(c&&l)return`${c}: ${l}`}let t=o.getAttribute("aria-label");if(t)return T(t);let n=o.getAttribute("aria-labelledby");if(n){let r=n.split(/\s+/).map(c=>document.getElementById(c)?.textContent).filter(Boolean).join(" ");if(r.trim())return T(r)}if("labels"in o&&o.labels){let r=Array.from(o.labels??[]).map(c=>c.textContent).join(" ");if(r.trim())return T(r)}let a=o.closest('.docssharedWizToggleLabeledContainer, [role="listitem"], .answer, label, .quiz-option, .form-check, .option-card');if(a&&a!==o){let r=T(a.textContent);if(r)return r}let s=o instanceof HTMLInputElement||o instanceof HTMLButtonElement?o.value:"",i=o.getAttribute("placeholder")||o.getAttribute("title")||o.textContent||s||"";return T(i)}function ye(o,e){let t=o instanceof HTMLSelectElement?o:null,n=o;o.dataset.easyquizRole=e;let a=o.tagName.toLowerCase(),s=["input","textarea","select","button"].includes(a)?a:"other",i=o.getAttribute("role")||"",r=(o.getAttribute("data-testid")||o.getAttribute("data-test-id")||"").toLowerCase(),c=(o.className&&typeof o.className=="string"?o.className:"").toLowerCase(),l=o.getAttribute("draggable")==="true"||o.classList.contains("perseus-drag-item")||o.classList.contains("sortable-item")||!!o.getAttribute("aria-grabbed")||/drag|card|option|item/i.test(r)||/drag|card-item|sortable/i.test(c),u=o.getAttribute("data-role")==="dropzone"||o.classList.contains("category-container")||o.hasAttribute("data-category")||!!o.getAttribute("aria-dropeffect")||/drop|category|bucket/i.test(r)||/dropzone|category-box|bucket|target-zone/i.test(c),d=T((l?"draggable":u?"dropzone":"")||n.type||i||s,40),h="";if(n.type==="checkbox"||n.type==="radio"||i==="radio"||i==="checkbox")h=n.checked||o.getAttribute("aria-checked")==="true"?"checked":"unchecked";else if(s==="button"||a==="a"||e==="navigation"||z(o))h="";else{let w=o instanceof HTMLInputElement||o instanceof HTMLTextAreaElement||o instanceof HTMLSelectElement?o.value:"";h=T(w||o.getAttribute("data-category")||"",2e3)}let g=[];if(t)for(let w of Array.from(t.options).slice(0,80))g.push({value:T(w.value),label:T(w.textContent)});let y=!!(n.required||o.getAttribute("aria-required")==="true"),m=!!(n.disabled||o.getAttribute("aria-disabled")==="true"),v=xt(o);return{id:o.id||v,tag:s,type:d,label:qt(o),name:T(n.name||o.getAttribute("name")||"",180),value:h,options:g,required:y,disabled:m,role:e}}var Ke=['[data-test-id*="exercise" i]','[data-testid*="exercise" i]',".perseus-renderer",".framework-perseus",".Qr7Oae",".que",".question-holder",".quiz-question",".question_holder",".display_question",'[data-functional-selector*="question"]',".question-container","[data-question-id]",'[data-testid*="question" i]','[class*="question-container" i]','[class*="question" i]','[class*="pergunta" i]',"article","form","section","main"].join(",");function Ye(o){if(!q(o))return-1/0;let e=o.getBoundingClientRect(),t=Array.from(o.querySelectorAll(_)).filter(q),n=T(o.innerText,4e3).length;if(n<10||!t.length&&n<60)return-1/0;let a=Math.max(1,window.innerWidth*window.innerHeight),s=Math.max(1,e.width*e.height),i=Math.min(1,s/a),r=e.top+e.height/2,c=Math.abs(r-window.innerHeight/2)/Math.max(1,window.innerHeight),l=n>40?35:0,u=e.top>=0&&e.bottom<=window.innerHeight?25:0;return t.length*15+Math.min(60,n/20)+l+u-i*20-c*10}function xe(o){let e=o;for(;e.parentElement&&e.parentElement!==document.body&&e.parentElement!==document.documentElement;){let t=e.parentElement,n=t.tagName.toLowerCase();if(["header","footer","nav","aside"].includes(n))break;if(t.matches?.('article, section, form, [data-test-id*="exercise" i], [data-testid*="exercise" i], .perseus-renderer, .framework-perseus, [class*="question-container" i], .que, main')){e=t;break}let a=T(e.innerText,1e4),s=T(t.innerText,1e4),i=e.querySelectorAll(_).length,r=t.querySelectorAll(_).length;if(a.length<150&&s.length>a.length&&r<=i+4){e=t;continue}break}return e}function wt(o){let e=o,t=e.closest('main, [role="main"], article, form, [data-test-id*="exercise" i], [data-testid*="exercise" i], .framework-perseus, section');if(t&&t!==document.body&&q(t))return t;let n=0;for(;e.parentElement&&e.parentElement!==document.body&&n<3;)e=e.parentElement,n++;return e||document.body}function Et(){let o=document.activeElement;if(o&&o!==document.body){let a=o.closest(Ke);if(a&&Ye(a)>0)return xe(a)}let t=Array.from(document.querySelectorAll(Ke)).map(a=>({element:a,score:Ye(a)})).filter(a=>Number.isFinite(a.score)).sort((a,s)=>s.score-a.score);if(t.length>0&&t[0].score>0)return xe(t[0].element);let n=document.querySelector('form, main, [role="main"]');return n&&q(n)?n:document.body}function Je(o){let e=o.cloneNode(!0);e.querySelectorAll("script, style, iframe, object, embed, svg, canvas, noscript, audio, video").forEach(n=>n.remove());let t=["type","name","value","role","aria-label","aria-labelledby","aria-checked","aria-required","required","disabled","data-easyquiz-id","draggable","class","id","data-widget-type","data-role","data-category","data-testid"];return e.querySelectorAll("*").forEach(n=>{for(let a of Array.from(n.attributes))t.includes(a.name)||n.removeAttribute(a.name)}),e.outerHTML.replace(/\s+/g," ").slice(0,2e4)}function Tt(o){return Array.from(o.querySelectorAll(_)).filter(e=>{if(!q(e)||z(e))return!1;if(e.tagName.toLowerCase()==="a"){let t=e.getAttribute("role");return!!(t==="button"||t==="radio"||t==="checkbox"||t==="option"||e.closest('[class*="choice" i], [class*="option" i], [class*="answer" i], [data-testid*="option" i]'))}return!0}).slice(0,100).map(e=>ye(e,"answer"))}function qe(o){let e=[o,o.parentElement,o.parentElement?.parentElement,document.body].filter(Boolean),t=new Set,n=[];for(let a of e)for(let s of Array.from(a.querySelectorAll(_)))if(!(t.has(s)||!q(s)||!z(s))&&(t.add(s),n.push(ye(s,"navigation")),n.length>=10))return n;return n}function J(o=!1){let e=Et();e=xe(e),o&&(e=wt(e));let t=e.innerText&&e.innerText.trim().length>0?e.innerText:e.textContent||"",n=T(t,16e3),a=Tt(e),s=qe(e);s.length===0&&(s=qe(document.body));let i=[...a,...s].slice(0,120);return!n||i.length===0&&n.length<30?T(document.body.innerText||document.body.textContent||"",16e3).length>=30?j():null:{sourceUrl:window.location.href.slice(0,2e3),pageTitle:document.title.slice(0,500)||"P\xE1gina de Quest\xE3o",questionText:n,htmlSnippet:Je(e),controls:i,scope:e}}function j(){let o=document.body.innerText||document.body.textContent||document.documentElement.textContent||"",e=T(o,14e3),t=qe(document.body),n=document.querySelector('main, article, [role="main"], [data-test-id*="content" i], [class*="content" i]')||document.body;return{sourceUrl:window.location.href.slice(0,2e3),pageTitle:document.title.slice(0,500)||"P\xE1gina de Leitura/Contexto",questionText:e,htmlSnippet:Je(n).slice(0,15e3),controls:t,scope:n}}function Xe(o){let e=o.controls.map(t=>`${t.role}:${t.id}:${t.type}:${t.value}:${t.disabled}`).join("|");return[window.location.href,o.pageTitle,o.questionText.slice(0,500),e].join("::")}function C(o){return o?!!(o.closest('#easyquiz-shadow-root, .eq-sidebar, .eq-launcher, [data-easyquiz-ignore="true"], .btn-inject-eq, #btn-inject-script')||o.getAttribute?.("data-easyquiz-ignore")==="true"):!1}function f(o){return o?o.replace(/^(\([0-9a-zA-Z]{1,2}\)|[0-9]{1,3}|[a-zA-Z])[\.\)\-\:]\s+/,"").replace(/[\.\u2026]{2,}/g," ").replace(/['"“”«»]/g,"").replace(/\s+/g," ").trim():""}function A(o){if(!o||o instanceof HTMLInputElement||o instanceof HTMLSelectElement||o instanceof HTMLTextAreaElement||o.getAttribute("draggable")==="true"||o.classList.contains("dnd-card")||o.hasAttribute("data-category")||o.hasAttribute("data-dropzone"))return o;if(o.hasAttribute("for")){let n=o.getAttribute("for");if(n){let a=o.ownerDocument.getElementById(n);if(a)return a}}let e=o.closest('label, .option-card, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, tr, li, .dnd-card, [class*="option-card" i], [class*="choice-card" i]');if(e&&!["article","section","main","form","body"].includes(e.tagName.toLowerCase())){let n=e.getAttribute("for"),s=(n?e.ownerDocument.getElementById(n):null)||e.querySelector('input:not([type="hidden"]), select, textarea');return s||e}let t=o.closest('button, a, [role="button"], [draggable="true"]');if(t)return t;if(["body","html","main","section","article","form"].includes(o.tagName.toLowerCase())){let n=o.querySelector('button, [role="button"], a, input:not([type="hidden"]), select, textarea, [role="radio"], [role="checkbox"], .option-card, label');if(n)return A(n)}return o}function x(o){if(!o)return null;let e=o.trim().replace(/^["'“”«»]+|["'“”«»]+$/g,"");if(!e)return null;let t=CSS.escape(e),n=document.querySelector(`[data-easyquiz-id="${t}"]`);if(n&&!C(n)&&q(n))return A(n);try{let l=document.getElementById(e);if(l&&!C(l)&&q(l))return l.hasAttribute("data-category")||l.hasAttribute("data-dropzone")||l.classList.contains("dnd-zone")?l:A(l)}catch{}let a=e.match(/^(?:item|opção|opcao|afirmação|afirmacao|alternativa|linha|afirmativa|questão|questao)?\s*#?([0-9]+)$/i);if(a){let l=parseInt(a[1],10)-1;if(l>=0){let u=Array.from(document.querySelectorAll('input[type="checkbox"], input[type="radio"], [role="checkbox"], [role="radio"]')).filter(p=>q(p)&&!C(p));if(l<u.length)return A(u[l])}}let s=e.match(/^(?:item|opção|opcao|afirmação|afirmacao|alternativa|linha|afirmativa|questão|questao)?\s*#?([a-eA-E])$/i);if(s){let l=s[1].toUpperCase().charCodeAt(0)-65;if(l>=0){let u=Array.from(document.querySelectorAll('input[type="checkbox"], input[type="radio"], [role="checkbox"], [role="radio"]')).filter(p=>q(p)&&!C(p));if(l<u.length)return A(u[l])}}if(/^[a-zA-Z0-9_-]{1,10}$/.test(e)){let u=Array.from(document.querySelectorAll(`[data-category="${t}" i], [data-dropzone="${t}" i], [data-role="dropzone"][data-category="${t}" i]`)).find(g=>q(g)&&!C(g));if(u)return u;let d=Array.from(document.querySelectorAll(`input[value="${t}" i], [data-value="${t}" i], input[id="${t}" i]`)).find(g=>q(g)&&!C(g));if(d)return A(d);let h=Array.from(document.querySelectorAll('.option-badge, [class*="badge" i], [class*="letter" i], .option-card span, label span')).find(g=>{if(!q(g)||C(g))return!1;let y=f(g.textContent).toLowerCase();return y===e.toLowerCase()||y===e.toLowerCase()+")"});if(h)return A(h)}try{let u=Array.from(document.querySelectorAll(`[name="${t}"], [value="${t}"], [data-category="${t}" i], [data-dropzone="${t}" i], [data-testid="${t}" i], [data-test-id="${t}" i], [aria-label="${t}" i]`)).find(p=>q(p)&&!C(p));if(u)return u.hasAttribute("data-category")||u.hasAttribute("data-dropzone")||u.classList.contains("dnd-zone")?u:A(u)}catch{}if(/^[.#\[]|\s|[>+~:]/.test(e))try{let u=Array.from(document.querySelectorAll(e)).find(p=>q(p)&&!C(p));if(u)return A(u)}catch{}try{let l=e.replace(/"/g,""),u=`//button[normalize-space(.)="${l}"] | //a[normalize-space(.)="${l}"] | //*[not(*) and normalize-space(.)="${l}"] | //*[@aria-label="${l}"] | //*[@data-category="${l}"] | //*[@data-testid="${l}"]`,p=document.evaluate(u,document,null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,null);for(let d=0;d<p.snapshotLength;d++){let h=p.snapshotItem(d);if(h&&q(h)&&!C(h)){if(["body","html"].includes(h.tagName.toLowerCase())){let y=h.querySelector('button, [role="button"], a, input, [role="radio"], [role="checkbox"], label');if(y&&q(y))return A(y)}return h.closest('[data-role="dropzone"], [class*="category" i], [class*="bucket" i], [class*="column" i], [class*="drop" i]')||A(h)}}}catch{}let r=f(e).toLowerCase(),c=Array.from(document.querySelectorAll('button, a, div, span, li, p, label, input, [draggable="true"], [data-testid], [class*="option" i], [class*="card" i], [class*="item" i], [class*="choice" i], [class*="category" i], [class*="bucket" i]'));for(let l of c){if(!q(l)||C(l)||l.closest("header, nav, .stepper, .step-item, .progress-bar-container")||!!(l.matches('article, section, form, main, [class*="container" i], [class*="grid" i], .dnd-pool, .dnd-zones')||l.querySelector('label, [role="radio"], [role="checkbox"], .dnd-card, [draggable="true"], .option-card, tr'))&&!l.matches(".dnd-zone, [data-category], [data-dropzone]"))continue;let p=f(l.textContent).toLowerCase(),d=f(l.getAttribute("aria-label")||"").toLowerCase(),h=f(l.getAttribute("data-category")||"").toLowerCase(),g=l instanceof HTMLInputElement||l instanceof HTMLButtonElement?l.value:"",y=f(g).toLowerCase(),m=p.startsWith(r+")")||p.startsWith(r+".")||p.startsWith(r+" -")||p.startsWith(r+":");if(p===r||d===r||h&&h===r||y&&y===r||m)return l.closest('[data-role="dropzone"], [class*="category" i], [class*="bucket" i], [class*="column" i], [class*="drop" i]')||A(l)}if(r.length>=3)for(let l of c){if(!q(l)||C(l)||l.closest("header, nav, .stepper, .step-item, .progress-bar-container")||!!(l.matches('article, section, form, main, [class*="container" i], [class*="grid" i], .dnd-pool, .dnd-zones')||l.querySelector('label, [role="radio"], [role="checkbox"], .dnd-card, [draggable="true"], .option-card, tr'))&&!l.matches(".dnd-zone, [data-category], [data-dropzone]"))continue;let p=f(l.textContent).toLowerCase(),d=f(l.getAttribute("aria-label")||"").toLowerCase();if(p.includes(r)||d.includes(r)){if(Array.from(l.children).some(m=>{let v=f(m.textContent).toLowerCase();return v&&v.includes(r)}))continue;return l.closest('[data-role="dropzone"], [class*="category" i], [class*="bucket" i], [class*="column" i], [class*="drop" i]')||A(l)}let h=r.split(/\s+/).filter(Boolean);if(h.length>=3){let g=h.slice(0,Math.min(5,h.length)).join(" ");if(p.includes(g)||d.includes(g))return A(l)}}return null}function X(o,e){for(let t of e)o.dispatchEvent(new Event(t,{bubbles:!0,composed:!0}))}function L(o,e){if(!o)return;let t=o instanceof HTMLInputElement&&["checkbox","radio"].includes(o.type)?o:o.querySelector('input[type="checkbox"], input[type="radio"]')||(o.hasAttribute("for")?o.ownerDocument.getElementById(o.getAttribute("for")):null);if(t&&o!==t){if(t.type==="checkbox"){I(t,!t.checked);return}if(t.type==="radio"){I(t,!0);return}}try{o.scrollIntoView({block:"center",inline:"center",behavior:"instant"})}catch{}try{o.focus?.()}catch{}if(o instanceof HTMLButtonElement||o instanceof HTMLAnchorElement||o instanceof HTMLInputElement&&!["checkbox","radio"].includes(o.type)){try{o.click()}catch{}return}let a=o.getBoundingClientRect(),s=e?e[0]:Math.round(a.left+Math.max(1,a.width/2)),i=e?e[1]:Math.round(a.top+Math.max(1,a.height/2)),r={bubbles:!0,cancelable:!0,composed:!0,view:window,clientX:s,clientY:i};try{o.dispatchEvent(new PointerEvent("pointerdown",{...r,button:0,buttons:1}))}catch{}try{o.dispatchEvent(new MouseEvent("mousedown",{...r,button:0,buttons:1}))}catch{}try{o.dispatchEvent(new PointerEvent("pointerup",{...r,button:0,buttons:0}))}catch{}try{o.dispatchEvent(new MouseEvent("mouseup",{...r,button:0,buttons:0}))}catch{}try{o.dispatchEvent(new MouseEvent("click",{...r,button:0,buttons:0}))}catch{}try{o.click()}catch{}}function Te(o,e){let t=o;if(!(t instanceof HTMLInputElement)&&!(t instanceof HTMLTextAreaElement)&&!(t instanceof HTMLSelectElement)&&!t.isContentEditable){let a=o.querySelector('input:not([type="hidden"]), textarea, select, [contenteditable="true"]');a&&(t=a)}if(t instanceof HTMLButtonElement||t.tagName.toLowerCase()==="a"||t.getAttribute("role")==="button"||t.getAttribute("role")==="link"||t instanceof HTMLInputElement&&["button","submit"].includes(t.type)||z(t)){console.log("[EasyQuiz] Auto-corre\xE7\xE3o em setNativeValue: elemento \xE9 bot\xE3o/navega\xE7\xE3o. Clicando..."),L(t);return}if(t instanceof HTMLSelectElement){Se(t,[e]);return}if(t instanceof HTMLInputElement&&["checkbox","radio"].includes(t.type)){let a=["true","1","checked","yes","sim"].includes(e.toLowerCase())||e===t.value;I(t,a);return}try{t.focus?.()}catch{}try{t.dispatchEvent(new InputEvent("beforeinput",{bubbles:!0,cancelable:!0,composed:!0,data:e}))}catch{}if(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement){try{let i=t._valueTracker;i&&i.setValue("")}catch{}let a=t instanceof HTMLTextAreaElement?HTMLTextAreaElement.prototype:HTMLInputElement.prototype,s=Object.getOwnPropertyDescriptor(a,"value")?.set;s?s.call(t,e):t.value=e;try{let i=t._valueTracker;i&&i.setValue(e)}catch{}X(t,["input","change","blur"]);return}if(t.isContentEditable){try{document.execCommand?.("selectAll",!1,void 0),document.execCommand?.("insertText",!1,e)}catch{}if(t.textContent?.trim()!==e.trim()){t.textContent=e;try{t.innerText=e}catch{}}X(t,["input","change","blur"]);return}try{t.value=e,t.textContent=e,X(t,["input","change","blur"])}catch{}}function Ce(o,e=""){if(!o)return e;let t=f(o),n=x(o)||x(t);if(!n)return t||e;let a=n.closest('label, .option-card, [class*="choice" i], [class*="option" i], .quiz-option, tr');if(a){let c=f(a.textContent);if(c&&c.length>0&&c.length<150)return c}if(n.id){let c=document.querySelector(`label[for="${CSS.escape(n.id)}"]`);if(c){let l=f(c.textContent);if(l&&l.length>0&&l.length<150)return l}}let s=n.getAttribute("aria-label");if(s)return f(s);let i=n.getAttribute("placeholder");if(i)return f(i);let r=f(n.textContent);return r&&r.length>0&&r.length<120?r:t||e}function I(o,e){let t=o.closest('.option-card, label, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, [class*="option" i], [class*="choice" i], li')||o,n=o instanceof HTMLInputElement&&["checkbox","radio"].includes(o.type)?o:t.querySelector('input[type="checkbox"], input[type="radio"]');if(!n&&t.hasAttribute("for")&&(n=t.ownerDocument.getElementById(t.getAttribute("for"))),t){let a=e?"true":"false";t.setAttribute("aria-checked",a),t.setAttribute("aria-selected",a),t.classList.toggle("selected",e),t.classList.toggle("active",e),t.classList.toggle("checked",e)}if(n){if(n.type==="radio"&&n.checked===e)return;n.checked=e;try{let s=n._valueTracker;s&&s.setValue(!e)}catch{}try{Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"checked")?.set?.call(n,e)}catch{}n.checked=e,X(n,["input","change"]);let a=t!==n?t:n;try{a.focus?.()}catch{}a.dispatchEvent(new MouseEvent("click",{bubbles:!0,cancelable:!0,composed:!0,view:window}))}else{try{t.focus?.()}catch{}t.dispatchEvent(new MouseEvent("click",{bubbles:!0,cancelable:!0,composed:!0,view:window}))}}function Se(o,e){let t=o instanceof HTMLSelectElement?o:o.querySelector("select");if(t){let a=e.map(i=>f(i).toLowerCase()),s=!1;for(let i=0;i<t.options.length;i++){let r=t.options[i],c=r.value.toLowerCase(),l=f(r.textContent).toLowerCase();if(a.some(p=>p===c||p===l)){if(r.selected=!0,t.selectedIndex=i,s=!0,!t.multiple)break}else t.multiple||(r.selected=!1)}if(!s)for(let i=0;i<t.options.length;i++){let r=t.options[i],c=r.value.toLowerCase(),l=f(r.textContent).toLowerCase();if(a.some(p=>c.includes(p)||l.includes(p)||p.length>3&&(p.includes(c)||p.includes(l)))&&(r.selected=!0,t.selectedIndex=i,s=!0,!t.multiple))break}if(s){X(t,["input","change","blur"]);return}}let n=o.closest('[role="combobox"], [class*="select" i], [class*="dropdown" i]');if(n){L(n);for(let a of e){let s=x(a);if(s){L(s);return}}}}function Ct(o,e){try{let t=new DataTransfer;try{t.setData("text/plain",o)}catch{}try{t.setData("text/html",e)}catch{}return t}catch{return null}}function we(o){try{o.click()}catch{let e=o.ownerDocument.defaultView||window;o.dispatchEvent(new e.MouseEvent("click",{bubbles:!0,cancelable:!0,composed:!0,view:e}))}}function N(o,e){let t=f(o).toLowerCase();if(!t)return null;let n=e==="source"?'.dnd-card, [draggable="true"]':'[data-dropzone], [data-category], [data-role="dropzone"]',a=Array.from(document.querySelectorAll(n)),s=e==="destination"?a.find(i=>[i.getAttribute("data-category"),i.getAttribute("data-dropzone")].some(r=>r?.trim().toLowerCase()===t)):null;return s&&q(s)&&!C(s)?s:a.find(i=>{if(!q(i)||C(i))return!1;let r=f(`${i.textContent||""} ${i.getAttribute("data-category")||""} ${i.getAttribute("data-dropzone")||""}`).toLowerCase();return r===t||r.includes(t)})||null}async function ie(o,e,t=1){try{o.scrollIntoView({block:"center",inline:"center",behavior:"instant"})}catch{}let n=o.getBoundingClientRect(),a=e.getBoundingClientRect(),s=Math.round(n.left+Math.max(1,n.width/2)),i=Math.round(n.top+Math.max(1,n.height/2)),r=Math.round(a.left+Math.max(1,a.width/2)),c=Math.round(a.top+Math.max(1,a.height/2)),l=f(e.textContent).toLowerCase();if(l){let y=Array.from(o.querySelectorAll('button, [role="button"], input[type="radio"], input[type="checkbox"], option, .btn, [class*="tag" i]')).find(m=>{let v=f(m.textContent).toLowerCase(),E=m instanceof HTMLInputElement||m instanceof HTMLOptionElement?f(m.value).toLowerCase():"";return v&&(l.includes(v)||v.includes(l))||E&&(l.includes(E)||E.includes(l))});y&&(L(y),await new Promise(m=>setTimeout(m,120)))}we(o),await new Promise(g=>setTimeout(g,140)),we(e);let u=e.querySelector('[data-role="dropzone"], [class*="bucket" i], [class*="slot" i], [class*="drop" i], [class*="target" i], [class*="items" i], ul, ol');if(u&&u!==e&&we(u),await new Promise(g=>setTimeout(g,100)),!e.contains(o)&&o.matches('.dnd-card, [draggable="true"]')&&e.matches('.dnd-zone, [data-dropzone], [data-role="dropzone"]')&&e.appendChild(o),e.contains(o)&&o.matches('.dnd-card, [draggable="true"]'))return;let p={bubbles:!0,cancelable:!0,composed:!0,view:window,clientX:s,clientY:i,screenX:s,screenY:i,button:0,buttons:1};try{o.dispatchEvent(new PointerEvent("pointerdown",{...p,isPrimary:!0,pointerId:1,pointerType:"mouse",pressure:.5}))}catch{}o.dispatchEvent(new MouseEvent("mousedown",p));let d=4;for(let g=1;g<=d;g++){let y=Math.round(s+(r-s)*(g/d)),m=Math.round(i+(c-i)*(g/d)),v={...p,clientX:y,clientY:m,screenX:y,screenY:m};try{o.dispatchEvent(new PointerEvent("pointermove",{...v,isPrimary:!0,pointerId:1,pointerType:"mouse",pressure:.5}))}catch{}document.dispatchEvent(new MouseEvent("mousemove",v))}let h={bubbles:!0,cancelable:!0,composed:!0,view:window,clientX:r,clientY:c,screenX:r,screenY:c,button:0,buttons:0};try{e.dispatchEvent(new PointerEvent("pointerup",{...h,isPrimary:!0,pointerId:1,pointerType:"mouse",pressure:0}))}catch{}e.dispatchEvent(new MouseEvent("mouseup",h)),e.dispatchEvent(new MouseEvent("click",h));try{let g=Ct(T(o.textContent),o.outerHTML),y={...p},m={...h};g&&(y.dataTransfer=g,m.dataTransfer=g);let v=o.ownerDocument.defaultView?.DragEvent;if(!v)throw new Error("DragEvent n\xE3o dispon\xEDvel neste documento");o.dispatchEvent(new v("dragstart",y)),e.dispatchEvent(new v("dragenter",m)),e.dispatchEvent(new v("dragover",m)),e.dispatchEvent(new v("drop",m)),o.dispatchEvent(new v("dragend",y))}catch(g){console.warn("[EasyQuiz] DragEvent ignorado com seguran\xE7a:",g)}try{let g=new Touch({identifier:1,target:o,clientX:s,clientY:i}),y=new Touch({identifier:1,target:e,clientX:r,clientY:c});o.dispatchEvent(new TouchEvent("touchstart",{bubbles:!0,cancelable:!0,touches:[g]})),e.dispatchEvent(new TouchEvent("touchmove",{bubbles:!0,cancelable:!0,touches:[y]})),e.dispatchEvent(new TouchEvent("touchend",{bubbles:!0,cancelable:!0,touches:[]}))}catch{}if(t>=2&&!e.contains(o))try{o.focus?.(),o.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",bubbles:!0})),o.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",bubbles:!0})),await new Promise(g=>setTimeout(g,80)),e.focus?.(),e.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",code:"Enter",bubbles:!0})),e.dispatchEvent(new KeyboardEvent("keyup",{key:"Enter",code:"Enter",bubbles:!0}))}catch{}}var et={fill:(o,e)=>{let t=x(o);t?Te(t,e):console.warn(`$eq.fill: Elemento '${o}' n\xE3o encontrado`)},click:o=>{let e=x(o);e?!!(e.closest('.option-card, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, [class*="option" i], [class*="choice" i]')||e.querySelector('input[type="radio"], input[type="checkbox"]')||e instanceof HTMLInputElement&&["checkbox","radio"].includes(e.type))?I(e,!0):L(e):console.warn(`$eq.click: Elemento '${o}' n\xE3o encontrado`)},check:(o,e)=>{let t=x(o);t?I(t,e):console.warn(`$eq.check: Elemento '${o}' n\xE3o encontrado`)},find:o=>x(o),drag:(o,e)=>{let t=N(o,"source")||x(o),n=N(e,"destination")||x(e);t&&n?ie(t,n):console.warn(`$eq.drag: Origem ou destino n\xE3o encontrado ('${o}' -> '${e}')`)},categorize:async(o,e)=>{let t=N(o,"source")||x(o),n=N(e,"destination")||x(e);if(!t||!n){console.warn(`$eq.categorize: Item ou categoria n\xE3o encontrados ('${o}' -> '${e}')`);return}await ie(t,n)},execute:(o,e=!1,t=1)=>Me(o,e,t)};window.$eq=et;async function St(o,e=1,t=Y()){if(be(o,t),o.t==="js"){let i=String(o.v||"");Ue(i);try{new Function("$eq","document","window",i)(et,document,window)}catch(r){throw console.warn("[EasyQuiz JS Execution]",r),r}return}if(o.t==="drag"){let i=N(o.from,"source")||x(o.from),r=N(o.to,"destination")||x(o.to);!i&&o.from&&(i=x(f(o.from))),!r&&o.to&&(r=x(f(o.to))),i&&r?await ie(i,r,e):console.warn(`[EasyQuiz] Drag: alvo n\xE3o encontrado ('${o.from}' -> '${o.to}')`);return}let n=o.id||"",a=x(n);!a&&n&&(a=x(f(n)));let s=o.v!==void 0?String(o.v).trim():"";if(a&&s){if(a instanceof HTMLInputElement&&a.type==="radio"&&a.name){if(f(a.value).toLowerCase()!==f(s).toLowerCase()){let i=document.querySelector(`input[type="radio"][name="${CSS.escape(a.name)}"][value="${CSS.escape(s)}" i]`);if(i)a=i;else{let c=Array.from(document.querySelectorAll(`input[type="radio"][name="${CSS.escape(a.name)}"]`)).find(l=>{let u=l.closest("label, .vf-label, .option-card, tr, td, div");return u&&f(u.textContent).toLowerCase().includes(f(s).toLowerCase())});c&&(a=c)}}}else if(!(a instanceof HTMLInputElement)&&!(a instanceof HTMLSelectElement)&&!(a instanceof HTMLTextAreaElement)){let i=a.querySelector(`input[value="${CSS.escape(s)}" i], [data-value="${CSS.escape(s)}" i]`);if(i)a=i;else{let c=Array.from(a.querySelectorAll('input[type="radio"], input[type="checkbox"]')).find(l=>{let u=l.closest("label, .vf-label, .option-card, td, div");return u&&f(u.textContent).toLowerCase().includes(f(s).toLowerCase())});c&&(a=c)}}}if(!a&&o.t!=="adv"){console.warn(`[EasyQuiz] Alvo '${n}' n\xE3o encontrado para a\xE7\xE3o '${o.t}'. Prosseguindo...`);return}switch(o.t){case"val":a&&(a instanceof HTMLButtonElement||a.tagName.toLowerCase()==="a"||a.getAttribute("role")==="button"||a.getAttribute("role")==="link"||a instanceof HTMLInputElement&&["button","submit"].includes(a.type)||z(a)?(console.log(`[EasyQuiz] Auto-corre\xE7\xE3o: A\xE7\xE3o 'val' direcionada a bot\xE3o/link '${o.id}'. Clicando...`),L(a)):Te(a,String(o.v)));break;case"chk":a&&I(a,!!o.c);break;case"sel":if(a){let r=Array.isArray(o.v)?o.v:[String(o.v)];Se(a,r)}break;case"clk":a&&(!!(a.closest('.option-card, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice')||a.querySelector('input[type="radio"], input[type="checkbox"]')||a instanceof HTMLInputElement&&["checkbox","radio"].includes(a.type))?I(a,!0):L(a,o.co));break;case"adv":let i=tt(o.id);if(i){await Ee(i,1200);let r=o.id||i.textContent?.trim()||"";r&&me(window.location.hostname,{advanceSelector:r}),L(i)}else console.warn("[EasyQuiz] Bot\xE3o de avan\xE7o n\xE3o localizado.");break}}function Mt(){let o=["button","a",'[role="button"]','input[type="submit"]','input[type="button"]','[data-testid*="check" i]','[data-test-id*="check" i]'].join(",");return Array.from(document.querySelectorAll(o)).find(t=>{if(!q(t)||C(t)||t.closest("header, nav, aside"))return!1;let n=t instanceof HTMLInputElement||t instanceof HTMLButtonElement?t.value:"",a=(t.textContent||n||t.getAttribute("aria-label")||"").trim();return/(verificar|checar|check|conferir|validar|enviar|responder)/i.test(a)})||null}function tt(o){if(o){let s=x(o);if(s&&q(s)&&!C(s))return s}try{let s=Q(window.location.hostname);if(s.advanceSelector){let i=x(s.advanceSelector);if(i&&q(i)&&!C(i))return i}}catch{}let e=["button","a",'[role="button"]','[role="link"]','input[type="button"]','input[type="submit"]','[data-testid*="next" i]','[data-testid*="continue" i]','[data-testid*="check" i]','[data-test-id*="next" i]','[data-test-id*="continue" i]','[data-test-id*="check" i]','[class*="next" i]','[class*="continue" i]','[class*="proximo" i]','[class*="avancar" i]'].join(","),n=Array.from(document.querySelectorAll(e)).filter(s=>q(s)&&!C(s)&&!s.closest("header, nav, aside"));for(let s of n)if(z(s))return s;for(let s of n){let i=s instanceof HTMLInputElement||s instanceof HTMLButtonElement?s.value:"",r=(s.textContent||i||s.getAttribute("aria-label")||"").trim();if(ae.test(r))return s}let a=document.querySelector('[data-test-id*="next" i], [data-testid*="next" i], [aria-label*="next" i], [aria-label*="pr\xF3xim" i], [aria-label*="avan\xE7ar" i], [aria-label*="continuar" i]');return a&&q(a)&&!C(a)?a:null}async function Ee(o,e=1500){let t=Date.now();for(;Date.now()-t<e;){if(!(o.disabled===!0||o.getAttribute("aria-disabled")==="true"||o.classList.contains("disabled")||o.getAttribute("disabled")!==null))return;await new Promise(a=>setTimeout(a,100))}}function ot(){let o=(document.body?.innerText||document.body?.textContent||"").replace(/\s+/g," ").trim(),e=document.querySelectorAll('input, textarea, select, button, [role="button"], [role="option"]').length;return`${window.location.href}|${document.title}|${o.slice(0,900)}|${e}`}async function At(o,e=1800){let t=Date.now();for(;Date.now()-t<e;){if(ot()!==o)return{changed:!0,evidence:"URL, texto, t\xEDtulo ou conjunto de controles mudou ap\xF3s a a\xE7\xE3o."};await new Promise(a=>setTimeout(a,100))}return{changed:!1,evidence:"Nenhuma mudan\xE7a observ\xE1vel foi detectada dentro do tempo limite."}}async function We(o){if(o.t==="js"||o.t==="adv")return;if(o.t==="drag"){let n=x(o.from)||x(f(o.from)),a=x(o.to)||x(f(o.to));n&&a&&await ie(n,a,2);return}let e=o.id||"",t=x(e)||x(f(e));if(o.t==="clk"||o.t==="chk"){if(!t&&e){let a=Array.from(document.querySelectorAll('input, label, button, [role="radio"], [role="checkbox"], .option-card, [class*="option" i], [class*="choice" i]')),s=f(e).toLowerCase();t=a.find(i=>{let r=f(i.textContent).toLowerCase(),c=f(i.value||"").toLowerCase();return r.includes(s)||c===s||r.startsWith(s+")")||r.startsWith("("+s+")")})||null}let n=o.v!==void 0?String(o.v).trim():"";if(t&&n){if(t instanceof HTMLInputElement&&t.type==="radio"&&t.name){if(f(t.value).toLowerCase()!==f(n).toLowerCase()){let a=document.querySelector(`input[type="radio"][name="${CSS.escape(t.name)}"][value="${CSS.escape(n)}" i]`);if(a)t=a;else{let i=Array.from(document.querySelectorAll(`input[type="radio"][name="${CSS.escape(t.name)}"]`)).find(r=>{let c=r.closest("label, .vf-label, .option-card, tr, td, div");return c&&f(c.textContent).toLowerCase().includes(f(n).toLowerCase())});i&&(t=i)}}}else if(!(t instanceof HTMLInputElement)&&!(t instanceof HTMLSelectElement)&&!(t instanceof HTMLTextAreaElement)){let a=t.querySelector(`input[value="${CSS.escape(n)}" i], [data-value="${CSS.escape(n)}" i]`);if(a)t=a;else{let i=Array.from(t.querySelectorAll('input[type="radio"], input[type="checkbox"]')).find(r=>{let c=r.closest("label, .vf-label, .option-card, td, div");return c&&f(c.textContent).toLowerCase().includes(f(n).toLowerCase())});i&&(t=i)}}}if(t){let a=t.closest('.option-card, label, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, li')||t,s=t instanceof HTMLInputElement&&["radio","checkbox"].includes(t.type)?t:a.querySelector('input[type="radio"], input[type="checkbox"]')||(a.getAttribute("for")?a.ownerDocument.getElementById(a.getAttribute("for")):null),i=o.t==="chk"?!!o.c:!0;if(I(s||a,i),s)try{s.checked=i;try{Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"checked")?.set?.call(s,i)}catch{}(s.type!=="checkbox"||s.checked!==i)&&s.dispatchEvent(new MouseEvent("click",{bubbles:!0,cancelable:!0,composed:!0,view:window})),s.dispatchEvent(new Event("change",{bubbles:!0,composed:!0})),s.dispatchEvent(new Event("input",{bubbles:!0,composed:!0}))}catch{}try{a.focus?.(),a.setAttribute("aria-checked",i?"true":"false"),a.classList.toggle("selected",i),a.classList.toggle("active",i),a.classList.toggle("checked",i),s||a.dispatchEvent(new MouseEvent("click",{bubbles:!0,cancelable:!0,composed:!0,view:window}))}catch{}if(!s||s.checked!==i)try{a.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",bubbles:!0})),a.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",bubbles:!0}))}catch{}try{a.onclick?.()}catch{}try{s?.onclick?.()}catch{}try{s?.onchange?.()}catch{}}return}if(o.t==="val"){if(!t&&e){let n=Array.from(document.querySelectorAll('input:not([type="hidden"]), textarea, [contenteditable="true"]')),a=f(e).toLowerCase();t=n.find(s=>{let i=(s.getAttribute("placeholder")||"").toLowerCase(),r=(s.name||"").toLowerCase(),c=(s.id||"").toLowerCase(),l=(s.getAttribute("aria-label")||"").toLowerCase();return i.includes(a)||r.includes(a)||c.includes(a)||l.includes(a)})||null}if(t){let a=(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement?t:t.querySelector('input:not([type="hidden"]), textarea, [contenteditable="true"]'))||t,s=String(o.v??"");try{a.focus?.(),document.execCommand?.("selectAll",!1,void 0),document.execCommand?.("insertText",!1,s)}catch{}Te(a,s)}return}if(o.t==="sel"){if(!t&&e){let n=Array.from(document.querySelectorAll("select")),a=f(e).toLowerCase();t=n.find(s=>{let i=(s.name||"").toLowerCase(),r=(s.id||"").toLowerCase(),c=(s.getAttribute("aria-label")||"").toLowerCase();return i.includes(a)||r.includes(a)||c.includes(a)})||null}if(t){let n=Array.isArray(o.v)?o.v:[String(o.v)];Se(t,n)}return}}function G(o){try{if(o.t==="val"){let e=x(o.id)||x(f(o.id));if(!e)return!1;if(e instanceof HTMLButtonElement||e.tagName.toLowerCase()==="a"||e.getAttribute("role")==="button"||e.getAttribute("role")==="link"||e instanceof HTMLInputElement&&["button","submit"].includes(e.type)||z(e))return!0;let n=String(o.v??"").trim(),a=e instanceof HTMLInputElement&&e.type==="radio"?e:e.querySelector('input[type="radio"]');if(a&&a.name){let l=document.querySelector(`input[type="radio"][name="${CSS.escape(a.name)}"]:checked`);if(!l)return!1;let u=f(l.value).toLowerCase(),p=f(n).toLowerCase(),d=f(l.closest("label, .vf-label, .option-card, tr, td, div")?.textContent||"").toLowerCase();return u===p||d===p||d.includes(p)}let s=e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement?e:e.querySelector('input:not([type="hidden"]), textarea, [contenteditable="true"]'),i=(s?s.value??s.textContent??"":e.textContent??"").trim();if(!i&&!n)return!0;if(!i&&n)return!1;let r=i.replace(",",".").toLowerCase(),c=n.replace(",",".").toLowerCase();return r===c||r.includes(c)||i.toLowerCase()===n.toLowerCase()}if(o.t==="sel"){let e=x(o.id)||x(f(o.id));if(!e)return!1;let t=e instanceof HTMLSelectElement?e:e.querySelector("select");if(!t)return!1;let a=(Array.isArray(o.v)?o.v:[String(o.v)]).map(s=>f(s).toLowerCase());return Array.from(t.options).some(s=>{if(!s.selected)return!1;let i=s.value.toLowerCase(),r=f(s.textContent).toLowerCase();return a.some(c=>c===i||c===r||i.includes(c)||r.includes(c))})}if(o.t==="chk"||o.t==="clk"){let e=x(o.id)||x(f(o.id));if(!e)return!1;let t=e.closest('.option-card, label, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, li')||e,n=e instanceof HTMLInputElement&&["checkbox","radio"].includes(e.type)?e:t.querySelector('input[type="checkbox"], input[type="radio"]')||(t.getAttribute("for")?t.ownerDocument.getElementById(t.getAttribute("for")):null),a=o.t==="chk"?!!o.c:!0;if(n&&n.type==="radio"&&o.v){let l=f(String(o.v)).toLowerCase();if(n.name){let u=document.querySelector(`input[type="radio"][name="${CSS.escape(n.name)}"]:checked`);return u?f(u.value).toLowerCase()===l:!1}}if(n&&["checkbox","radio"].includes(n.type))return n.checked===a;let s=t.getAttribute("aria-checked")===String(a)||t.getAttribute("aria-selected")===String(a)||t.getAttribute("aria-pressed")===String(a),i=a?t.getAttribute("data-selected")==="true"||t.getAttribute("data-checked")==="true"||t.getAttribute("data-active")==="true"||t.getAttribute("data-state")==="checked"||t.getAttribute("data-state")==="on":t.getAttribute("data-selected")==="false"||t.getAttribute("data-checked")==="false"||t.getAttribute("data-state")==="unchecked",r=a?/active|selected|checked|picked|correct|is-selected|choice-selected|selected-option|is-checked|chosen|current|highlight|ring|border-primary/i.test(t.className||""):!/active|selected|checked|picked|correct|is-selected|choice-selected|selected-option|is-checked|chosen|current|highlight|ring|border-primary/i.test(t.className||"");return!!(s||i||r||(t instanceof HTMLButtonElement||t.getAttribute("role")==="button")&&o.t==="clk"||o.t==="clk"&&!n)}if(o.t==="drag"){let e=x(o.from)||x(f(o.from)),t=x(o.to)||x(f(o.to));return!e||!t?!1:t.contains(e)?!0:/placed|dropped|assigned|matched|done|selected/i.test(e.className||"")||e.getAttribute("data-placed")==="true"}}catch{}return!1}async function Me(o,e,t=1,n=Y({engine:"smart",autoAdvance:e})){let a=o.actions.filter(m=>m.t!=="adv"),s=o.actions.filter(m=>m.t==="adv"),i=0,r=[],c=new Map;for(let m of a){try{await St(m,t,n),i++}catch(v){c.set(m,v instanceof Error?v.message:String(v)),console.warn("[EasyQuiz] A\xE7\xE3o declarativa prim\xE1ria falhou com seguran\xE7a:",m,v)}m.t==="drag"&&await new Promise(v=>setTimeout(v,250))}await new Promise(m=>setTimeout(m,a.length>0?300:50));let l=0;for(let m of a){if(G(m)){l++;continue}console.warn(`[EasyQuiz Auto-Cura] A\xE7\xE3o '${m.t}' no alvo '${m.id||m.from||""}' n\xE3o verificada no DOM. Disparando Passagem 2 de conting\xEAncia...`);try{be(m,n),await We(m)}catch(v){c.set(m,v instanceof Error?v.message:String(v)),console.warn("[EasyQuiz Auto-Cura] Rota alternativa falhou:",v)}await new Promise(v=>setTimeout(v,180)),G(m)&&(console.log("[EasyQuiz Auto-Cura] \u2713 A\xE7\xE3o recuperada com sucesso pela rota de conting\xEAncia!"),l++)}if(l<a.length&&a.length>0){console.warn(`[EasyQuiz Auto-Cura] ${a.length-l} de ${a.length} a\xE7\xE3o(\xF5es) ainda n\xE3o verificadas. Disparando Passagem 3 final...`),await new Promise(m=>setTimeout(m,200));for(let m of a)if(!G(m))try{await We(m)}catch(v){c.set(m,v instanceof Error?v.message:String(v))}await new Promise(m=>setTimeout(m,200)),l=0;for(let m of a)G(m)&&l++}let u=o.pageType==="question";for(let m of a)G(m)||r.push(m.t==="drag"?`${m.from} -> ${m.to}`:"id"in m?m.id:m.t);let p=a.map((m,v)=>{let E=m.t==="drag"?`${m.from} -> ${m.to}`:m.t==="js"?"$eq":m.id||m.t,w=m.t==="js"?!0:m.t==="drag"?!!(N(m.from,"source")&&N(m.to,"destination")):!!(x(m.id||"")||x(f(m.id||""))),M=G(m);return{index:v,action:m,target:E,located:w,applied:!c.has(m),verified:M,strategy:m.t==="drag"?"drag-adaptive":m.t==="js"?"javascript":"declarative-dom",evidence:M?"estado do controle confirmado no DOM":"nenhuma evid\xEAncia suficiente ap\xF3s as tentativas",...c.has(m)?{error:c.get(m)}:{}}}),d=!u||a.length===0?!0:i===a.length&&l===a.length&&r.length===0,h=!1,g=!1,y="Nenhuma a\xE7\xE3o de navega\xE7\xE3o solicitada.";if(e&&(d||!u)){if(await new Promise(w=>setTimeout(w,a.length>0?500:200)),o.pageType!=="info"){let w=Mt();w&&q(w)&&(await Ee(w,1200),L(w),await new Promise(M=>setTimeout(M,800)))}let m=ot(),v=s.length>0?s[0].id:void 0,E=tt(v);if(E){await Ee(E,1200);let w=v||E.textContent?.trim()||"";w&&me(window.location.hostname,{advanceSelector:w}),L(E);let M=await At(m);g=M.changed,y=M.evidence,h=M.changed,M.changed||console.warn("[EasyQuiz] O bot\xE3o foi acionado, mas a navega\xE7\xE3o n\xE3o foi confirmada.")}else console.warn("[EasyQuiz] Nenhum bot\xE3o de avan\xE7o encontrado na p\xE1gina.")}return{applied:i,verified:l,success:d,advanced:h,failed:r,reports:p,navigationVerified:g,navigationEvidence:y}}var Ze=!1,P=null;function Ae(){let o=typeof window<"u"&&window.document?window.document:typeof document<"u"?document:null;!o||Ze||(Ze=!0,o.addEventListener("click",e=>{let t=e.target;if(!t||C(t))return;if(P&&(t===P||t.contains(P))){e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation?.();return}if(t instanceof HTMLInputElement&&["checkbox","radio"].includes(t.type))return;let n=t.closest('label, .option-card, [role="radio"], [role="checkbox"], .quiz-option, .answer, .choice, [class*="option" i], [class*="choice" i]');if(!n||C(n))return;let a=n.querySelector('input[type="checkbox"], input[type="radio"]');if(!a&&n.hasAttribute("for")){let s=n.getAttribute("for");s&&(a=n.ownerDocument.getElementById(s))}a&&(a.type==="checkbox"?(e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation?.(),P=a,setTimeout(()=>{P===a&&(P=null)},70),I(a,!a.checked)):a.type==="radio"&&(e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation?.(),P=a,setTimeout(()=>{P===a&&(P=null)},70),I(a,!0)))},!0))}var W=null,F=[];function Z(){W&&(W.style.removeProperty("outline"),W.style.removeProperty("outline-offset"),W=null);for(let o of F)o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("background-color"),o.style.removeProperty("box-shadow"),o.removeAttribute("data-easyquiz-highlight");F=[]}function Le(o){Z(),W=o,o.style.outline="2px solid #00e5ff",o.style.outlineOffset="4px"}function nt(o){for(let e of o){if(e.t==="adv"||e.t==="js")continue;if(e.t==="drag"){try{let s=x(e.from),i=x(e.to);s&&(s.style.outline="2px solid #00ff88",F.push(s)),i&&(i.style.outline="2px dashed #00e5ff",F.push(i))}catch{}continue}if(!e.id)continue;let t=x(e.id);if(!t)continue;let n=t.closest('label, .option-card, [role="radio"], [role="checkbox"], [role="listitem"], .answer, .quiz-option, .form-check, [class*="option" i], [class*="choice" i], tr, li')||t;n.style.outline="2px solid #00ff88",n.style.outlineOffset="2px",n.style.backgroundColor="rgba(0, 255, 136, 0.12)",n.setAttribute("data-easyquiz-highlight","true"),F.push(n);let a=t instanceof HTMLInputElement&&["checkbox","radio"].includes(t.type)?t:n.querySelector('input[type="checkbox"], input[type="radio"]');a&&a!==n&&(a.style.outline="2px solid #00ff88",a.style.outlineOffset="2px",a.style.boxShadow="0 0 10px rgba(0, 255, 136, 0.8)",a.setAttribute("data-easyquiz-highlight","true"),F.push(a))}}var ee=4,Lt=1200,ke=12e5;function se(o){return new Promise((e,t)=>{let n=new FileReader;n.onerror=()=>t(new Error("Falha ao converter blob para base64.")),n.onload=()=>{let a=String(n.result||"");e(a.split(",")[1]||"")},n.readAsDataURL(o)})}async function re(o){let e=0,t=0;if(o instanceof HTMLImageElement?(e=o.naturalWidth||o.width,t=o.naturalHeight||o.height):(e=o.width,t=o.height),e<=0||t<=0)throw new Error("Dimens\xF5es inv\xE1lidas.");let n=Math.min(1,Lt/Math.max(e,t)),a=Math.max(1,Math.round(e*n)),s=Math.max(1,Math.round(t*n)),i=document.createElement("canvas");i.width=a,i.height=s;let r=i.getContext("2d",{alpha:!1});if(!r)throw new Error("Sem suporte a Canvas 2D.");return r.fillStyle="#ffffff",r.fillRect(0,0,a,s),r.drawImage(o,0,0,a,s),new Promise((c,l)=>{i.toBlob(u=>u?c(u):l(new Error("Falha compress\xE3o.")),"image/jpeg",.8)})}async function at(o){try{let e=o.cloneNode(!0),t=o.offsetWidth||500,n=o.offsetHeight||500,a=`
    ...
    `,this.shadow.appendChild(this.element),this.element.querySelector("#eq-fah-pill").addEventListener("click",()=>this.restore()),this.element.querySelector("#eq-fah-min-btn").addEventListener("click",()=>this.minimize()),this.element.querySelector("#eq-fah-close-btn").addEventListener("click",()=>this.hide());let a=this.element.querySelector("#eq-fah-copy-md-btn");a.addEventListener("click",()=>this.copyMarkdownToClipboard(a));let s=this.element.querySelector("#eq-fah-copy-all-btn");s.addEventListener("click",()=>this.copyMarkdownToClipboard(s));let i=this.element.querySelector("#eq-fah-header");this.setupDraggable(i)}setupDraggable(e){let t=n=>{if(n.target.closest(".eq-fah-btn"))return;n.preventDefault(),this.isDragging=!0,this.dragStartX=n.clientX,this.dragStartY=n.clientY;let a=this.element.getBoundingClientRect();this.initialLeft=a.left,this.initialTop=a.top;let s=r=>{if(!this.isDragging||!this.element)return;let c=r.clientX-this.dragStartX,l=r.clientY-this.dragStartY,u=Math.max(10,window.innerWidth-this.element.offsetWidth-10),p=Math.max(10,window.innerHeight-this.element.offsetHeight-10),d=Math.min(Math.max(10,this.initialLeft+c),u),h=Math.min(Math.max(10,this.initialTop+l),p);this.element.style.left=`${d}px`,this.element.style.top=`${h}px`},i=()=>{this.isDragging=!1,window.removeEventListener("mousemove",s),window.removeEventListener("mouseup",i)};window.addEventListener("mousemove",s),window.addEventListener("mouseup",i)};e.addEventListener("mousedown",t)}renderContent(){if(!this.element||!this.currentPlan)return;let e=this.element.querySelector("#eq-fah-body"),t=this.element.querySelector("#eq-fah-pill-text"),n=this.element.querySelector("#eq-fah-pill-badge");e.innerHTML="";let a=this.currentPlan,s=a.actions.filter(d=>d.t==="drag"),i=a.actions.filter(d=>{if(d.t!=="val")return!1;let h=f(d.id||"").toLowerCase();return!/continu|avan[cç]|pr[oó]xim|submet|enviar|check|verific/i.test(h)}),r=a.actions.filter(d=>d.t==="clk"||d.t==="chk"),c=s.length||i.length||r.length,l=document.createElement("div");l.className="eq-fah-meta";let u=document.createElement("span");u.textContent=`Modo: ${a.mode.replace("_"," ")}`;let p=document.createElement("span");if(p.className="eq-fah-meta-badge",p.textContent=`${Math.round(a.confidence*100)}% Confian\xE7a`,l.append(u,p),e.appendChild(l),s.length>0||a.mode==="categorizacao"||a.mode==="arrastar_soltar"){t.textContent=`Categoriza\xE7\xE3o (${s.length} itens)`,n.textContent=String(s.length);let d={};for(let h of s){let g=f(h.to)||"Geral";d[g]||(d[g]=[]),d[g].push(f(h.from))}for(let[h,g]of Object.entries(d)){let y=document.createElement("div"),m=/fato|true|verdadeiro|sim/i.test(h),v=/opini[aã]o|false|falso|n[aã]o/i.test(h);y.className=`eq-fah-group ${m?"group-fato":v?"group-opiniao":""}`;let E=document.createElement("div");E.className="eq-fah-group-title",E.textContent=`\u{1F4C1} ${h} (${g.length})`,y.appendChild(E);let w=document.createElement("div");w.className="eq-fah-group-items";for(let M of g){let S=document.createElement("div");S.className="eq-fah-item";let $=document.createElement("span");$.className="eq-fah-item-text",$.textContent=M,S.appendChild($);let k=document.createElement("button");k.className="eq-fah-copy-inline",k.textContent="Copiar",k.addEventListener("click",()=>{navigator.clipboard.writeText(M),k.textContent="\u2713 Copiado",setTimeout(()=>k.textContent="Copiar",1200)}),S.appendChild(k),w.appendChild(S)}y.appendChild(w),e.appendChild(y)}}else if(i.length>0){t.textContent=`Preenchimento (${i.length} campos)`,n.textContent=String(i.length);let d=document.createElement("div");d.className="eq-fah-group";let h=document.createElement("div");h.className="eq-fah-group-title",h.textContent="Respostas para Inserir:",d.appendChild(h);let g=document.createElement("div");g.className="eq-fah-group-items";for(let y of i){let m=document.createElement("div");m.className="eq-fah-item";let v=document.createElement("span");v.className="eq-fah-item-text";let E=Ce(y.id),M=/^[#\.\$]|input|mat-|cell|field|q[0-9]/i.test(E)?"":E;v.textContent=`${M?`${M}: `:""}${y.v}`,m.appendChild(v);let S=document.createElement("button");S.className="eq-fah-copy-inline",S.textContent="Copiar",S.addEventListener("click",()=>{navigator.clipboard.writeText(String(y.v)),S.textContent="\u2713 Copiado",setTimeout(()=>S.textContent="Copiar",1200)}),m.appendChild(S),g.appendChild(m)}d.appendChild(g),e.appendChild(d)}else if(r.length>0){t.textContent=`Op\xE7\xF5es (${r.length} marcadas)`,n.textContent=String(r.length);let d=document.createElement("div");d.className="eq-fah-group";let h=document.createElement("div");h.className="eq-fah-group-title",h.textContent="Alternativa(s) Correta(s):",d.appendChild(h);let g=document.createElement("div");g.className="eq-fah-group-items";for(let y of r){let m=document.createElement("div");m.className="eq-fah-item";let v=document.createElement("span");v.className="eq-fah-item-text";let E=Ce(y.id);/^[#\.\$]|opt|choice|radio|chk|q[0-9]/i.test(E)&&y.v&&(E=String(y.v)),v.textContent=`\u2611 ${E}`,m.appendChild(v);let w=document.createElement("button");w.className="eq-fah-copy-inline",w.textContent="Copiar",w.addEventListener("click",()=>{navigator.clipboard.writeText(E),w.textContent="\u2713 Copiado",setTimeout(()=>w.textContent="Copiar",1200)}),m.appendChild(w),g.appendChild(m)}d.appendChild(g),e.appendChild(d)}else{t.textContent="Gabarito",n.textContent="0";let d=document.createElement("div");d.style.padding="10px",d.style.color="#888",d.textContent="Nenhuma resposta direta para exibir.",e.appendChild(d)}if(a.rationale){let d=document.createElement("div");d.className="eq-fah-rationale",d.textContent=`\u{1F4A1} Racioc\xEDnio da IA: ${a.rationale}`,e.appendChild(d)}}generateMarkdown(){if(!this.currentPlan)return"";let e=this.currentPlan,t=[];t.push("# Gabarito da Quest\xE3o \u2014 EasyQuiz Pro"),t.push(`- **Modo:** ${e.mode}`),t.push(`- **Confian\xE7a:** ${(e.confidence*100).toFixed(0)}%`),t.push("");let n=e.actions.filter(i=>i.t==="drag"),a=e.actions.filter(i=>i.t==="val"),s=e.actions.filter(i=>i.t==="clk"||i.t==="chk");if(n.length>0){t.push("## \u{1F4C2} Categoriza\xE7\xE3o:");let i={};for(let r of n){let c=f(r.to)||"Geral";i[c]||(i[c]=[]),i[c].push(f(r.from))}for(let[r,c]of Object.entries(i)){t.push(`### Categoria: ${r}`);for(let l of c)t.push(`- ${l}`);t.push("")}}else if(a.length>0){t.push("## \u270F\uFE0F Respostas para Preenchimento:");for(let i of a){let r=f(i.id);t.push(`- **${r||"Campo"}:** \`${i.v}\``)}t.push("")}else if(s.length>0){t.push("## \u2705 Alternativas Corretas:");for(let i of s)t.push(`- [x] ${f(i.id)}`);t.push("")}return e.rationale&&(t.push("---"),t.push(`**\u{1F4A1} Racioc\xEDnio:** ${e.rationale}`)),t.join(`
`)}copyMarkdownToClipboard(e){let t=this.generateMarkdown();t&&navigator.clipboard.writeText(t).then(()=>{let n=e.innerHTML;e.id==="eq-fah-copy-md-btn"?e.innerHTML='<span style="font-size:10px; color:#00ffcc; font-weight:bold;">\u2713</span>':e.innerHTML="\u2713 Copiado!",setTimeout(()=>{e.innerHTML=n},1500)})}};var it=`
