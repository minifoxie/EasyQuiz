...
[RESPOSTAS]:
${(()=>{if(p.length===0)return"Nenhuma";let f=p.filter(E=>E.type==="checkbox"||E.type==="chk"),v=new Set(p.filter(E=>E.type==="radio").map(E=>E.name).filter(Boolean)),y=f.filter(E=>!E.name||!v.has(E.name)),w=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(o.questionText),q=y.length>=2||w,k=v.size>1||/verdadeir|fals[oa]|\bv\s*\/\s*f\b|julgue|itens/i.test(o.questionText)&&v.size>=1,T=p.every(E=>E.type==="radio"||E.type==="chk")&&v.size===1&&!q&&!k,x=p.filter(E=>E.type==="text"||E.type==="number"||E.type==="val"||E.tag==="input"||E.tag==="textarea"),L=x.length>=2;return(k?`[GRADE VERDADEIRO/FALSO (${v.size||"m\xFAltiplas"} afirma\xE7\xF5es): voc\xEA DEVE julgar e marcar exatamente 1 op\xE7\xE3o (V ou F) para CADA uma das ${v.size} afirma\xE7\xF5es \u2014 emita ${v.size} a\xE7\xF5es chk separadas + adv]
