...
[HTML]:
${n.htmlSnippet.slice(0,c).replace(/\s+/g," ")}`:"",p="";if(r&&typeof document<"u")try{let E=Array.from(document.querySelectorAll('input:not([type=hidden]), textarea, select, button, [role="button"], [role="radio"], [role="checkbox"], [role="option"], [onclick], [data-action], a[href]:not([href="#"]), [tabindex]:not([tabindex="-1"])')).filter(f=>{let b=f,x=b.getBoundingClientRect?.()||{width:0,height:0};return x.width>0&&x.height>0&&!b.closest("#easyquiz-shadow-root, .eq-sidebar")}).slice(0,40).map(f=>{let b=f,x=b.tagName.toLowerCase(),M=b.id?`#${b.id}`:"",S=b.className&&typeof b.className=="string"?`.${b.className.trim().split(/\s+/).slice(0,2).join(".")}`:"",A=(b.textContent||b.value||b.getAttribute("aria-label")||"").trim().slice(0,60),C=b.getAttribute("type")||b.getAttribute("role")||"";return`${x}${M}${S}[${C}] txt="${A}"`});E.length>0&&(p=`
[DOM-INTERATIVO]:
${E.join(`
...
[RESPOSTAS]:
${(()=>{if(h.length===0)return"Nenhuma";let E=h.filter(w=>w.type==="checkbox"||w.type==="chk"),f=new Set(h.filter(w=>w.type==="radio").map(w=>w.name).filter(Boolean)),b=E.filter(w=>!w.name||!f.has(w.name)),x=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(n.questionText),M=b.length>=2||x,S=f.size>1||/verdadeir|fals[oa]|\bv\s*\/\s*f\b|julgue|itens/i.test(n.questionText)&&f.size>=1,A=h.every(w=>w.type==="radio"||w.type==="chk")&&f.size===1&&!M&&!S,C=h.filter(w=>w.type==="text"||w.type==="number"||w.type==="val"||w.tag==="input"||w.tag==="textarea"),H=C.length>=2;return(S?`[GRADE VERDADEIRO/FALSO (${f.size||"m\xFAltiplas"} afirma\xE7\xF5es): voc\xEA DEVE julgar e marcar exatamente 1 op\xE7\xE3o (V ou F) para CADA uma das ${f.size} afirma\xE7\xF5es \u2014 emita ${f.size} a\xE7\xF5es chk separadas + adv]
