/* EasyQuiz Discreto v3.7.1 — Modo Stealth sem interface
...
[HTML]:
${o.htmlSnippet.slice(0,u).replace(/\s+/g," ")}`:"",p="";if(r&&typeof document<"u")try{let b=Array.from(document.querySelectorAll('input:not([type=hidden]), textarea, select, button, [role="button"], [role="radio"], [role="checkbox"], [role="option"], [onclick], [data-action], a[href]:not([href="#"]), [tabindex]:not([tabindex="-1"])')).filter(y=>{let x=y,v=x.getBoundingClientRect?.()||{width:0,height:0};return v.width>0&&v.height>0&&!x.closest("#easyquiz-shadow-root, .eq-sidebar")}).slice(0,40).map(y=>{let x=y,v=x.tagName.toLowerCase(),_=x.id?`#${x.id}`:"",M=x.className&&typeof x.className=="string"?`.${x.className.trim().split(/\s+/).slice(0,2).join(".")}`:"",z=(x.textContent||x.value||x.getAttribute("aria-label")||"").trim().slice(0,60),w=x.getAttribute("type")||x.getAttribute("role")||"";return`${v}${_}${M}[${w}] txt="${z}"`});b.length>0&&(p=`
[DOM-INTERATIVO]:
${b.join(`
`)}`)}catch{}let l=vt(),f=l.length>0?`
[MEM\xD3RIA]:
${l.join(" | ")}
`:"",m=o.controls.filter(b=>b.role!=="navigation"),g=o.controls.filter(b=>b.role==="navigation"),h=Gt(o.sourceUrl,o.htmlSnippet),A=h?`
${h}
`:"";return`--- AN\xC1LISE ---
[MODO]: ${t.engine} | Dica: ${t.modeHint||"Auto"}
[URL]: ${o.sourceUrl}
[P\xC1GINA]: ${o.pageTitle}${f}${A}
[DADOS]
[TEXTO]:
${o.questionText}${d}${p}

[RESPOSTAS]:
${(()=>{if(m.length===0)return"Nenhuma";let b=m.filter(C=>C.type==="checkbox"||C.type==="chk"),y=new Set(m.filter(C=>C.type==="radio").map(C=>C.name).filter(Boolean)),x=b.filter(C=>!C.name||!y.has(C.name)),v=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(o.questionText),_=x.length>=2||v,M=y.size>1||/verdadeir|fals[oa]|\bv\s*\/\s*f\b|julgue|itens/i.test(o.questionText)&&y.size>=1,z=m.every(C=>C.type==="radio"||C.type==="chk")&&y.size===1&&!_&&!M,w=m.filter(C=>C.type==="text"||C.type==="number"||C.type==="val"||C.tag==="input"||C.tag==="textarea"),S=w.length>=2;return(M?`[GRADE VERDADEIRO/FALSO (${y.size||"m\xFAltiplas"} afirma\xE7\xF5es): voc\xEA DEVE julgar e marcar exatamente 1 op\xE7\xE3o (V ou F) para CADA uma das ${y.size} afirma\xE7\xF5es \u2014 emita ${y.size} a\xE7\xF5es chk separadas + adv]
...
`};var Be=class{el=null;state="idle";mouseX=-300;mouseY=-300;displayX=-300;displayY=-300;rafId=null;flashTimer=null;boundMove;constructor(){this.boundMove=e=>{this.mouseX=e.clientX,this.mouseY=e.clientY,this.displayX===-300&&(this.displayX=e.clientX,this.displayY=e.clientY)},window.addEventListener("mousemove",this.boundMove,{passive:!0}),this.injectStyle(),this.createEl(),this.startRaf()}injectStyle(){if(document.getElementById("__eqdc_style__"))return;let e=document.createElement("style");e.id="__eqdc_style__",e.textContent=`
    ...
    `,document.documentElement.appendChild(e)}open(){this.close();let t=ne().model,n=[...Te],i=(Me||[]).filter(g=>!n.some(h=>h.id===g.id)),a=document.createElement("div");a.id="__eqdm_menu__",a.setAttribute("role","menu"),a.tabIndex=-1;let s=document.createElement("div");s.className="__eqdm_section__",s.textContent="Modelo Gemini",a.appendChild(s);let r=document.createElement("div");r.className="__eqdm_sep__",a.appendChild(r);let c=(g,h,A)=>{let b=g===t,y=document.createElement("div");y.className="__eqdm_item__",y.setAttribute("role","menuitemradio"),y.setAttribute("aria-checked",b?"true":"false"),y.tabIndex=0;let x=document.createElement("span");x.className="__eqdm_check__",b&&(x.innerHTML=`<svg width="12" height="12" viewBox="0 0 12 12" fill="currentColor">
