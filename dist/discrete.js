...
[RESPOSTAS]:
${(()=>{if(h.length===0)return"Nenhuma";let v=h.filter(k=>k.type==="checkbox"||k.type==="chk"),f=new Set(h.filter(k=>k.type==="radio").map(k=>k.name).filter(Boolean)),b=v.filter(k=>!k.name||!f.has(k.name)),E=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(n.questionText),T=b.length>=2||E,w=f.size>1||/verdadeir|fals[oa]|\bv\s*\/\s*f\b|julgue|itens/i.test(n.questionText)&&f.size>=1,M=h.every(k=>k.type==="radio"||k.type==="chk")&&f.size===1&&!T&&!w,L=h.filter(k=>k.type==="text"||k.type==="number"||k.type==="val"||k.tag==="input"||k.tag==="textarea"),P=L.length>=2;return(w?`[GRADE VERDADEIRO/FALSO (${f.size||"m\xFAltiplas"} afirma\xE7\xF5es): voc\xEA DEVE julgar e marcar exatamente 1 op\xE7\xE3o (V ou F) para CADA uma das ${f.size} afirma\xE7\xF5es \u2014 emita ${f.size} a\xE7\xF5es chk separadas + adv]
`:T?`[MULTI-SELE\xC7\xC3O: marque TODOS os corretos, pode ser 2 ou mais]
`:M?`[ESCOLHA-\xDAnica: marque APENAS 1 op\xE7\xE3o]
`:P?`[M\xDALTIPLOS CAMPOS DE PREENCHIMENTO (${L.length} campos): emita uma a\xE7\xE3o val para CADA um dos ${L.length} campos abaixo com seu id exato]
`:"")+JSON.stringify(h.map(k=>({id:k.id,t:k.type,n:k.name||void 0,txt:k.label?k.label.length>160?k.label.slice(0,160)+"...":k.label:void 0,v:k.value||void 0,opt:k.options&&k.options.length?k.options.slice(0,20).map(se=>se.label||se.value):void 0})))})()}
