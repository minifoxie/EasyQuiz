...
[HTML]:
${o.htmlSnippet.slice(0,c).replace(/\s+/g," ")}`:"",h="";if(s&&typeof document<"u")try{let f=Array.from(document.querySelectorAll('input:not([type=hidden]), textarea, select, button, [role="button"], [role="radio"], [role="checkbox"], [role="option"], [onclick], [data-action], a[href]:not([href="#"]), [tabindex]:not([tabindex="-1"])')).filter(v=>{let x=v,C=x.getBoundingClientRect?.()||{width:0,height:0};return C.width>0&&C.height>0&&!x.closest("#easyquiz-shadow-root, .eq-sidebar")}).slice(0,40).map(v=>{let x=v,C=x.tagName.toLowerCase(),w=x.id?`#${x.id}`:"",q=x.className&&typeof x.className=="string"?`.${x.className.trim().split(/\s+/).slice(0,2).join(".")}`:"",T=(x.textContent||x.value||x.getAttribute("aria-label")||"").trim().slice(0,60),I=x.getAttribute("type")||x.getAttribute("role")||"";return`${C}${w}${q}[${I}] txt="${T}"`});f.length>0&&(h=`
[DOM-INTERATIVO]:
${f.join(`
`)}`)}catch{}let d=He(),m=d.length>0?`
[MEM\xD3RIA]:
${d.join(" | ")}
`:"",p=o.controls.filter(f=>f.role!=="navigation"),b=o.controls.filter(f=>f.role==="navigation"),y=Wt(o.sourceUrl,o.htmlSnippet),g=y?`
${y}
`:"";return`--- AN\xC1LISE ---
...
[RESPOSTAS]:
${(()=>{if(p.length===0)return"Nenhuma";let f=p.filter(A=>A.type==="checkbox"||A.type==="chk"),v=new Set(p.filter(A=>A.type==="radio").map(A=>A.name).filter(Boolean)),x=f.filter(A=>!A.name||!v.has(A.name)),C=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(o.questionText),w=x.length>=2||C,q=v.size>1||/verdadeir|fals[oa]|\bv\s*\/\s*f\b|julgue|itens/i.test(o.questionText)&&v.size>=1,T=p.every(A=>A.type==="radio"||A.type==="chk")&&v.size===1&&!w&&!q,I=p.filter(A=>A.type==="text"||A.type==="number"||A.type==="val"||A.tag==="input"||A.tag==="textarea"),M=I.length>=2;return(q?`[GRADE VERDADEIRO/FALSO (${v.size||"m\xFAltiplas"} afirma\xE7\xF5es): voc\xEA DEVE julgar e marcar exatamente 1 op\xE7\xE3o (V ou F) para CADA uma das ${v.size} afirma\xE7\xF5es \u2014 emita ${v.size} a\xE7\xF5es chk separadas + adv]
...
`;function Vt(){try{if(typeof document>"u"||!document.head)return;if(!document.getElementById("eq-image-pulse-style")){let o=document.createElement("style");o.id="eq-image-pulse-style",o.textContent=yo,document.head.appendChild(o)}}catch{}}function pe(){ve&&(ve.style.removeProperty("outline"),ve.style.removeProperty("outline-offset"),ve.style.removeProperty("position"),ve=null);for(let o of ue)o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("background-color"),o.style.removeProperty("box-shadow"),o.removeAttribute("data-easyquiz-highlight");ue=[];for(let o of rt)o.style.removeProperty("animation"),o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("box-shadow"),o.removeAttribute("data-easyquiz-image-highlight");rt=[];for(let o of lt)try{o.remove()}catch{}if(lt=[],ye){try{ye.remove()}catch{}ye=null}}function ct(o){Vt();for(let e of o){if(!e||!(e instanceof(typeof HTMLElement<"u"?HTMLElement:e.constructor)))continue;let t=e;t.style.outline="3px solid #ffd600",t.style.outlineOffset="3px",t.style.animation="eq-image-pulse-yellow-white 1.2s ease-in-out infinite",t.setAttribute("data-easyquiz-image-highlight","true"),rt.push(t);try{let a=t.parentElement;if(a&&!a.querySelector("[data-easyquiz-capture-badge]")){window.getComputedStyle(a).position==="static"&&(a.style.position="relative");let n=document.createElement("div");n.setAttribute("data-easyquiz-capture-badge","true"),n.textContent="\u{1F4F7} Capturado pela IA",n.style.cssText=`
    ...
    `,this.shadow.appendChild(this.element),this.element.querySelector("#eq-fah-pill").addEventListener("click",()=>this.restore()),this.element.querySelector("#eq-fah-min-btn").addEventListener("click",()=>this.minimize()),this.element.querySelector("#eq-fah-close-btn").addEventListener("click",()=>this.hide());let r=this.element.querySelector("#eq-fah-copy-md-btn");r.addEventListener("click",()=>this.copyMarkdownToClipboard(r));let n=this.element.querySelector("#eq-fah-copy-all-btn");n.addEventListener("click",()=>this.copyMarkdownToClipboard(n));let l=this.element.querySelector("#eq-fah-header");this.setupDraggable(l)}setupDraggable(e){let t=a=>{if(a.target.closest(".eq-fah-btn"))return;a.preventDefault(),this.isDragging=!0,this.dragStartX=a.clientX,this.dragStartY=a.clientY;let r=this.element.getBoundingClientRect();this.initialLeft=r.left,this.initialTop=r.top;let n=s=>{if(!this.isDragging||!this.element)return;let i=s.clientX-this.dragStartX,c=s.clientY-this.dragStartY,u=Math.max(10,window.innerWidth-this.element.offsetWidth-10),h=Math.max(10,window.innerHeight-this.element.offsetHeight-10),d=Math.min(Math.max(10,this.initialLeft+i),u),m=Math.min(Math.max(10,this.initialTop+c),h);this.element.style.left=`${d}px`,this.element.style.top=`${m}px`},l=()=>{this.isDragging=!1,window.removeEventListener("mousemove",n),window.removeEventListener("mouseup",l)};window.addEventListener("mousemove",n),window.addEventListener("mouseup",l)};e.addEventListener("mousedown",t)}renderContent(){if(!this.element||!this.currentPlan)return;let e=this.element.querySelector("#eq-fah-body"),t=this.element.querySelector("#eq-fah-pill-text"),a=this.element.querySelector("#eq-fah-pill-badge");e.innerHTML="";let r=this.currentPlan,n=r.actions.filter(m=>m.t==="drag"),l=r.actions.filter(m=>{if(m.t!=="val")return!1;let p=E(m.id||"").toLowerCase();return!/continu|avan[cç]|pr[oó]xim|submet|enviar|check|verific/i.test(p)}),s=r.actions.filter(m=>m.t==="clk"||m.t==="chk"),i=r.actions.filter(m=>m.t==="sel"),c=n.length||l.length||s.length||i.length,u=document.createElement("div");u.className="eq-fah-meta";let h=document.createElement("span");h.textContent=`Modo: ${r.mode.replace("_"," ")}`;let d=document.createElement("span");if(d.className="eq-fah-meta-badge",d.textContent=`${Math.round(r.confidence*100)}% Confian\xE7a`,u.append(h,d),e.appendChild(u),n.length>0||r.mode==="categorizacao"||r.mode==="arrastar_soltar"){t.textContent=`Categoriza\xE7\xE3o (${n.length} itens)`,a.textContent=String(n.length);let m={};for(let p of n){let b=E(p.to)||"Geral";m[b]||(m[b]=[]),m[b].push(E(p.from))}for(let[p,b]of Object.entries(m)){let y=document.createElement("div"),g=/fato|true|verdadeiro|sim/i.test(p),f=/opini[aã]o|false|falso|n[aã]o/i.test(p);y.className=`eq-fah-group ${g?"group-fato":f?"group-opiniao":""}`;let v=document.createElement("div");v.className="eq-fah-group-title",v.textContent=`\u{1F4C1} ${p} (${b.length})`,y.appendChild(v);let x=document.createElement("div");x.className="eq-fah-group-items";for(let C of b){let w=document.createElement("div");w.className="eq-fah-item";let q=document.createElement("span");q.className="eq-fah-item-text",q.textContent=C,w.appendChild(q);let T=document.createElement("button");T.className="eq-fah-copy-inline",T.textContent="Copiar",T.addEventListener("click",()=>{navigator.clipboard.writeText(C),T.textContent="\u2713 Copiado",setTimeout(()=>T.textContent="Copiar",1200)}),w.appendChild(T),x.appendChild(w)}y.appendChild(x),e.appendChild(y)}}else if(l.length>0){t.textContent=`Preenchimento (${l.length} campos)`,a.textContent=String(l.length);let m=document.createElement("div");m.className="eq-fah-group";let p=document.createElement("div");p.className="eq-fah-group-title",p.textContent="\u{1F4DD} Respostas para os Campos de Texto:",m.appendChild(p);let b=document.createElement("div");b.className="eq-fah-group-items";for(let y=0;y<l.length;y++){let g=l[y],f=document.createElement("div");f.className="eq-fah-item";let v=Me(g.id);(!v||/^[#\.\$]|input|mat-|cell|field|q[0-9]|eq-/i.test(v))&&(v=`Campo ${y+1}`);let x=String(g.v??""),C=document.createElement("div");C.className="eq-fah-field-box";let w=document.createElement("div");w.className="eq-fah-field-label",w.textContent=v,C.appendChild(w);let q=document.createElement("div");q.className="eq-fah-field-val",q.textContent=x,C.appendChild(q),f.appendChild(C);let T=document.createElement("button");T.className="eq-fah-copy-inline",T.textContent="Copiar",T.addEventListener("click",()=>{navigator.clipboard.writeText(x),T.textContent="\u2713 Copiado",setTimeout(()=>T.textContent="Copiar",1200)}),f.appendChild(T),b.appendChild(f)}m.appendChild(b),e.appendChild(m)}else if(s.length>0){t.textContent=`Op\xE7\xF5es (${s.length} marcadas)`,a.textContent=String(s.length);let m=document.createElement("div");m.className="eq-fah-group";let p=document.createElement("div");p.className="eq-fah-group-title",p.textContent="\u{1F3AF} Alternativa(s) Correta(s):",m.appendChild(p);let b=document.createElement("div");b.className="eq-fah-group-items";for(let y=0;y<s.length;y++){let g=s[y],f=document.createElement("div");f.className="eq-fah-item";let v=Me(g.id);(!v||/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(v))&&g.v&&(v=String(g.v)),v=E(v),/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(v)&&(v="");let x="",C=v.match(/^(\([A-Za-z0-9]\)|[A-Za-z0-9][\)\.\:\-])\s*(.*)$/);C?(x=C[1].replace(/[\(\)\.\:\-\s]/g,"").toUpperCase(),v=C[2].trim()||v):s.length>1&&(x=String.fromCharCode(65+y));let w=document.createElement("div");if(w.style.display="flex",w.style.alignItems="center",w.style.gap="8px",w.style.flex="1",x){let I=document.createElement("span");I.className="eq-fah-letter-badge",I.textContent=x,w.appendChild(I)}let q=document.createElement("span");q.className="eq-fah-item-text",q.textContent=v||(x?`Alternativa ${x}`:"Alternativa Selecionada"),w.appendChild(q),f.appendChild(w);let T=document.createElement("button");T.className="eq-fah-copy-inline",T.textContent="Copiar",T.addEventListener("click",()=>{navigator.clipboard.writeText(v||x),T.textContent="\u2713 Copiado",setTimeout(()=>T.textContent="Copiar",1200)}),f.appendChild(T),b.appendChild(f)}m.appendChild(b),e.appendChild(m)}else if(i.length>0){t.textContent=`Sele\xE7\xE3o (${i.length} listas)`,a.textContent=String(i.length);let m=document.createElement("div");m.className="eq-fah-group";let p=document.createElement("div");p.className="eq-fah-group-title",p.textContent="\u{1F4CB} Op\xE7\xF5es para Selecionar na Lista:",m.appendChild(p);let b=document.createElement("div");b.className="eq-fah-group-items";for(let y=0;y<i.length;y++){let g=i[y],f=document.createElement("div");f.className="eq-fah-item";let v=Me(g.id);(!v||/^[#\.\$]|select|input|mat-|cell|field|q[0-9]|eq-/i.test(v))&&(v=`Lista ${y+1}`);let w=(Array.isArray(g.v)?g.v:[String(g.v??"")]).map(k=>{let A=H(g.id,void 0,!0)||H(E(g.id),void 0,!0),z=A instanceof HTMLSelectElement?A:A?.querySelector("select");if(z){let D=E(k).toLowerCase();for(let K=0;K<z.options.length;K++){let N=z.options[K];if(N.value.toLowerCase()===D||E(N.textContent).toLowerCase()===D){let B=E(N.textContent);if(B&&!B.toLowerCase().includes("selecione"))return B}}}return k}).join(", "),q=document.createElement("div");q.className="eq-fah-field-box";let T=document.createElement("div");T.className="eq-fah-field-label",T.textContent=v,q.appendChild(T);let I=document.createElement("div");I.className="eq-fah-field-val",I.textContent=w,q.appendChild(I),f.appendChild(q);let M=document.createElement("button");M.className="eq-fah-copy-inline",M.textContent="Copiar",M.addEventListener("click",()=>{navigator.clipboard.writeText(w),M.textContent="\u2713 Copiado",setTimeout(()=>M.textContent="Copiar",1200)}),f.appendChild(M),b.appendChild(f)}m.appendChild(b),e.appendChild(m)}else{t.textContent="Gabarito",a.textContent="0";let m=document.createElement("div");m.style.padding="10px",m.style.color="#888",m.textContent="Nenhuma resposta direta para exibir.",e.appendChild(m)}if(r.rationale){let m=document.createElement("div");m.className="eq-fah-rationale",m.textContent=`\u{1F4A1} Racioc\xEDnio da IA: ${r.rationale}`,e.appendChild(m)}}generateMarkdown(){if(!this.currentPlan)return"";let e=this.currentPlan,t=[];t.push("# Gabarito da Quest\xE3o \u2014 EasyQuiz Pro"),t.push(`- **Modo:** ${e.mode}`),t.push(`- **Confian\xE7a:** ${(e.confidence*100).toFixed(0)}%`),t.push("");let a=e.actions.filter(s=>s.t==="drag"),r=e.actions.filter(s=>s.t==="val"),n=e.actions.filter(s=>s.t==="clk"||s.t==="chk"),l=e.actions.filter(s=>s.t==="sel");if(a.length>0){t.push("## \u{1F4C2} Categoriza\xE7\xE3o:");let s={};for(let i of a){let c=E(i.to)||"Geral";s[c]||(s[c]=[]),s[c].push(E(i.from))}for(let[i,c]of Object.entries(s)){t.push(`### Categoria: ${i}`);for(let u of c)t.push(`- ${u}`);t.push("")}}else if(r.length>0){t.push("## \u270F\uFE0F Respostas para Preenchimento:");for(let s of r){let i=E(s.id);t.push(`- **${i||"Campo"}:** \`${s.v}\``)}t.push("")}else if(n.length>0){t.push("## \u2705 Alternativas Corretas:");for(let s=0;s<n.length;s++){let i=n[s],c=Me(i.id);(!c||/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(c))&&i.v&&(c=String(i.v)),c=E(c),/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(c)&&(c="");let u=n.length>1?`${String.fromCharCode(65+s)}) `:"";t.push(`- [x] ${u}${c||"Alternativa "+String.fromCharCode(65+s)}`)}t.push("")}else if(l.length>0){t.push("## \u{1F4CB} Op\xE7\xF5es Selecionadas em Lista:");for(let s of l){let i=E(s.id)||"Lista",c=Array.isArray(s.v)?s.v.join(", "):String(s.v??"");t.push(`- **${i}:** \`${c}\``)}t.push("")}return e.rationale&&(t.push("---"),t.push(`**\u{1F4A1} Racioc\xEDnio:** ${e.rationale}`)),t.join(`
