...
[RESPOSTAS]:
${(()=>{if(b.length===0)return"Nenhuma";let h=b.filter(L=>L.type==="checkbox"||L.type==="chk"),g=new Set(b.filter(L=>L.type==="radio").map(L=>L.name).filter(Boolean)),v=h.filter(L=>!L.name||!g.has(L.name)),T=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(o.questionText),A=v.length>=2||T,w=g.size>1||/verdadeir|fals[oa]|\bv\s*\/\s*f\b|julgue|itens/i.test(o.questionText)&&g.size>=1,E=b.every(L=>L.type==="radio"||L.type==="chk")&&g.size===1&&!A&&!w,I=b.filter(L=>L.type==="text"||L.type==="number"||L.type==="val"||L.tag==="input"||L.tag==="textarea"),P=I.length>=2;return(w?`[GRADE VERDADEIRO/FALSO (${g.size||"m\xFAltiplas"} afirma\xE7\xF5es): voc\xEA DEVE julgar e marcar exatamente 1 op\xE7\xE3o (V ou F) para CADA uma das ${g.size} afirma\xE7\xF5es \u2014 emita ${g.size} a\xE7\xF5es chk separadas + adv]
`:A?`[MULTI-SELE\xC7\xC3O: marque TODOS os corretos, pode ser 2 ou mais]
`:E?`[ESCOLHA-\xDAnica: marque APENAS 1 op\xE7\xE3o]
`:P?`[M\xDALTIPLOS CAMPOS DE PREENCHIMENTO (${I.length} campos): emita uma a\xE7\xE3o val para CADA um dos ${I.length} campos abaixo com seu id exato]
