 ...
 */
"use strict";(()=>{var ne={apiKey:"",apiKeys:[],model:"gemini-3.8-flash",uiMode:"easy",modeHint:"",engine:"smart",dryRun:!1,autoApply:!0,autoAdvance:!1,hostDarkMode:!0,useVision:!1,confidenceThreshold:.8};function N(o){if(!o||typeof o!="string")return!1;let e=o.toLowerCase().trim().replace(/^models\//,"");if(!e.includes("gemini"))return!1;let t=["imagen","image","veo","omni","video","embedding","embed","tts","audio","speech","voice","sound","live","transcribe","bidi","aqa","learnlm","deep-research","computer-use","robotics","rt-1","rt-2","mediapipe","cyber","latest","-ultra","experimental"];for(let n of t)if(e.includes(n))return!1;return!(!e.includes("flash")&&!e.includes("pro"))}var Ge="easyquiz_settings_v2",se="easyquiz_activity_metrics";function Fe(){try{let o=localStorage.getItem(Ge);if(!o){let l=localStorage.getItem("easyquiz_settings_v1");if(l){let s=JSON.parse(l);return{...ne,apiKey:s.apiKey||""}}return{...ne}}let e=JSON.parse(o),t=typeof e.model=="string"&&N(e.model)?e.model:ne.model,n=Array.isArray(e.apiKeys)?e.apiKeys.map(l=>typeof l=="string"?l.trim().replace(/^["']|["']$/g,""):"").filter(l=>l.length>5):[],i=typeof e.apiKey=="string"?e.apiKey.trim().replace(/^["']|["']$/g,""):"";return n.length===0&&i&&(n=[i]),{apiKey:n[0]||i||ne.apiKey,apiKeys:n,model:t,uiMode:e.uiMode==="easy"||e.uiMode==="advanced"?e.uiMode:ne.uiMode,modeHint:e.modeHint??"",engine:e.engine??"smart",dryRun:!!e.dryRun,autoApply:e.autoApply!==void 0?!!e.autoApply:!0,autoAdvance:!!e.autoAdvance,hostDarkMode:e.hostDarkMode!==void 0?!!e.hostDarkMode:!0,useVision:!!e.useVision,confidenceThreshold:typeof e.confidenceThreshold=="number"?e.confidenceThreshold:ne.confidenceThreshold}}catch{return{...ne}}}function wt(){try{localStorage.removeItem(Ge),localStorage.removeItem("easyquiz_settings_v1"),localStorage.removeItem(se),sessionStorage.removeItem(se);let o=[];for(let e=0;e<localStorage.length;e++){let t=localStorage.key(e);t&&(t.startsWith("eq_")||t.startsWith("easyquiz_"))&&o.push(t)}o.forEach(e=>localStorage.removeItem(e)),Qe()}catch(o){console.warn("[EasyQuiz] Erro ao resetar dados:",o)}}function xe(o){try{let e=localStorage.getItem("eq_domain_cache_"+o);if(!e)return{};let t=JSON.parse(e);if(t.advanceSelector&&/inject|injetar/i.test(t.advanceSelector)){t.advanceSelector=void 0;try{localStorage.removeItem("eq_domain_cache_"+o)}catch{}}return t}catch{return{}}}function Ue(o,e){if(e.advanceSelector&&/inject|injetar/i.test(e.advanceSelector))return;let n={...xe(o),...e};try{localStorage.setItem("eq_domain_cache_"+o,JSON.stringify(n))}catch(i){console.warn("[EasyQuiz] Erro cache de dominio:",i)}}function Et(o){let e=Fe(),t=Array.isArray(o.apiKeys)?o.apiKeys.map(a=>typeof a=="string"?a.trim().replace(/^["']|["']$/g,""):"").filter(a=>a.length>5):e.apiKeys,n;typeof o.apiKey=="string"?n=o.apiKey.trim().replace(/^["']|["']$/g,""):Array.isArray(o.apiKeys)&&o.apiKeys.length>0?n=t[0]||"":n=e.apiKey,n&&!t.includes(n)&&(t=[n,...t]),t.length>0&&(!n||!t.includes(n))&&(n=t[0]);let i={...e,...o,apiKey:n,apiKeys:t};try{localStorage.setItem(Ge,JSON.stringify(i))}catch(a){console.warn("[EasyQuiz] Falha ao persistir configura\xE7\xF5es no localStorage:",a)}return i}var ie=[],qt=12,Ft=1200;function Ct(o){let e=o.trim().replace(/\s+/g," ").slice(0,Ft);e&&!ie.includes(e)&&(ie.push(e),ie.length>qt&&(ie=ie.slice(-qt)))}function ke(){return ie}function Qe(){ie=[]}function Tt(){return{startTime:Date.now(),totalElapsedMs:0,completedQuestionsCount:0,averageDurationMs:0,records:[]}}var ye=Tt();function de(){try{localStorage.removeItem(se)}catch{}return ye}function Ut(o){ye=o;try{let e=JSON.stringify(o);sessionStorage.setItem(se,e),localStorage.removeItem(se)}catch{}}function Ye(o){let e=ye,t=Date.now(),n=e.records[e.records.length-1];if(n&&n.id===o.id&&t-n.timestamp<3e3)return e;let i={...o,timestamp:t},a=[...e.records,i],l=a.filter(d=>d.status==="answered"||d.status==="verified").length,s=a.reduce((d,m)=>d+m.durationMs,0),r=l>0?Math.round(s/l):0,c={startTime:e.startTime||t,totalElapsedMs:Math.max(t-(e.startTime||t),s),completedQuestionsCount:l,averageDurationMs:r,records:a};return Ut(c),c}function qe(){ye=Tt();try{sessionStorage.removeItem(se),localStorage.removeItem(se)}catch{}return ye}var At=`Voc\xEA \xE9 o motor operacional inteligente do EasyQuiz. Sa\xEDda EXCLUSIVA em JSON minificado, sem markdown ou conversa.
...
- Para escolha \xFAnica (r\xE1dio, [ESCOLHA-\xDAnica]): marque com clk ou chk APENAS a alternativa mais correta (somente 1).
...
[RESPOSTAS]:
${(()=>{if(c.length===0)return"Nenhuma";let h=c.filter(b=>b.type==="checkbox"||b.type==="chk"),f=new Set(c.filter(b=>b.type==="radio").map(b=>b.name).filter(Boolean)),v=h.filter(b=>!b.name||!f.has(b.name)).length>=2,g=c.every(b=>b.type==="radio"||b.type==="chk")&&f.size>=1&&!v;return(v?`[MULTI-SELE\xC7\xC3O: marque TODOS os corretos, pode ser 2 ou mais]
`:g?`[ESCOLHA-\xDAnica: marque APENAS 1 op\xE7\xE3o]
`:"")+JSON.stringify(c.map(b=>({id:b.id,t:b.type,n:b.name||void 0,txt:b.label,v:b.value||void 0,opt:b.options.length?b.options:void 0})))})()}
...
`):"Nenhum anexo visual."}
[/DADOS]
Sa\xEDda em JSON v\xE1lido.`}var Yt=new Set(["question","info","start","conclusion"]),Jt=new Set(["texto_livre","escolha_unica","escolha_multipla","verdadeiro_falso","preenchimento","acao_sem_resposta","categorizacao","ordenacao","arrastar_soltar"]),Xt=new Set(["val","chk","sel","clk","adv","js","drag"]),Wt=150,Ee=2e3;function U(o,e=""){return o==null?e:typeof o=="string"?o.trim().slice(0,Ee):typeof o=="number"||typeof o=="boolean"?String(o).trim().slice(0,Ee):e}function Zt(o,e){if(!o||typeof o!="object")return null;let t=o,n=t.t;if(typeof n!="string"||!Xt.has(n))return null;if(n==="adv"){let s=t.id??t.target??t.name??t.selector;return{t:"adv",...U(s)?{id:U(s,"").slice(0,500)}:{}}}if(n==="drag"){let s=U(t.from??t.source),r=U(t.to??t.target??t.destination);return!s||!r?null:{t:"drag",from:s.slice(0,500),to:r.slice(0,500)}}if(n==="js"){let s=U(t.v??t.code??t.script);return!s||s.length>8e3?null:{t:"js",v:s}}let i=t.id??t.target??t.name??t.selector??t.element;(i==null||i==="")&&n==="val"&&(i="1");let a=U(i).slice(0,500);if(!a)return null;if(n==="val"){let s=t.v!==void 0?t.v:t.value!==void 0?t.value:t.val!==void 0?t.val:t.text!==void 0?t.text:t.answer;return{t:"val",id:a,v:U(s).slice(0,Ee)}}if(n==="sel"){let s=t.v!==void 0?t.v:t.value!==void 0?t.value:t.val!==void 0?t.val:t.values,c=(Array.isArray(s)?s:[s]).map(d=>U(d).slice(0,500)).filter(Boolean);return{t:"sel",id:a,v:c}}if(n==="chk"){let s=t.c===!1||t.c==="false"||t.c===0||t.c==="0"||t.c==="off"||t.c==="unchecked"||t.c==="desmarcar",r={t:"chk",id:a,c:!s};return t.v!==void 0&&(r.v=U(t.v).slice(0,Ee)),r}let l={t:"clk",id:a};if(t.c!==void 0){let s=t.c===!1||t.c==="false"||t.c===0||t.c==="0"||t.c==="off"||t.c==="unchecked"||t.c==="desmarcar";l.c=!s}return t.v!==void 0&&(l.v=U(t.v).slice(0,Ee)),Array.isArray(t.co)&&t.co.length===2&&t.co.every(s=>typeof s=="number"&&Number.isFinite(s))&&(l.co=[t.co[0],t.co[1]]),l}function Lt(o){if(!o||typeof o!="object")return{pageType:"info",mode:"acao_sem_resposta",confidence:.5,rationale:"Resposta estruturada n\xE3o identificada; avan\xE7ando como informativo.",actions:[{t:"adv"}]};let e=o,t=e.pageType,n=e.mode;(typeof t!="string"||!Yt.has(t))&&(t="question"),(typeof n!="string"||!Jt.has(n))&&(n="escolha_unica");let i=Array.isArray(e.actions)?e.actions:[],a=[];for(let c=0;c<Math.min(i.length,Wt);c++){let d=Zt(i[c],c);d&&a.push(d)}let l=a.filter(c=>c.t!=="adv"),s=a.some(c=>c.t==="adv");t==="conclusion"?a.length=0:t==="info"||t==="start"?s||a.push({t:"adv"}):t==="question"&&!s&&a.push({t:"adv"});let r=typeof e.confidence=="number"&&Number.isFinite(e.confidence)?Math.min(1,Math.max(0,e.confidence)):.85;return{pageType:t,mode:n,confidence:r,rationale:U(e.rationale,"Plano validado e auto-recuperado."),actions:a,...U(e.memoryToStore)?{memoryToStore:U(e.memoryToStore)}:{},...e.needsMoreContext?{needsMoreContext:!!e.needsMoreContext}:{}}}var ee=class{keys=new Map;constructor(e=[]){this.init(e)}init(e){let t=new Map(this.keys);this.keys.clear(),Array.from(new Set(e.map(i=>i.trim().replace(/^["']|["']$/g,"")).filter(i=>i.length>5))).forEach((i,a)=>{let l=this.generateId(i),s=t.get(l)||t.get(i);this.keys.set(l,{id:l,key:i,label:s?.label||`Chave ${a+1}`,addedAt:s?.addedAt||Date.now(),lastUsedAt:s?.lastUsedAt,lastLatencyMs:s?.lastLatencyMs,cooldownUntil:s?.cooldownUntil,errorCount:s?.errorCount||0,lastError:s?.lastError,winCount:s?.winCount||0})})}generateId(e){let t=0;for(let n=0;n<e.length;n++)t=(t<<5)-t+e.charCodeAt(n),t|=0;return`key_${Math.abs(t).toString(36).slice(0,8)}`}static maskKey(e){let t=e.trim().replace(/^["']|["']$/g,"");return t.length<=10?"\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022":`${t.slice(0,6)}...${t.slice(-4)}`}getAllKeys(){let e=Date.now();return Array.from(this.keys.values()).map(t=>{let n=Math.max(0,(t.cooldownUntil||0)-e);return{...t,isCooldown:n>0,remainingCooldownMs:n}})}getHealthyKeys(){let e=Date.now();return Array.from(this.keys.values()).filter(t=>(t.cooldownUntil||0)<=e&&(t.errorCount||0)<5)}getBestKey(){let e=this.getHealthyKeys();if(e.length>0)return e.sort((n,i)=>{let a=n.lastLatencyMs??99999,l=i.lastLatencyMs??99999;return a-l}),e[0].key;let t=Array.from(this.keys.values());return t.length>0?(t.sort((n,i)=>(n.cooldownUntil||0)-(i.cooldownUntil||0)),t[0].key):""}getDiverseKeys(e){let t=this.getHealthyKeys();if(t.length===0){let i=this.getBestKey();return i?[i]:[]}t.sort((i,a)=>{let l=i.lastLatencyMs??99999,s=a.lastLatencyMs??99999;return l-s});let n=[];for(let i=0;i<e;i++){let a=t[i%t.length];n.push(a.key)}return n}markQuotaHit(e,t=5e3){let n=this.findKeyObj(e);n&&(n.cooldownUntil=Date.now()+t,n.errorCount=(n.errorCount||0)+1,n.lastError=`Cota tempor\xE1ria atingida (HTTP 429). Cooldown de ${t/1e3}s ativado.`)}markOverloaded(e,t=5e3){let n=this.findKeyObj(e);n&&(n.cooldownUntil=Date.now()+t,n.errorCount=(n.errorCount||0)+1,n.lastError=`Servidores sobrecarregados (HTTP 503). Cooldown de ${t/1e3}s ativado.`)}markSuccess(e,t){let n=this.findKeyObj(e);n&&(n.lastLatencyMs=t,n.lastUsedAt=Date.now(),n.errorCount=0,n.lastError=void 0,n.cooldownUntil=void 0)}markWinner(e){let t=this.findKeyObj(e);t&&(t.winCount=(t.winCount||0)+1)}markInvalid(e,t){let n=this.findKeyObj(e);n&&(n.errorCount=99,n.lastError=t)}addKey(e,t){let n=e.trim().replace(/^["']|["']$/g,"");if(!n)return{ok:!1,message:"Chave n\xE3o pode ser vazia."};if(n.length<15)return{ok:!1,message:"Chave de API inv\xE1lida ou muito curta."};let i=this.generateId(n);if(this.keys.has(i))return{ok:!1,message:"Esta chave de API j\xE1 est\xE1 cadastrada."};let a={id:i,key:n,label:t?.trim()||`Chave ${this.keys.size+1}`,addedAt:Date.now(),errorCount:0};return this.keys.set(i,a),{ok:!0,message:"Chave adicionada com sucesso!",keyItem:a}}updateKey(e,t,n){let i=this.keys.get(e);if(!i)return{ok:!1,message:"Chave n\xE3o encontrada."};let a=t.trim().replace(/^["']|["']$/g,"");return!a||a.length<15?{ok:!1,message:"Chave de API inv\xE1lida."}:(i.key=a,n!==void 0&&(i.label=n.trim()),i.errorCount=0,i.cooldownUntil=void 0,i.lastError=void 0,{ok:!0,message:"Chave atualizada com sucesso!"})}removeKey(e){if(this.keys.size<=1)return{ok:!1,message:"Voc\xEA precisa manter pelo menos 1 chave de API cadastrada."};let t=this.findKeyObj(e);return t?(this.keys.delete(t.id),{ok:!0,message:"Chave removida com sucesso."}):{ok:!1,message:"Chave n\xE3o encontrada."}}exportRawKeys(){return Array.from(this.keys.values()).map(e=>e.key)}size(){return this.keys.size}findKeyObj(e){if(this.keys.has(e))return this.keys.get(e);for(let t of this.keys.values())if(t.key===e)return t}},k=new ee;var ue=[{id:"gemini-3.8-flash",name:"Gemini 3.8 Flash (Mais Inteligente 2026)",description:"Modelo flagship Flash lan\xE7ado em Set/2026. Ultra-r\xE1pido e altamente capaz.",stable:!0},{id:"gemini-3.7-flash",name:"Gemini 3.7 Flash (Agentic)",description:"Alta capacidade para racioc\xEDnio multimodal e workflows ag\xEAnticos.",stable:!0},{id:"gemini-3.6-flash",name:"Gemini 3.6 Flash (Est\xE1vel)",description:"Modelo est\xE1vel e confi\xE1vel com excelente velocidade.",stable:!0},{id:"gemini-3.5-flash",name:"Gemini 3.5 Flash (R\xE1pido)",description:"Modelo de alta performance para tarefas r\xE1pidas.",stable:!0},{id:"gemini-3.5-flash-lite",name:"Gemini 3.5 Flash-Lite (Econ\xF4mico)",description:"Modelo econ\xF4mico de alta velocidade para volume elevado.",stable:!0},{id:"gemini-2.5-flash",name:"Gemini 2.5 Flash (Legacy R\xE1pido)",description:"Modelo legacy com zero-thinking suportado. Ultra-baixa lat\xEAncia.",stable:!0},{id:"gemini-2.5-pro",name:"Gemini 2.5 Pro (Legacy Avan\xE7ado)",description:"Modelo legacy avan\xE7ado para quest\xF5es de alta complexidade.",stable:!0}],Je=["gemini-3.8-flash","gemini-3.6-flash","gemini-3.5-flash"],eo={"gemini-2.5-flash":"gemini-3.6-flash","gemini-2.0-flash":"gemini-3.5-flash","gemini-2.0-flash-lite":"gemini-3.5-flash-lite","gemini-1.5-flash":"gemini-3.5-flash","gemini-1.5-pro":"gemini-3.6-flash"};function Xe(o){return eo[o]??o}var Ce=null;function to(o){let e={temperature:0,maxOutputTokens:1200,response_mime_type:"application/json",response_schema:oo};return/gemini-3\.[567]-flash/i.test(o)?e.thinkingConfig={thinkingLevel:"none"}:/gemini-3\.[89]|gemini-3\.[1-9][0-9]/i.test(o)?e.thinkingConfig={thinkingLevel:"low"}:/gemini-2\.5-flash/i.test(o)&&(e.thinkingConfig={thinkingBudget:0}),e}var oo={type:"OBJECT",properties:{pageType:{type:"STRING",enum:["question","info","start","conclusion"]},mode:{type:"STRING",enum:["texto_livre","escolha_unica","escolha_multipla","verdadeiro_falso","preenchimento","acao_sem_resposta","categorizacao","ordenacao","arrastar_soltar"]},confidence:{type:"NUMBER"},rationale:{type:"STRING"},memoryToStore:{type:"STRING"},actions:{type:"ARRAY",items:{type:"OBJECT",properties:{t:{type:"STRING",enum:["val","chk","sel","clk","adv","js","drag"]},id:{type:"STRING"},v:{},c:{type:"BOOLEAN"},co:{type:"ARRAY",items:{type:"NUMBER"}},from:{type:"STRING"},to:{type:"STRING"}},required:["t"]}}},required:["pageType","mode","confidence","rationale","actions"]};function St(o){let e=o.trim().replace(/^google\//,"").replace(/^models\//,"");return!e||!N(e)?"gemini-3.8-flash":e}function kt(o,e){let t="";try{let n=JSON.parse(o);t=n.error?.message||n.message||""}catch{t=o.slice(0,160)}return/API_KEY_INVALID|API key not valid|key.*invalid|unregistered/i.test(t)?"Chave de API do Gemini inv\xE1lida ou n\xE3o autorizada no Google AI Studio.":/RESOURCE_EXHAUSTED|Quota exceeded/i.test(t)||e===429?"Limite tempor\xE1rio de cota do Gemini (HTTP 429) atingido. Aguardando recupera\xE7\xE3o...":e===404?`HTTP 404: ${t||"Modelo ou endpoint n\xE3o encontrado no Google AI Studio"}`:e===503||/overloaded/i.test(t)?`Servidores Google sobrecarregados (HTTP 503): ${t||"Aguardando"}`:t?`Erro Gemini (HTTP ${e}): ${t}`:`Falha na requisi\xE7\xE3o ao Gemini (HTTP ${e}).`}function no(o){let e=o.trim(),t=e.match(/```(?:json)?\s*([\s\S]*?)\s*```/i);if(t)try{return JSON.parse(t[1].trim())}catch{}try{return JSON.parse(e)}catch{}let n=e.match(/\{[\s\S]*\}/);if(n)try{return JSON.parse(n[0].trim())}catch{}throw new Error("Falha ao decodificar JSON da IA.")}var Ht=(()=>{try{let o=typeof localStorage<"u"?localStorage.getItem("easyquiz_cached_models"):null;if(!o)return null;let e=JSON.parse(o);if(Array.isArray(e)){let t=e.filter(n=>n&&typeof n.id=="string"&&N(n.id));return t.length>0?t:null}return null}catch{return null}})(),He=new Set;async function ze(o){let e=o.trim().replace(/^["']|["']$/g,"");if(!e)return ue;let t=[`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(e)}`,`https://generativelanguage.googleapis.com/v1/models?key=${encodeURIComponent(e)}`];for(let n of t)try{let i=await fetch(n,{headers:{"Content-Type":"application/json","x-goog-api-key":e}});if(!i.ok){let l=await i.text(),s=kt(l,i.status);if(s.includes("inv\xE1lida")||s.includes("n\xE3o autorizada"))throw new Error(s);continue}let a=await i.json();if(Array.isArray(a.models)&&a.models.length>0){let l=a.models.filter(s=>{let r=s.supportedGenerationMethods||[],c=(s.name||"").replace(/^models\//,""),d=r.includes("generateContent");return N(c)&&d}).map(s=>{let r=s.supportedGenerationMethods||[],c=s.name.replace(/^models\//,""),d=s.displayName||c;return{id:c,name:d.includes(c)?d:`${d} (${c})`,description:s.description||"",stable:!/-preview|-experimental|-latest/i.test(c),supportsVision:!/embedding|tts|transcribe|live|image|sound|voice/i.test(c),supportsStructuredOutput:r.includes("generateContent"),supportedGenerationMethods:r,discoveredAt:Date.now()}});if(l.length>0){l.sort((s,r)=>{let c=d=>d==="gemini-3.8-flash"?200:d==="gemini-3.7-flash"?190:d==="gemini-3.6-flash"?180:d==="gemini-3.5-flash"?170:d==="gemini-3.5-flash-lite"?160:d==="gemini-2.5-flash"?130:d.includes("flash")?80:d==="gemini-2.5-pro"?60:d.includes("pro")?50:10;return c(r.id)-c(s.id)}),Ht=l;try{typeof localStorage<"u"&&localStorage.setItem("easyquiz_cached_models",JSON.stringify(l))}catch{}return l}}}catch(i){if(i.message?.includes("Chave de API"))throw i}return ue}async function $e(o){let e=o.trim().replace(/^["']|["']$/g,"");if(!e)return{ok:!1,message:"Insira sua chave de API."};try{let n=await ze(e);if(n.length>0&&n!==ue){let i=n[0];return{ok:!0,message:`Chave v\xE1lida! ${n.length} modelos Gemini dispon\xEDveis em sua conta. Recomendado: ${i.name}`,models:n}}}catch(n){return{ok:!1,message:n instanceof Error?n.message:String(n)}}let t=["gemini-3.8-flash","gemini-3.6-flash","gemini-3.5-flash"];for(let n of t)for(let i of["v1beta","v1"]){let a=`https://generativelanguage.googleapis.com/${i}/models/${n}:generateContent?key=${encodeURIComponent(e)}`;try{if((await fetch(a,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":e},body:JSON.stringify({contents:[{role:"user",parts:[{text:"PING"}]}],generationConfig:{maxOutputTokens:5}})})).ok)return{ok:!0,message:`Chave validada com sucesso no ${n} (${i})!`,models:ue}}catch{}}return{ok:!1,message:"Chave de API inv\xE1lida, sem cota ou sem permiss\xE3o para modelos Gemini."}}async function We(o,e){let t=e.map(c=>c.trim().replace(/^["']|["']$/g,"")).filter(c=>c.length>5);if(t.length===0)return{ok:!1,model:o,key:"",message:"Nenhuma chave dispon\xEDvel."};let n=Xe(St(o)),i=JSON.stringify({contents:[{role:"user",parts:[{text:"PING"}]}],generationConfig:{maxOutputTokens:5}}),a={"Content-Type":"application/json"};async function l(c,d,m){let u=new AbortController,h=setTimeout(()=>u.abort(),m);try{let f=`https://generativelanguage.googleapis.com/v1beta/models/${d}:generateContent?key=${encodeURIComponent(c)}`,y=await fetch(f,{method:"POST",headers:{...a,"x-goog-api-key":c},body:i,signal:u.signal});if(clearTimeout(h),y.ok)return{ok:!0,model:d,key:c,message:`Modelo '${d}' validado com sucesso!`};let v=await y.text().catch(()=>"");throw new Error(`HTTP ${y.status}: ${v.slice(0,80)}`)}catch(f){throw clearTimeout(h),f}}if(t.length>=2){let c=t.slice(0,6);try{return await Promise.any(c.map(m=>l(m,n,8e3)))}catch{}}let s=t[0],r=[n,...Je.filter(c=>c!==n)];for(let c of r)try{let d=await l(s,c,4e3);return c!==n&&(d.message=`Modelo preferido indispon\xEDvel. Validado via fallback '${c}'.`),d}catch{}return{ok:!1,model:n,key:s,message:"Nenhum modelo Gemini respondeu. Verifique sua chave e cota."}}async function ao(o,e,t,n,i){let a=["v1beta","v1"],l=new Error(`Falha ao consultar modelo ${o}`),r={...to(o)};for(let c of a){if(i.aborted)throw new Error("Opera\xE7\xE3o cancelada pelo usu\xE1rio.");let d=`https://generativelanguage.googleapis.com/${c}/models/${o}:generateContent?key=${encodeURIComponent(e)}`,m=Date.now();try{let u=await fetch(d,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":e},body:JSON.stringify({...t,generationConfig:r}),signal:i,keepalive:n});if(!u.ok){let y=await u.text();if(u.status===400&&r.thinkingConfig&&/thinking/i.test(y)){delete r.thinkingConfig;let g=await fetch(d,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":e},body:JSON.stringify({...t,generationConfig:r}),signal:i,keepalive:n});if(g.ok){let p=await g.json(),b=p.candidates?.[0];if(b?.content?.parts?.[0]?.text)return k.markSuccess(e,Date.now()-m),{rawText:b.content.parts[0].text,data:p,usedModel:o,usedKey:e}}}let v=kt(y,u.status);if(u.status===404&&c==="v1beta")continue;throw u.status===429?k.markQuotaHit(e,5e3):u.status===503||/no capacity|overloaded|unavailable/i.test(y)?(k.markOverloaded(e,5e3),He.add(o)):u.status===403||/API_KEY_INVALID/i.test(y)?k.markInvalid(e,v):u.status===404&&He.add(o),new Error(`[${o}|${ee.maskKey(e)}] ${v}`)}let h=await u.json(),f=h.candidates?.[0];if(!f||!f.content?.parts?.[0]?.text)throw new Error(`[${o}|${ee.maskKey(e)}] A IA n\xE3o retornou uma resposta estruturada v\xE1lida.`);return k.markSuccess(e,Date.now()-m),{rawText:f.content.parts[0].text,data:h,usedModel:o,usedKey:e}}catch(u){if(i.aborted)throw u;l=u;let h=l.message||"";if(h.includes("404")||/no longer available/i.test(h)){He.add(o);break}}}throw l}var Ie=new Map;function Mt(o){let e=Ie.get(o);return e===void 0?!1:Date.now()>e?(Ie.delete(o),!1):!0}function io(o,e=6e4){Ie.set(o,Date.now()+e)}function It(){Ie.clear()}async function Ze(o,e,t,n,i){if(i?.aborted)throw new Error("Opera\xE7\xE3o cancelada pelo usu\xE1rio.");let a=Array.isArray(t.apiKeys)&&t.apiKeys.length>0?t.apiKeys:t.apiKey?[t.apiKey]:[];k.init(a);let l=t.apiKey.trim().replace(/^[\"']|[\"']$/g,""),s=k.getBestKey()||l;if(!s)throw new Error("Nenhuma chave de API do Gemini configurada ou dispon\xEDvel.");let r=St(t.model);if(!Ht&&s&&ze(s).catch(()=>{}),i?.aborted)throw new Error("Opera\xE7\xE3o cancelada pelo usu\xE1rio.");let c=Date.now(),d=we(o,e,t),m=[{text:d}];for(let S=0;S<e.length;S++){let H=e[S],z=H.associatedLabel||(H.alt?`Imagem: ${H.alt}`:`Imagem ${S+1}`);m.push({text:`[ANEXO VISUAL ${S+1} - V\xCDNCULO: ${z}]:`}),m.push({inline_data:{mime_type:H.mediaType,data:H.base64}})}let u={system_instruction:{parts:[{text:At}]},contents:[{role:"user",parts:m}]},h=!0;He.clear();let f=Xe(r),y=Ce?Xe(Ce):null,v=[];y&&N(y)&&y!==f&&v.push(y);for(let S of Je)S!==f&&!v.includes(S)&&v.push(S);let g=[];N(f)&&g.push(f);for(let S of v)N(S)&&!g.includes(S)&&g.push(S);g.length<2&&g.push(...Je.filter(S=>!g.includes(S)));let p=k.getHealthyKeys().filter(S=>!Mt(S.key)).sort((S,H)=>(S.lastLatencyMs??99999)-(H.lastLatencyMs??99999)),b=k.getAllKeys().filter(S=>!Mt(S.key)),q=p.length>0?p:b.map(S=>({key:S.key,lastLatencyMs:S.lastLatencyMs,label:S.label})),w=new Set;function C(S,H,z,Z){let G=[];for(let F of H){for(let Y of S){let X=`${Y.key}::${F}`;!w.has(X)&&G.length<z&&(G.push({model:F,key:Y.key,label:Y.label||"Chave",timeout:Z}),w.add(X))}if(G.length>=z)break}return G}let T=async(S,H)=>{if(H.length===0||i?.aborted)return null;let z=H.map(()=>new AbortController),Z=()=>z.forEach(F=>{try{F.abort()}catch{}});i?.addEventListener("abort",Z,{once:!0});let G=H.map(F=>`${F.model.replace("gemini-","")}/${F.label}`).join(" | ");n?.(`\u26A1 ${S}: ${H.length} slot(s) [${G}]...`,"info");try{let F=H.map(async(X,vt)=>{let je=z[vt],yt=setTimeout(()=>{try{je.abort(new Error(`Timeout ${X.timeout/1e3}s (${X.model}|${X.label})`))}catch{je.abort()}},X.timeout);try{let _=await ao(X.model,X.key,u,h,je.signal);clearTimeout(yt);let J=Lt(no(_.rawText));return J.usedModel=_.usedModel,J.durationMs=Date.now()-c,J.promptSent=d,J.tokensUsed=_.data.usageMetadata?.totalTokenCount,J.promptTokens=_.data.usageMetadata?.promptTokenCount,J.candidatesTokens=_.data.usageMetadata?.candidatesTokenCount,J.rawResponse=_.rawText,z.forEach((xt,Gt)=>{if(Gt!==vt)try{xt.abort(new Error("Cancelado: vencedor respondeu."))}catch{xt.abort()}}),{plan:J,rawUsage:_.data.usageMetadata,usedModel:_.usedModel,usedKey:_.usedKey,slotLabel:X.label}}catch(_){clearTimeout(yt);let J=_ instanceof Error?_.message:String(_);throw(J.includes("429")||J.includes("Quota")||J.includes("RESOURCE_EXHAUSTED"))&&io(X.key,6e4),_}}),Y=await Promise.any(F);return i?.removeEventListener("abort",Z),k.markWinner(Y.usedKey),Y.usedModel!==f&&(Ce=Y.usedModel),Y}catch{return i?.removeEventListener("abort",Z),null}},L=q.length,P=L<=1?1:L<=3?2:3,D=/pro/i.test(f),W=(S,H)=>{let z=H?/pro/i.test(H):D;return S===0?z?15e3:1e4:S===1?z?18e3:13e3:z?22e3:16e3},j=6,V=0,ce="";for(;V<j;){if(i?.aborted)throw new Error("Opera\xE7\xE3o cancelada pelo usu\xE1rio.");let S=g.find(F=>!Array.from(w).some(Y=>Y.startsWith(`${q[0]?.key}::${F}`)))??g[0],H=W(V,S),z=C(q,g,P,H);if(z.length===0)break;let Z=V===0?"Onda 1":`Onda ${V+1}`,G=await T(Z,z);if(G){let F=G.plan.durationMs||Date.now()-c,Y=ee.maskKey(G.usedKey);return n?.(`\u2705 ${F}ms via '${G.usedModel}' (${G.slotLabel}: ${Y})`,"info"),G}V++}throw new Error(ce||"Todas as ondas falharam. Verifique sua cota e conex\xE3o com a internet.")}var so=[/\bfetch\b/i,/\bXMLHttpRequest\b/i,/\bWebSocket\b/i,/\b(?:localStorage|sessionStorage|indexedDB)\b/i,/\b(?:eval|Function|AsyncFunction|GeneratorFunction)\b/i,/\bimport(?:Scripts)?\b/i,/\bnavigator\s*\.\s*credentials\b/i,/\b(?:cookie|location\s*=|history\s*\.)/i,/\bwindow\s*\[\s*['"](?:fetch|eval|Function|localStorage|sessionStorage)/i];function Te(o){let e=o?.engine||"smart",t=new Set(["dom","framework","keyboard","drag"]);return o?.autoAdvance&&t.add("navigation"),e==="javascript"&&t.add("javascript"),{engine:e,capabilities:t,maxAttemptsPerAction:e==="command"?1:2,maxActionMs:e==="command"?1500:3e3,allowJavaScript:e==="javascript",allowNavigation:!!o?.autoAdvance}}function et(o,e){if(o.t==="js"&&!e.allowJavaScript)throw new Error("A\xE7\xE3o JavaScript bloqueada pela engine atual. Use o motor JavaScript explicitamente.");if(o.t==="adv"&&!e.allowNavigation)throw new Error("Avan\xE7o autom\xE1tico bloqueado pela pol\xEDtica atual.")}function zt(o){if(!o.trim())throw new Error("JavaScript recusado: c\xF3digo vazio.");if(o.length>8e3)throw new Error("JavaScript recusado: c\xF3digo acima do limite operacional.");if(so.find(t=>t.test(o)))throw new Error("JavaScript recusado: acesso externo, persist\xEAncia ou avalia\xE7\xE3o din\xE2mica n\xE3o permitidos.");if(!/^\s*(?:\/\/[^\n]*\n|\/\*[\s\S]*?\*\/|[\s\S])*\$eq\./.test(o)&&!o.includes("$eq."))throw new Error("JavaScript recusado: use somente a API declarativa $eq.")}var pe=['input:not([type="hidden"])',"textarea","select","button","a","label",'[role="button"]','[role="link"]','[role="radio"]','[role="checkbox"]','[role="option"]','[role="treeitem"]','[role="menuitemcheckbox"]','[role="menuitemradio"]','[contenteditable="true"]','[draggable="true"]',"[aria-grabbed]","[aria-dropeffect]","[data-widget-type]",".perseus-drag-item",".sortable-item",'[data-testid*="drag" i]','[data-testid*="card" i]','[data-testid*="option" i]','[data-testid*="choice" i]','[data-testid*="category" i]',"[data-choice]","[data-option]","[data-answer]","[data-value]",".quiz-option",".option-card",".choice-card",'[class*="option-card" i]','[class*="choice-card" i]','[class*="option-item" i]','[class*="choice-item" i]','[class*="answer-item" i]','[class*="alternative" i]','li[class*="choice" i]','li[class*="option" i]','li[class*="answer" i]','[data-role="dropzone"]',"[data-category]"].join(","),Pe=/(verificar|checar|check|conferir|validar|próxim[oa]|next|continuar|continue|avançar|prosseguir|enviar|submit|concluir|finalizar|terminar|começar|iniciar|start|vamos lá|próxima tarefa|next task|próxima pergunta|next question|marcar como concluíd[oa]|mostrar resumo|entendi|compreendi|ok|leitura concluída|seguir|ir para o exercício|fazer o teste|próximo artigo|ir para a aula)/i,ro=0;function tt(o){try{let e=o.closest('[hidden], [style*="display: none"], [style*="display:none"], [aria-hidden="true"]');if(e&&!he(e))return!1}catch{}try{let e=window.getComputedStyle?window.getComputedStyle(o):o.style;if(e&&(e.display==="none"||e.visibility==="hidden"))return!1}catch{}try{if(typeof o.getBoundingClientRect=="function"){let e=o.getBoundingClientRect();if(e.width>0||e.height>0)return!0}}catch{}return(o.textContent||"").trim().length>0}function R(o){try{if(typeof CSS<"u"&&typeof CSS.escape=="function")return CSS.escape(o)}catch{}return String(o).replace(/["\\]/g,"\\$&")}function M(o){let e=o;if(!e||typeof e.isConnected=="boolean"&&!e.isConnected||he(e))return!1;let t=e.tagName?.toLowerCase();if(["input","select","textarea","button"].includes(t)){let n=e.type?.toLowerCase();if(n==="checkbox"||n==="radio"){if(e.id)try{let a=e.ownerDocument?.querySelector(`label[for="${R(e.id)}"]`);if(a&&tt(a))return!0}catch{}let i=e.closest('label, .option-card, .quiz-option, .choice, .answer, [role="radio"], [role="checkbox"], [class*="option" i], [class*="choice" i], [class*="item" i], li, tr');if(i&&i!==e&&tt(i))return!0}try{if(!e.closest('[hidden], [style*="display: none"], [style*="display:none"], [aria-hidden="true"]')){let a=window.getComputedStyle?window.getComputedStyle(e):e.style;if(!a||a.display!=="none"&&a.visibility!=="hidden"){if(typeof e.getBoundingClientRect=="function"){let l=e.getBoundingClientRect();if(l.width>0||l.height>0)return!0}return!0}}}catch{}}return tt(e)}function lo(o){if(o==null)return"";if(typeof o=="string")return o;if(typeof o=="number"||typeof o=="boolean")return String(o);if(o instanceof Node)return o.textContent||"";try{if(typeof o?.toString=="function"){let e=o.toString();if(typeof e=="string")return e}}catch{}return""}function I(o,e=500){return lo(o).replace(/\s+/g," ").trim().slice(0,e)}function co(o){let e=o.dataset.easyquizId;if(e)return e;let t=`eq-${Date.now().toString(36)}-${(ro+=1).toString(36)}`;return o.dataset.easyquizId=t,t}function he(o){return o?!!(o.closest('#easyquiz-shadow-root, .eq-sidebar, .eq-launcher, [data-easyquiz-ignore="true"], .btn-inject-eq, #btn-inject-script')||o.getAttribute?.("data-easyquiz-ignore")==="true"):!1}var Ae=/(leaderboard|scoreboard|placar|ranking|trophy|pause|pausar|mute|mutar|audio|sound|som|música|music|configuraç|settings|theme|ajuda|help|report|denunciar|feedback|power-?up|streak|coins|fullscreen|full-screen|(?:audio|sound|som|media)[-_ ]*volume|volume[-_ ]*(?:slider|control|level|btn|button|icon|mute)|vol-slider)/i;function B(o){if(!o||typeof o.getAttribute!="function"||typeof Element<"u"&&!(o instanceof Element))return!1;if(he(o))return!0;let e=o.tagName?.toLowerCase();if(["select","textarea"].includes(e)||e==="input"&&!["button","submit","reset"].includes((o.type||"").toLowerCase()))return!1;let n=o.closest?.('button, a, [role="button"], [class*="leaderboard" i], [data-testid*="leaderboard" i], [class*="scoreboard" i], [class*="trophy" i]')||o,i=String(n.getAttribute?.("data-testid")||n.getAttribute?.("data-test-id")||n.getAttribute?.("id")||""),a=String(n.getAttribute?.("aria-label")||""),l=String(n.getAttribute?.("title")||""),s=typeof n.className=="string"?n.className:typeof n.className?.baseVal=="string"?n.className.baseVal:"",r=I(n.textContent,60);return!!(Ae.test(i)||Ae.test(a)||Ae.test(l)||Ae.test(s)||r.length>0&&r.length<=25&&Ae.test(r))}function oe(o){if(!o||typeof o.getAttribute!="function"||typeof Element<"u"&&!(o instanceof Element)||he(o)||B(o)||o.closest?.('.option-card, .choice-card, .quiz-option, [class*="option-card" i], [class*="choice-card" i], [class*="option-item" i], [class*="choice-item" i], [class*="answer-item" i], [data-testid*="option" i], [data-testid*="choice" i], [data-choice], [data-option], [data-answer], [role="radio"], [role="checkbox"], [role="option"]')||o.closest?.("header, nav, aside"))return!1;let e=typeof HTMLInputElement<"u"&&o instanceof HTMLInputElement||typeof HTMLButtonElement<"u"&&o instanceof HTMLButtonElement?o.value:"",t=I(o.getAttribute?.("aria-label")||o.textContent||o.getAttribute?.("value")||e),n=o.type,i=t.replace(/[\d\(\)\[\]→\>\•\-\/\\]+/g," ").trim(),a=String(o.getAttribute?.("data-testid")||o.getAttribute?.("data-test-id")||o.getAttribute?.("id")||o.getAttribute?.("href")||"").toLowerCase();return Pe.test(i)||Pe.test(t)||n==="submit"||a.includes("next")||a.includes("check")||a.includes("continue")||a.includes("proximo")||a.includes("forward")||!1}function ot(o){let e=o.closest("tr");if(e){let r=e.querySelector("th, td:first-child"),c=r&&r!==o.closest("td")?I(r.textContent,100):"",d=I(o.closest("label, td")?.textContent||"",50);if(c&&d)return`${c}: ${d}`}let t=o.closest('.dropdown-row, [class*="dropdown-row" i], [class*="select-row" i]');if(t){let r=t.querySelector('.dropdown-label, [class*="label" i]'),c=r&&r!==o?I(r.textContent,150):"";if(c)return c}let n=o.getAttribute("aria-label");if(n)return I(n);let i=o.getAttribute("aria-labelledby");if(i){let r=i.split(/\s+/).map(c=>document.getElementById(c)?.textContent).filter(Boolean).join(" ");if(r.trim())return I(r)}if("labels"in o&&o.labels){let r=Array.from(o.labels??[]).map(c=>c.textContent).join(" ");if(r.trim())return I(r)}let a=o.closest('.docssharedWizToggleLabeledContainer, [role="listitem"], .answer, label, .quiz-option, .form-check, .option-card');if(a&&a!==o){let r=I(a.textContent);if(r)return r}let l=o instanceof HTMLInputElement||o instanceof HTMLButtonElement?o.value:"",s=o.getAttribute("placeholder")||o.getAttribute("title")||o.textContent||l||"";return I(s)}function nt(o,e){let n=typeof HTMLSelectElement<"u"&&o instanceof HTMLSelectElement||o.tagName.toLowerCase()==="select"?o:null,i=o;o.dataset.easyquizRole=e;let a=o.tagName.toLowerCase(),l=["input","textarea","select","button"].includes(a)?a:"other",s=o.getAttribute("role")||"",r=(o.getAttribute("data-testid")||o.getAttribute("data-test-id")||"").toLowerCase(),c=(o.className&&typeof o.className=="string"?o.className:"").toLowerCase(),d=o.getAttribute("draggable")==="true"||o.classList.contains("perseus-drag-item")||o.classList.contains("sortable-item")||!!o.getAttribute("aria-grabbed")||/drag|card|option|item/i.test(r)||/drag|card-item|sortable/i.test(c),m=o.getAttribute("data-role")==="dropzone"||o.classList.contains("category-container")||o.hasAttribute("data-category")||!!o.getAttribute("aria-dropeffect")||/drop|category|bucket/i.test(r)||/dropzone|category-box|bucket|target-zone/i.test(c),h=I((d?"draggable":m?"dropzone":"")||i.type||s||l,40),f="";if(i.type==="checkbox"||i.type==="radio"||s==="radio"||s==="checkbox")f=i.checked||o.getAttribute("aria-checked")==="true"?"checked":"unchecked";else if(l==="button"||a==="a"||e==="navigation"||oe(o))f="";else{let q=typeof o.value=="string"||typeof o.value=="number"?o.value:"";f=I(q||o.getAttribute("data-category")||"",2e3)}let y=[];if(n&&n.options)for(let q of Array.from(n.options).slice(0,80))y.push({value:I(q.value),label:I(q.textContent)});else if(s==="combobox"||s==="listbox"||c.includes("select")||c.includes("dropdown")){let q=o.getAttribute("aria-controls")||o.getAttribute("aria-owns"),w=q?document.getElementById(q):o;if(w){let C=w.querySelectorAll('[role="option"], li, .dropdown-item, .option');for(let T of Array.from(C).slice(0,80)){let L=I(T.textContent);L&&y.push({value:T.getAttribute("data-value")||T.getAttribute("value")||L,label:L})}}}let v=!!(i.required||o.getAttribute("aria-required")==="true"),g=!!(i.disabled||o.getAttribute("aria-disabled")==="true"),p=co(o);return{id:o.id||p,tag:l,type:h,label:ot(o),name:I(i.name||o.getAttribute("name")||"",180),value:f,options:y,required:v,disabled:g,role:e}}var $t=['[data-test-id*="exercise" i]','[data-testid*="exercise" i]',".perseus-renderer",".framework-perseus",".Qr7Oae",".que",".question-holder",".quiz-question",".question_holder",".display_question",'[data-functional-selector*="question"]',".question-container","[data-question-id]",'[data-testid*="question" i]','[class*="question-container" i]','[class*="question" i]','[class*="pergunta" i]',"article","form","section","main"].join(",");function Pt(o){if(!M(o))return-1/0;let e=o.getBoundingClientRect(),t=Array.from(o.querySelectorAll(pe)).filter(M),n=I(o.innerText||o.textContent||"",4e3).length;if(n<10||!t.length&&n<60)return-1/0;let i=Math.max(1,window.innerWidth*window.innerHeight),a=Math.max(1,e.width*e.height),l=Math.min(1,a/i),s=e.top+e.height/2,r=Math.abs(s-window.innerHeight/2)/Math.max(1,window.innerHeight),c=n>40?35:0,d=e.top>=0&&e.bottom<=window.innerHeight?25:0;return t.length*15+Math.min(60,n/20)+c+d-l*20-r*10}function Re(o){let e=o;if(e.matches?.('article, [data-test-id*="exercise" i], [data-testid*="exercise" i], .perseus-renderer, .framework-perseus, [class*="question-container" i], .que')&&e.tagName.toLowerCase()!=="main"&&e.tagName.toLowerCase()!=="body")return e;for(;e.parentElement&&e.parentElement!==document.body&&e.parentElement!==document.documentElement;){let t=e.parentElement,n=t.tagName.toLowerCase();if(["header","footer","nav","aside"].includes(n))break;if(t.matches?.('article, [data-test-id*="exercise" i], [data-testid*="exercise" i], .perseus-renderer, .framework-perseus, [class*="question-container" i], .que')&&n!=="main"&&n!=="body"){e=t;break}let i=I(e.innerText||e.textContent||"",1e4),a=I(t.innerText||t.textContent||"",1e4),l=e.querySelectorAll(pe).length,s=t.querySelectorAll(pe).length;if(i.length<150&&a.length>i.length&&s<=l+4&&n!=="main"&&n!=="body"){e=t;continue}break}return e}function Rt(o){let e=o,t=e.closest('main, [role="main"], article, form, [data-test-id*="exercise" i], [data-testid*="exercise" i], .framework-perseus, section');if(t&&t!==document.body&&M(t))return t;let n=0;for(;e.parentElement&&e.parentElement!==document.body&&n<3;)e=e.parentElement,n++;return e||document.body}function O(){let o=document.activeElement;if(o&&o!==document.body){let a=o.closest($t);if(a&&Pt(a)>0)return Re(a)}let t=Array.from(document.querySelectorAll($t)).map(a=>({element:a,score:Pt(a)})).filter(a=>Number.isFinite(a.score)).sort((a,l)=>l.score-a.score),n=t.find(a=>{let l=a.element.tagName.toLowerCase();return l!=="main"&&l!=="body"&&a.score>0});if(n)return Re(n.element);if(t.length>0&&t[0].score>0)return Re(t[0].element);let i=document.querySelector('form, main, [role="main"]');return i&&M(i)?i:document.body}function Bt(o){let e=o.cloneNode(!0);e.querySelectorAll("script, style, iframe, object, embed, svg, canvas, noscript, audio, video").forEach(n=>n.remove());let t=["type","name","value","role","aria-label","aria-labelledby","aria-checked","aria-required","required","disabled","data-easyquiz-id","draggable","class","id","data-widget-type","data-role","data-category","data-testid"];return e.querySelectorAll("*").forEach(n=>{for(let i of Array.from(n.attributes))t.includes(i.name)||n.removeAttribute(i.name)}),e.outerHTML.replace(/\s+/g," ").slice(0,2e4)}function Be(o){let e=Array.from(o.querySelectorAll(pe)),t=new Set,n=[];for(let i of e){if(!M(i)||oe(i)||B(i))continue;let a=i.tagName.toLowerCase();["input","textarea","select"].includes(a)&&(t.add(i),n.push(i))}for(let i of e){if(!M(i)||oe(i)||B(i))continue;let a=i.tagName.toLowerCase();if(["input","textarea","select"].includes(a))continue;let l=i.querySelector("input, textarea, select");if(!(l&&t.has(l))){if(i.hasAttribute("for")){let s=i.getAttribute("for"),r=s?i.ownerDocument.getElementById(s):null;if(r&&t.has(r))continue}if(a==="a"){let s=i.getAttribute("role");if(!(s==="button"||s==="radio"||s==="checkbox"||s==="option"||i.closest('[class*="choice" i], [class*="option" i], [class*="answer" i], [data-testid*="option" i]')))continue}n.push(i)}}return n.slice(0,100).map(i=>nt(i,"answer"))}function at(o){let e=[o,o.parentElement,o.parentElement?.parentElement,document.body].filter(Boolean),t=new Set,n=[];for(let i of e)for(let a of Array.from(i.querySelectorAll(pe)))if(!(t.has(a)||!M(a)||!oe(a)||B(a))&&(t.add(a),n.push(nt(a,"navigation")),n.length>=10))return n;return n}function Le(o=!1){let e=O();e=Re(e),o&&(e=Rt(e));let t=Be(e),n=at(e);if(t.length===0){let s=Be(document.body);s.length>0&&(e=Rt(e),t=Be(e),t.length===0&&(t=s,e=document.querySelector('main, article, form, [role="main"]')||document.body))}n.length===0&&(n=at(document.body));let i=e.innerText&&e.innerText.trim().length>0?e.innerText:e.textContent||"",a=I(i,16e3),l=[...t,...n].slice(0,120);return!a||l.length===0&&a.length<30?I(document.body.innerText||document.body.textContent||"",16e3).length>=30?me():null:{sourceUrl:window.location.href.slice(0,2e3),pageTitle:document.title.slice(0,500)||"P\xE1gina de Quest\xE3o",questionText:a,htmlSnippet:Bt(e),controls:l,scope:e}}function me(){let o=document.body.innerText||document.body.textContent||document.documentElement.textContent||"",e=I(o,16e3),t=Be(document.body),n=at(document.body),i=[...t,...n].slice(0,120),a=document.querySelector('main, article, form, [role="main"], [data-test-id*="content" i], [class*="content" i]')||document.body;return{sourceUrl:window.location.href.slice(0,2e3),pageTitle:document.title.slice(0,500)||"P\xE1gina de Quest\xE3o",questionText:e,htmlSnippet:Bt(a).slice(0,15e3),controls:i,scope:a}}function Nt(o){let e=o.controls.map(t=>`${t.role}:${t.id}:${t.type}`).join("|");return[window.location.href,o.pageTitle,o.questionText.slice(0,400),e].join("::")}function $(o){return o?!!(o.closest('#easyquiz-shadow-root, .eq-sidebar, .eq-launcher, [data-easyquiz-ignore="true"], .btn-inject-eq, #btn-inject-script')||o.getAttribute?.("data-easyquiz-ignore")==="true"):!1}function x(o){return o==null?"":(typeof o=="string"?o:String(o)).replace(/^(\([0-9a-zA-Z]{1,2}\)|[0-9]{1,3}|[a-zA-Z])[\.\)\-\:]\s+/,"").replace(/[\.\u2026]{2,}/g," ").replace(/['"“”«»]/g,"").replace(/\s+/g," ").trim()}function K(o){if(!o||o instanceof HTMLInputElement||o instanceof HTMLSelectElement||o instanceof HTMLTextAreaElement||o.getAttribute("draggable")==="true"||o.classList.contains("dnd-card")||o.hasAttribute("data-category")||o.hasAttribute("data-dropzone"))return o;if(o.hasAttribute("for")){let n=o.getAttribute("for");if(n){let i=o.ownerDocument.getElementById(n);if(i)return i}}let e=o.closest('label, .option-card, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, tr, li, .dnd-card, [class*="option-card" i], [class*="choice-card" i], .dropdown-row, [class*="dropdown" i], [class*="select-row" i]');if(e&&!["article","section","main","form","body"].includes(e.tagName.toLowerCase())){let n=e.getAttribute("for"),a=(n?e.ownerDocument.getElementById(n):null)||e.querySelector('input:not([type="hidden"]), select, textarea');return a||e}let t=o.closest('button, a, [role="button"], [draggable="true"]');if(t)return t;if(["body","html","main","section","article","form"].includes(o.tagName.toLowerCase())){let n=o.querySelector('button, [role="button"], a, input:not([type="hidden"]), select, textarea, [role="radio"], [role="checkbox"], .option-card, label');if(n)return K(n)}return o}function Ot(o){let e=o;if(!e||!document.contains(e))try{e=O()}catch{}e=e||document.body;let t=Array.from(e.querySelectorAll("tr")).filter(a=>M(a)&&a.querySelector('input[type="radio"], input[type="checkbox"]'));if(t.length>1)return t;let n=Array.from(e.querySelectorAll('input[type="checkbox"], input[type="radio"], [role="checkbox"], [role="radio"]')).filter(a=>M(a)&&!$(a));return n.length>0?n:Array.from(e.querySelectorAll('.option-card, [role="option"], [class*="choice-card" i], [class*="option-card" i], li[class*="choice" i], li[class*="option" i]')).filter(a=>M(a)&&!$(a)).filter(a=>!a.parentElement?.closest('.option-card, [role="option"], [class*="choice-card" i], [class*="option-card" i]'))}function A(o,e,t=!1){if(o==null)return null;let i=(typeof o=="string"?o:String(o)).trim().replace(/^["'“”«»]+|["'“”«»]+$/g,"");if(!i)return null;let a=R(i),l=document.querySelector(`[data-easyquiz-id="${a}"]`);if(l&&!$(l)&&M(l))return K(l);try{let u=document.getElementById(i);if(u&&!$(u)&&M(u))return u.hasAttribute("data-category")||u.hasAttribute("data-dropzone")||u.classList.contains("dnd-zone")?u:K(u)}catch{}let s=i.match(/^(?:item|opção|opcao|afirmação|afirmacao|alternativa|linha|afirmativa|questão|questao|campo|blank|lacuna|input|resposta)?\s*#?_?([0-9]+)$/i);if(s){let u=parseInt(s[1],10);if(t){let f=document.body;try{f=O()||document.body}catch{}let y=Array.from(f.querySelectorAll('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="button"]), textarea, select, [contenteditable="true"]')).filter(v=>M(v)&&!$(v));if(u>=1&&u-1<y.length)return y[u-1];if(u===0&&y.length>0)return y[0]}let h=u-1;if(h>=0){let f=Ot();if(h<f.length){let g=f[h];if(g.tagName.toLowerCase()==="tr"){if(e){let b=g.querySelector(`input[value="${R(e)}" i], [data-value="${R(e)}" i]`);if(b)return b}let p=g.querySelector("input");if(p)return p}return K(g)}let y=document.body;try{y=O()||document.body}catch{}let v=Array.from(y.querySelectorAll('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="button"]), textarea, select, [contenteditable="true"]')).filter(g=>M(g)&&!$(g));if(h<v.length)return v[h]}}let r=i.match(/^(?:item|opção|opcao|afirmação|afirmacao|alternativa|linha|afirmativa|questão|questao)?\s*#?([a-eA-E])$/i);if(r){let u=r[1].toUpperCase().charCodeAt(0)-65;if(u>=0){let h=Ot();if(u<h.length){let f=h[u];if(f.tagName.toLowerCase()==="tr"){if(e){let v=f.querySelector(`input[value="${R(e)}" i], [data-value="${R(e)}" i]`);if(v)return v}let y=f.querySelector("input");if(y)return y}return K(f)}}}if(/^[a-zA-Z0-9_-]{1,10}$/.test(i)){let h=Array.from(document.querySelectorAll(`[data-category="${a}" i], [data-dropzone="${a}" i], [data-role="dropzone"][data-category="${a}" i]`)).find(g=>M(g)&&!$(g));if(h)return h;let y=Array.from(document.querySelectorAll(`input[value="${a}" i], [data-value="${a}" i], input[id="${a}" i], input[placeholder="${a}" i], textarea[placeholder="${a}" i], [title="${a}" i]`)).find(g=>M(g)&&!$(g));if(y)return K(y);let v=Array.from(document.querySelectorAll('.option-badge, [class*="badge" i], [class*="letter" i], .option-card span, label span')).find(g=>{if(!M(g)||$(g))return!1;let p=x(g.textContent).toLowerCase();return p===i.toLowerCase()||p===i.toLowerCase()+")"});if(v)return K(v)}try{let h=Array.from(document.querySelectorAll(`[name="${a}"], [value="${a}"], [placeholder="${a}" i], [title="${a}" i], [data-category="${a}" i], [data-dropzone="${a}" i], [data-testid="${a}" i], [data-test-id="${a}" i], [aria-label="${a}" i]`)).find(f=>M(f)&&!$(f));if(h)return h.hasAttribute("data-category")||h.hasAttribute("data-dropzone")||h.classList.contains("dnd-zone")?h:K(h)}catch{}if(/^[.#\[]|\s|[>+~:]/.test(i))try{let h=Array.from(document.querySelectorAll(i)).find(f=>M(f)&&!$(f));if(h)return K(h)}catch{}try{let u=i.replace(/"/g,""),h=`//button[normalize-space(.)="${u}"] | //a[normalize-space(.)="${u}"] | //*[not(*) and normalize-space(.)="${u}"] | //*[@aria-label="${u}"] | //*[@data-category="${u}"] | //*[@data-testid="${u}"]`,f=document.evaluate(h,document,null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,null);for(let y=0;y<f.snapshotLength;y++){let v=f.snapshotItem(y);if(v&&M(v)&&!$(v)){if(["body","html"].includes(v.tagName.toLowerCase())){let p=v.querySelector('button, [role="button"], a, input, [role="radio"], [role="checkbox"], label');if(p&&M(p))return K(p)}return v.closest('[data-role="dropzone"], [class*="category" i], [class*="bucket" i], [class*="column" i], [class*="drop" i]')||K(v)}}}catch{}let d=x(i).toLowerCase(),m=Array.from(document.querySelectorAll('button, a, div, span, li, p, label, input, textarea, select, [draggable="true"], [data-testid], [class*="option" i], [class*="card" i], [class*="item" i], [class*="choice" i], [class*="category" i], [class*="bucket" i]'));for(let u of m){if(!M(u)||$(u)||u.closest("header, nav, .stepper, .step-item, .progress-bar-container")||B(u)||!!(u.matches('article, section, form, main, [class*="container" i], [class*="grid" i], .dnd-pool, .dnd-zones')||u.querySelector('label, [role="radio"], [role="checkbox"], .dnd-card, [draggable="true"], .option-card, tr'))&&!u.matches(".dnd-zone, [data-category], [data-dropzone]"))continue;let f=x(u.textContent).toLowerCase(),y=x(u.getAttribute("aria-label")||"").toLowerCase(),v=x(u.getAttribute("placeholder")||"").toLowerCase(),g=x(u.getAttribute("title")||"").toLowerCase(),p=x(u.getAttribute("name")||"").toLowerCase(),b=x(u.getAttribute("data-category")||"").toLowerCase(),q=u instanceof HTMLInputElement||u instanceof HTMLButtonElement?u.value:"",w=x(q).toLowerCase(),C=f.startsWith(d+")")||f.startsWith(d+".")||f.startsWith(d+" -")||f.startsWith(d+":");if(f===d||y===d||v===d||g===d||p===d||b&&b===d||w&&w===d||C)return u.closest('[data-role="dropzone"], [class*="category" i], [class*="bucket" i], [class*="column" i], [class*="drop" i]')||K(u)}if(d.length>=3)for(let u of m){if(!M(u)||$(u)||u.closest("header, nav, .stepper, .step-item, .progress-bar-container")||B(u)||!!(u.matches('article, section, form, main, [class*="container" i], [class*="grid" i], .dnd-pool, .dnd-zones')||u.querySelector('label, [role="radio"], [role="checkbox"], .dnd-card, [draggable="true"], .option-card, tr'))&&!u.matches(".dnd-zone, [data-category], [data-dropzone]"))continue;let f=x(u.textContent).toLowerCase(),y=x(u.getAttribute("aria-label")||"").toLowerCase(),v=x(u.getAttribute("placeholder")||"").toLowerCase(),g=x(u.getAttribute("title")||"").toLowerCase(),p=x(u.getAttribute("name")||"").toLowerCase();if(f.includes(d)||y.includes(d)||v.includes(d)||g.includes(d)||p.includes(d)){if(Array.from(u.children).some(C=>{let T=x(C.textContent).toLowerCase();return T&&T.includes(d)}))continue;return u.closest('[data-role="dropzone"], [class*="category" i], [class*="bucket" i], [class*="column" i], [class*="drop" i]')||K(u)}let b=d.split(/\s+/).filter(Boolean);if(b.length>=3){let q=b.slice(0,Math.min(5,b.length)).join(" ");if(f.includes(q)||y.includes(q)||v.includes(q))return K(u)}}return null}function st(o,e){for(let t of e)o.dispatchEvent(new Event(t,{bubbles:!0,composed:!0}))}function Q(o,e){if(!o)return;let t=o instanceof HTMLInputElement&&["checkbox","radio"].includes(o.type)?o:o.querySelector('input[type="checkbox"], input[type="radio"]')||(o.hasAttribute("for")?o.ownerDocument.getElementById(o.getAttribute("for")):null);if(t&&o!==t){if(t.type==="checkbox"){te(t,!0);return}if(t.type==="radio"){te(t,!0);return}}try{o.scrollIntoView({block:"center",inline:"center",behavior:"instant"})}catch{}try{o.focus?.()}catch{}if(typeof HTMLButtonElement<"u"&&o instanceof HTMLButtonElement||typeof HTMLAnchorElement<"u"&&o instanceof HTMLAnchorElement||o.tagName?.toLowerCase()==="a"||o.tagName?.toLowerCase()==="button"||typeof HTMLInputElement<"u"&&o instanceof HTMLInputElement&&!["checkbox","radio"].includes(o.type)){try{o.click()}catch{}return}let i=o.getBoundingClientRect(),a=e?e[0]:Math.round(i.left+Math.max(1,i.width/2)),l=e?e[1]:Math.round(i.top+Math.max(1,i.height/2)),s={bubbles:!0,cancelable:!0,composed:!0,view:window,clientX:a,clientY:l};try{o.dispatchEvent(new PointerEvent("pointerdown",{...s,button:0,buttons:1}))}catch{}try{o.dispatchEvent(new MouseEvent("mousedown",{...s,button:0,buttons:1}))}catch{}try{o.dispatchEvent(new PointerEvent("pointerup",{...s,button:0,buttons:0}))}catch{}try{o.dispatchEvent(new MouseEvent("mouseup",{...s,button:0,buttons:0}))}catch{}try{o.dispatchEvent(new MouseEvent("click",{...s,button:0,buttons:0}))}catch{}try{o.click()}catch{}}function Ne(o,e){let t=o;if(t.hasAttribute("for")){let c=t.getAttribute("for"),d=t.ownerDocument.getElementById(c);d&&(t=d)}if(typeof HTMLSelectElement<"u"&&t instanceof HTMLSelectElement||t.tagName?.toLowerCase()==="select"||t.getAttribute("role")==="combobox"||t.getAttribute("role")==="listbox"||t.matches?.('[role="combobox"], [role="listbox"], [class*="select" i], [class*="dropdown" i]')){Oe(t,[e]);return}let i=t.querySelector('select, [role="combobox"], [role="listbox"]');if(i){Oe(i,[e]);return}if(!(t instanceof HTMLInputElement)&&!(t instanceof HTMLTextAreaElement)&&!(t instanceof HTMLSelectElement)&&!t.isContentEditable){let c=t.querySelector('input:not([type="hidden"]), textarea, select, [contenteditable="true"]');if(c)t=c;else{let m=t.closest('.form-group, .field, [class*="input" i], [class*="control" i], label, tr, td, li, p, div')?.querySelector('input:not([type="hidden"]):not([type="button"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea, select, [contenteditable="true"]');if(m)t=m;else{let u=t.nextElementSibling;for(;u;){if(u instanceof HTMLInputElement&&!["hidden","button","submit","checkbox","radio"].includes(u.type)||u instanceof HTMLTextAreaElement||u instanceof HTMLElement&&u.isContentEditable){t=u;break}let h=u.querySelector('input:not([type="hidden"]):not([type="button"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]');if(h){t=h;break}u=u.nextElementSibling}}}}if(t instanceof HTMLButtonElement||t.tagName.toLowerCase()==="a"||t.getAttribute("role")==="button"||t instanceof HTMLInputElement&&["button","submit","reset","image"].includes(t.type)){let c=t.parentElement?.querySelector('input:not([type="hidden"]):not([type="button"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]');if(c)t=c;else{let d=document.body;try{d=O()||document.body}catch{}let m=d.querySelector('input:not([type="hidden"]):not([type="button"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]');if(m)t=m;else{console.warn("[EasyQuiz] setNativeValue: Nenhum campo de texto encontrado para receber o valor.");return}}}if(!(t instanceof HTMLInputElement)&&!(t instanceof HTMLTextAreaElement)&&!(t instanceof HTMLSelectElement)&&!t.isContentEditable){let c=document.body;try{c=O()||document.body}catch{}let d=c.querySelector('input:not([type="hidden"]):not([type="button"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]');if(d)t=d;else{console.warn("[EasyQuiz] setNativeValue: Nenhum campo de texto encontrado para receber o valor.");return}}if(t instanceof HTMLInputElement&&["checkbox","radio"].includes(t.type)){let c=["true","1","checked","yes","sim"].includes(e.toLowerCase())||e===t.value;te(t,c);return}let l=String(e??""),s=l;if(t instanceof HTMLInputElement&&t.type==="number"){let c=l.replace(",",".").replace(/[^0-9.-]/g,"");c&&!isNaN(Number(c))&&(s=c)}try{t.scrollIntoView?.({block:"center",inline:"center",behavior:"instant"}),t.focus?.()}catch{}let r=!1;try{if(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement){if(t.type!=="number"){try{t.select?.()}catch{}r=document.execCommand?.("insertText",!1,s)||!1}}else if(t.isContentEditable){try{document.execCommand?.("selectAll",!1,void 0)}catch{}r=document.execCommand?.("insertText",!1,s)||!1}}catch{}if(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement){try{let m=t._valueTracker;m&&m.setValue(s===""?" ":"")}catch{}let c=t instanceof HTMLTextAreaElement?HTMLTextAreaElement.prototype:HTMLInputElement.prototype,d=Object.getOwnPropertyDescriptor(c,"value")?.set;d?d.call(t,s):t.value=s;try{t.dispatchEvent(new KeyboardEvent("keydown",{bubbles:!0,cancelable:!0,key:s.slice(-1)||"a"}))}catch{}try{t.dispatchEvent(new InputEvent("beforeinput",{bubbles:!0,cancelable:!0,composed:!0,data:s,inputType:"insertText"}))}catch{}try{t.dispatchEvent(new InputEvent("input",{bubbles:!0,cancelable:!0,composed:!0,data:s,inputType:"insertText"}))}catch{t.dispatchEvent(new Event("input",{bubbles:!0,cancelable:!0,composed:!0}))}try{t.dispatchEvent(new KeyboardEvent("keyup",{bubbles:!0,cancelable:!0,key:s.slice(-1)||"a"}))}catch{}try{t.dispatchEvent(new Event("change",{bubbles:!0,cancelable:!0,composed:!0}))}catch{}try{t.dispatchEvent(new FocusEvent("blur",{bubbles:!0,cancelable:!0,composed:!0}))}catch{}if(t.value!==s&&!(t instanceof HTMLInputElement&&t.type==="number"&&Number(t.value)===Number(s))){t.value=s;try{d?.call(t,s)}catch{}}return}if(t.isContentEditable){if(t.textContent?.trim()!==s.trim()){t.textContent=s;try{t.innerText=s}catch{}}try{t.dispatchEvent(new InputEvent("input",{bubbles:!0,cancelable:!0,composed:!0,data:s,inputType:"insertText"}))}catch{t.dispatchEvent(new Event("input",{bubbles:!0,cancelable:!0,composed:!0}))}try{t.dispatchEvent(new Event("change",{bubbles:!0,cancelable:!0,composed:!0}))}catch{}try{t.dispatchEvent(new FocusEvent("blur",{bubbles:!0,cancelable:!0,composed:!0}))}catch{}return}try{"value"in t&&(t.value=s),t.dispatchEvent(new Event("input",{bubbles:!0,cancelable:!0,composed:!0})),t.dispatchEvent(new Event("change",{bubbles:!0,cancelable:!0,composed:!0}))}catch{}}function Me(o,e=""){if(o==null)return e;let t=typeof o=="string"?o:String(o);if(!t)return e;let n=/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(t),i=x(t),a=A(t)||A(i);if(!a)return n?e:i||e;let l=a.closest('label, .option-card, [class*="choice" i], [class*="option" i], .quiz-option, tr, td, li');if(l){let m=x(l.textContent);if(m&&m.length>0&&m.length<150)return m}if(a.id){let m=document.querySelector(`label[for="${R(a.id)}"]`);if(m){let u=x(m.textContent);if(u&&u.length>0&&u.length<150)return u}}let s=a.getAttribute("aria-label");if(s)return x(s);let r=a.getAttribute("placeholder");if(r)return x(r);let c=x(a.textContent);if(c&&c.length>0&&c.length<120)return c;let d=a instanceof HTMLInputElement||a instanceof HTMLButtonElement?a.value:"";return d?x(d):n?e:i||e}function te(o,e){let t=o.closest('.option-card, label, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, [class*="option" i], [class*="choice" i], li')||o,n=o instanceof HTMLInputElement&&["checkbox","radio"].includes(o.type)?o:t.querySelector('input[type="checkbox"], input[type="radio"]');if(!n&&t.hasAttribute("for")&&(n=t.ownerDocument.getElementById(t.getAttribute("for"))),t){let i=e?"true":"false";t.setAttribute("aria-checked",i),t.setAttribute("aria-selected",i),t.classList.toggle("selected",e),t.classList.toggle("active",e),t.classList.toggle("checked",e)}if(n){let i=!!n._valueTracker,a=n.checked===e;if(a&&n.type==="radio"&&!i)return;if(a&&i){try{let l=n._valueTracker;l&&l.setValue(!e)}catch{}st(n,["click","input","change"]);return}try{n.focus?.(),n.click()}catch{}if(n.checked!==e){try{let l=n._valueTracker;l&&l.setValue(!e)}catch{}try{Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"checked")?.set?.call(n,e)}catch{}n.checked=e,st(n,["input","change"])}}else if((t.getAttribute("aria-checked")==="true"||t.classList.contains("selected")||t.classList.contains("active"))===e)Q(t);else{try{t.focus?.()}catch{}try{t.click()}catch{Q(t)}}}function Oe(o,e){let t=typeof HTMLSelectElement<"u"&&o instanceof HTMLSelectElement||o.tagName?.toLowerCase()==="select"?o:o.querySelector("select");if(t){let l=e.map(c=>x(c).toLowerCase()),s=!1,r=(c,d)=>{c.selected=!0,t.selectedIndex=d;try{t.value=c.value}catch{}try{Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,"value")?.set?.call(t,c.value)}catch{}try{let m=t._valueTracker;m&&m.setValue(c.value)}catch{}s=!0};for(let c=0;c<t.options.length;c++){let d=t.options[c],m=d.value.toLowerCase(),u=x(d.textContent).toLowerCase();if(l.some(f=>f===m||f===u)){if(r(d,c),!t.multiple)break}else t.multiple||(d.selected=!1)}if(!s)for(let c of l){let d=c.match(/^(?:item|opção|opcao|alternativa|linha|escolha|campo)?\s*#?_?([0-9]+)$/i);if(d){let m=parseInt(d[1],10),h=t.options[0]?.value===""||t.options[0]?.disabled?m:m>=1?m-1:0;if(h>=0&&h<t.options.length&&(r(t.options[h],h),!t.multiple))break}}if(!s){for(let c of l)if(/^[a-z]$/i.test(c)){let d=c.toUpperCase().charCodeAt(0)-65,u=t.options[0]?.value===""||t.options[0]?.disabled?d+1:d;if(u>=0&&u<t.options.length&&(r(t.options[u],u),!t.multiple))break}}if(!s){let c=d=>d.normalize("NFD").replace(/[\u0300-\u036f]/g,"");for(let d=0;d<t.options.length;d++){let m=t.options[d],u=c(m.value.toLowerCase()),h=c(x(m.textContent).toLowerCase());if(l.some(y=>{let v=c(y);return u.includes(v)||h.includes(v)||v.length>2&&(v.includes(u)||v.includes(h))})&&(r(m,d),!t.multiple))break}}if(s){st(t,["focus","input","change","blur"]);return}}let n=o.matches?.('[role="combobox"], [role="listbox"], [class*="select" i], [class*="dropdown" i]')?o:o.closest('[role="combobox"], [role="listbox"], [class*="select" i], [class*="dropdown" i]');n&&Q(n);let i=e.map(l=>x(l).toLowerCase()),a=Array.from(document.querySelectorAll('[role="listbox"] [role="option"], [role="menu"] [role="menuitem"], .select-dropdown li, .dropdown-menu .dropdown-item, .ant-select-item-option, .MuiMenuItem-root, [class*="option-item"], li[data-value]')).filter(l=>M(l)&&!$(l));for(let l of i){let s=a.find(c=>{let d=x(c.textContent).toLowerCase(),m=x(c.getAttribute("data-value")||c.getAttribute("value")||"").toLowerCase();return d===l||m===l||d.includes(l)||l.length>2&&l.includes(d)});if(s){Q(s);let c=s.querySelector('input[type="radio"], input[type="checkbox"]');c&&te(c,!0);return}let r=A(l);if(r){Q(r);return}}}function uo(o,e){try{let t=new DataTransfer;try{t.setData("text/plain",o)}catch{}try{t.setData("text/html",e)}catch{}return t}catch{return null}}function it(o){try{o.click()}catch{let e=o.ownerDocument.defaultView||window;o.dispatchEvent(new e.MouseEvent("click",{bubbles:!0,cancelable:!0,composed:!0,view:e}))}}function ae(o,e){let t=x(o).toLowerCase();if(!t)return null;let n=e==="source"?'.dnd-card, [draggable="true"]':'[data-dropzone], [data-category], [data-role="dropzone"]',i=Array.from(document.querySelectorAll(n)),a=e==="destination"?i.find(l=>[l.getAttribute("data-category"),l.getAttribute("data-dropzone")].some(s=>s?.trim().toLowerCase()===t)):null;return a&&M(a)&&!$(a)?a:i.find(l=>{if(!M(l)||$(l))return!1;let s=x(`${l.textContent||""} ${l.getAttribute("data-category")||""} ${l.getAttribute("data-dropzone")||""}`).toLowerCase();return s===t||s.includes(t)})||null}async function De(o,e,t=1){try{o.scrollIntoView({block:"center",inline:"center",behavior:"instant"})}catch{}let n=o.getBoundingClientRect(),i=e.getBoundingClientRect(),a=Math.round(n.left+Math.max(1,n.width/2)),l=Math.round(n.top+Math.max(1,n.height/2)),s=Math.round(i.left+Math.max(1,i.width/2)),r=Math.round(i.top+Math.max(1,i.height/2)),c=x(e.textContent).toLowerCase();if(c){let y=Array.from(o.querySelectorAll('button, [role="button"], input[type="radio"], input[type="checkbox"], option, .btn, [class*="tag" i]')).find(v=>{let g=x(v.textContent).toLowerCase(),p=v instanceof HTMLInputElement||v instanceof HTMLOptionElement?x(v.value).toLowerCase():"";return g&&(c.includes(g)||g.includes(c))||p&&(c.includes(p)||p.includes(c))});y&&(Q(y),await new Promise(v=>setTimeout(v,120)))}it(o),await new Promise(f=>setTimeout(f,140)),it(e);let d=e.querySelector('[data-role="dropzone"], [class*="bucket" i], [class*="slot" i], [class*="drop" i], [class*="target" i], [class*="items" i], ul, ol');if(d&&d!==e&&it(d),await new Promise(f=>setTimeout(f,100)),!e.contains(o)&&o.matches('.dnd-card, [draggable="true"]')&&e.matches('.dnd-zone, [data-dropzone], [data-role="dropzone"]')&&e.appendChild(o),e.contains(o)&&o.matches('.dnd-card, [draggable="true"]'))return;let m={bubbles:!0,cancelable:!0,composed:!0,view:window,clientX:a,clientY:l,screenX:a,screenY:l,button:0,buttons:1};try{o.dispatchEvent(new PointerEvent("pointerdown",{...m,isPrimary:!0,pointerId:1,pointerType:"mouse",pressure:.5}))}catch{}o.dispatchEvent(new MouseEvent("mousedown",m));let u=4;for(let f=1;f<=u;f++){let y=Math.round(a+(s-a)*(f/u)),v=Math.round(l+(r-l)*(f/u)),g={...m,clientX:y,clientY:v,screenX:y,screenY:v};try{o.dispatchEvent(new PointerEvent("pointermove",{...g,isPrimary:!0,pointerId:1,pointerType:"mouse",pressure:.5}))}catch{}document.dispatchEvent(new MouseEvent("mousemove",g))}let h={bubbles:!0,cancelable:!0,composed:!0,view:window,clientX:s,clientY:r,screenX:s,screenY:r,button:0,buttons:0};try{e.dispatchEvent(new PointerEvent("pointerup",{...h,isPrimary:!0,pointerId:1,pointerType:"mouse",pressure:0}))}catch{}e.dispatchEvent(new MouseEvent("mouseup",h)),e.dispatchEvent(new MouseEvent("click",h));try{let f=uo(I(o.textContent),o.outerHTML),y={...m},v={...h};f&&(y.dataTransfer=f,v.dataTransfer=f);let g=o.ownerDocument.defaultView?.DragEvent;if(!g)throw new Error("DragEvent n\xE3o dispon\xEDvel neste documento");o.dispatchEvent(new g("dragstart",y)),e.dispatchEvent(new g("dragenter",v)),e.dispatchEvent(new g("dragover",v)),e.dispatchEvent(new g("drop",v)),o.dispatchEvent(new g("dragend",y))}catch(f){console.warn("[EasyQuiz] DragEvent ignorado com seguran\xE7a:",f)}try{let f=new Touch({identifier:1,target:o,clientX:a,clientY:l}),y=new Touch({identifier:1,target:e,clientX:s,clientY:r});o.dispatchEvent(new TouchEvent("touchstart",{bubbles:!0,cancelable:!0,touches:[f]})),e.dispatchEvent(new TouchEvent("touchmove",{bubbles:!0,cancelable:!0,touches:[y]})),e.dispatchEvent(new TouchEvent("touchend",{bubbles:!0,cancelable:!0,touches:[]}))}catch{}if(t>=2&&!e.contains(o))try{o.focus?.(),o.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",bubbles:!0})),o.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",bubbles:!0})),await new Promise(f=>setTimeout(f,80)),e.focus?.(),e.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",code:"Enter",bubbles:!0})),e.dispatchEvent(new KeyboardEvent("keyup",{key:"Enter",code:"Enter",bubbles:!0}))}catch{}}var Vt={fill:(o,e)=>{let t=A(o);t?Ne(t,e):console.warn(`$eq.fill: Elemento '${o}' n\xE3o encontrado`)},click:o=>{let e=A(o);e?!!(e.closest('.option-card, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, [class*="option" i], [class*="choice" i]')||e.querySelector('input[type="radio"], input[type="checkbox"]')||e instanceof HTMLInputElement&&["checkbox","radio"].includes(e.type))?te(e,!0):Q(e):console.warn(`$eq.click: Elemento '${o}' n\xE3o encontrado`)},check:(o,e)=>{let t=A(o);t?te(t,e):console.warn(`$eq.check: Elemento '${o}' n\xE3o encontrado`)},find:(o,e)=>A(o,e),drag:(o,e)=>{let t=ae(o,"source")||A(o),n=ae(e,"destination")||A(e);t&&n?De(t,n):console.warn(`$eq.drag: Origem ou destino n\xE3o encontrado ('${o}' -> '${e}')`)},categorize:async(o,e)=>{let t=ae(o,"source")||A(o),n=ae(e,"destination")||A(e);if(!t||!n){console.warn(`$eq.categorize: Item ou categoria n\xE3o encontrados ('${o}' -> '${e}')`);return}await De(t,n)},execute:(o,e=!1,t=1)=>ct(o,e,t)};typeof window<"u"&&(window.$eq=Vt);async function po(o,e=1,t=Te()){if(et(o,t),o.t==="js"){let s=String(o.v||"");zt(s);try{new Function("$eq","document","window",s)(Vt,document,window)}catch(r){throw console.warn("[EasyQuiz JS Execution]",r),r}return}if(o.t==="drag"){let s=ae(o.from,"source")||A(o.from),r=ae(o.to,"destination")||A(o.to);!s&&o.from&&(s=A(x(o.from))),!r&&o.to&&(r=A(x(o.to))),s&&r?await De(s,r,e):console.warn(`[EasyQuiz] Drag: alvo n\xE3o encontrado ('${o.from}' -> '${o.to}')`);return}let n=o.id!==void 0&&o.id!==null?String(o.id):"";!n&&o.t==="val"&&(n=o.target??o.name??o.selector??"1");let i=o.v!==void 0?o.v:o.value!==void 0?o.value:o.val!==void 0?o.val:o.text,a=i!=null?String(i).trim():"",l=A(n,a,o.t==="val"||o.t==="sel");if(!l&&n&&(l=A(x(n),a,o.t==="val"||o.t==="sel")),l&&a){if(l instanceof HTMLInputElement&&l.type==="radio"&&l.name){if(x(l.value).toLowerCase()!==x(a).toLowerCase()){let s=document.querySelector(`input[type="radio"][name="${R(l.name)}"][value="${R(a)}" i]`);if(s)l=s;else{let c=Array.from(document.querySelectorAll(`input[type="radio"][name="${R(l.name)}"]`)).find(d=>{let m=d.closest("label, .vf-label, .option-card, tr, td, div");return m&&x(m.textContent).toLowerCase().includes(x(a).toLowerCase())});c&&(l=c)}}}else if(!(l instanceof HTMLInputElement)&&!(l instanceof HTMLSelectElement)&&!(l instanceof HTMLTextAreaElement)){let s=l.querySelector(`input[value="${R(a)}" i], [data-value="${R(a)}" i]`);if(s)l=s;else{let c=Array.from(l.querySelectorAll('input[type="radio"], input[type="checkbox"]')).find(d=>{let m=d.closest("label, .vf-label, .option-card, td, div");return m&&x(m.textContent).toLowerCase().includes(x(a).toLowerCase())});c&&(l=c)}}}if(!l&&(o.t==="val"||o.t==="sel")){let s=document.body;try{s=O()||document.body}catch{}let r=Array.from(s.querySelectorAll(o.t==="sel"?'select, [role="combobox"], [role="listbox"]':'input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="button"]), textarea, select, [contenteditable="true"]')).filter(c=>M(c)&&!$(c));if(r.length===1)l=r[0];else if(r.length>1){let c=x(n).toLowerCase(),d=c.match(/^#?_?([0-9]+)$/);if(d){let m=parseInt(d[1],10);m>=1&&m<=r.length?l=r[m-1]:m>=0&&m<r.length&&(l=r[m])}l||(l=r.find(u=>{let h=(u.getAttribute("placeholder")||"").toLowerCase(),f=(u.name||"").toLowerCase(),y=(u.getAttribute("aria-label")||"").toLowerCase(),v=(u.id||"").toLowerCase(),g=x(ot(u)).toLowerCase(),p=x(u.closest('label, tr, td, .form-group, .field, [class*="row" i], div')?.textContent||"").toLowerCase();return h.includes(c)||f.includes(c)||y.includes(c)||v.includes(c)||g&&g.includes(c)||c.length>=2&&p.includes(c)})||(r.length===1?r[0]:null))}}if(!l&&o.t!=="adv"){console.warn(`[EasyQuiz] Alvo '${n}' n\xE3o encontrado para a\xE7\xE3o '${o.t}'. Prosseguindo...`);return}switch(o.t){case"val":if(l){let r=l instanceof HTMLInputElement||l instanceof HTMLTextAreaElement||l instanceof HTMLSelectElement||l.isContentEditable?l:l.querySelector('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="button"]), textarea, [contenteditable="true"]');if(!r){let u=l.closest('.form-group, .field, [class*="input" i], [class*="control" i], label, tr, td, li, p, div')?.querySelector('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="button"]), textarea, [contenteditable="true"]');u&&(r=u)}if(!r){let m=l.nextElementSibling;for(;m;){if(m instanceof HTMLInputElement&&!["hidden","button","submit","checkbox","radio"].includes(m.type)||m instanceof HTMLTextAreaElement||m instanceof HTMLElement&&m.isContentEditable){r=m;break}let u=m.querySelector('input:not([type="hidden"]):not([type="button"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]');if(u){r=u;break}m=m.nextElementSibling}}if(!r){let m=document.body;try{m=O()||document.body}catch{}let u=Array.from(m.querySelectorAll('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="button"]), textarea, [contenteditable="true"]')).filter(h=>M(h)&&!$(h));u.length>0&&(r=u[0])}let c=o.v!==void 0?o.v:o.value!==void 0?o.value:o.val!==void 0?o.val:o.text,d=c!=null?String(c):"";Ne(r||l,d)}break;case"chk":l&&te(l,!!o.c);break;case"sel":if(l){let r=Array.isArray(o.v)?o.v:[String(o.v)];Oe(l,r)}break;case"clk":if(l)if(!!(l.closest('.option-card, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice')||l.querySelector('input[type="radio"], input[type="checkbox"]')||l instanceof HTMLInputElement&&["checkbox","radio"].includes(l.type))){let c=o.c!==void 0?!!o.c:!0;te(l,c)}else Q(l,o.co);break;case"adv":let s=rt(o.id);if(s){await lt(s,1200);let r=o.id||s.textContent?.trim()||"";r&&Ue(window.location.hostname,{advanceSelector:r}),Q(s)}else console.warn("[EasyQuiz] Bot\xE3o de avan\xE7o n\xE3o localizado.");break}}function ho(){let o=["button","a",'[role="button"]','input[type="submit"]','input[type="button"]','[data-testid*="check" i]','[data-test-id*="check" i]'].join(",");return Array.from(document.querySelectorAll(o)).find(t=>{if(!M(t)||$(t)||t.closest("header, nav, aside"))return!1;let n=t instanceof HTMLInputElement||t instanceof HTMLButtonElement?t.value:"",i=(t.textContent||n||t.getAttribute("aria-label")||"").trim();return/(verificar|checar|check|conferir|validar|enviar|responder)/i.test(i)})||null}function rt(o){if(o){let a=A(o);if(a&&M(a)&&!$(a)&&!B(a))return a}try{let a=xe(window.location.hostname);if(a.advanceSelector){let l=A(a.advanceSelector);if(l&&M(l)&&!$(l)&&!B(l))return l}}catch{}let e=["button","a",'[role="button"]','[role="link"]','input[type="button"]','input[type="submit"]','[data-testid*="next" i]','[data-testid*="continue" i]','[data-testid*="check" i]','[data-test-id*="next" i]','[data-test-id*="continue" i]','[data-test-id*="check" i]','[class*="next" i]','[class*="continue" i]','[class*="proximo" i]','[class*="avancar" i]'].join(","),n=Array.from(document.querySelectorAll(e)).filter(a=>M(a)&&!$(a)&&!a.closest("header, nav, aside")&&!B(a));for(let a of n)if(oe(a)&&!B(a))return a;for(let a of n){let l=a instanceof HTMLInputElement||a instanceof HTMLButtonElement?a.value:"",s=(a.textContent||l||a.getAttribute("aria-label")||"").trim();if(Pe.test(s)&&!B(a))return a}let i=document.querySelector('[data-test-id*="next" i], [data-testid*="next" i], [aria-label*="next" i], [aria-label*="pr\xF3xim" i], [aria-label*="avan\xE7ar" i], [aria-label*="continuar" i]');return i&&M(i)&&!$(i)&&!B(i)?i:null}async function lt(o,e=2500){let t=Date.now();for(;Date.now()-t<e;){if(!(o.disabled===!0||o.getAttribute("aria-disabled")==="true"||o.classList.contains("disabled")||o.getAttribute("disabled")!==null))return;await new Promise(i=>setTimeout(i,80))}}function mo(){let o=window.location.href,e=document.title,t=document.querySelectorAll('input, textarea, select, button, [role="button"], [role="option"], [role="radio"], [role="checkbox"]').length,n=(document.body?.innerText||document.body?.textContent||"").length;return`${o}|${e}|${t}|${n}`}async function go(o,e=3500){let[t,n,i,a]=o.split("|"),l=parseInt(a||"0",10),s=Date.now();for(;Date.now()-s<e;){let r=window.location.href,c=document.title,d=String(document.querySelectorAll('input, textarea, select, button, [role="button"], [role="option"], [role="radio"], [role="checkbox"]').length),m=(document.body?.innerText||document.body?.textContent||"").length;if(r!==t)return{changed:!0,evidence:`URL mudou: ${t} \u2192 ${r}`};if(c!==n)return{changed:!0,evidence:`T\xEDtulo da p\xE1gina mudou: "${n}" \u2192 "${c}"`};if(Math.abs(parseInt(d)-parseInt(i||"0"))>=2)return{changed:!0,evidence:`Controles interativos: ${i} \u2192 ${d}`};if(Math.abs(m-l)>50)return{changed:!0,evidence:`Conte\xFAdo da p\xE1gina mudou substancialmente (${Math.abs(m-l)} chars)`};await new Promise(u=>setTimeout(u,100))}return{changed:!1,evidence:"Nenhuma mudan\xE7a estrutural detectada dentro do tempo limite."}}async function Dt(o){if(o.t==="js"||o.t==="adv")return;if(o.t==="drag"){let i=A(o.from)||A(x(o.from)),a=A(o.to)||A(x(o.to));i&&a&&await De(i,a,2);return}let e=o.id||"",t=o.v!==void 0?String(o.v).trim():"",n=A(e,t)||A(x(e),t);if(o.t==="clk"||o.t==="chk"){if(!n&&e){let a=Array.from(document.querySelectorAll('input, label, button, [role="radio"], [role="checkbox"], .option-card, [class*="option" i], [class*="choice" i]')),l=x(e).toLowerCase();n=a.find(s=>{let r=x(s.textContent).toLowerCase(),c=x(s.value||"").toLowerCase();return r.includes(l)||c===l||r.startsWith(l+")")||r.startsWith("("+l+")")})||null}let i=o.v!==void 0?String(o.v).trim():"";if(n&&i){if(n instanceof HTMLInputElement&&n.type==="radio"&&n.name){if(x(n.value).toLowerCase()!==x(i).toLowerCase()){let a=document.querySelector(`input[type="radio"][name="${R(n.name)}"][value="${R(i)}" i]`);if(a)n=a;else{let s=Array.from(document.querySelectorAll(`input[type="radio"][name="${R(n.name)}"]`)).find(r=>{let c=r.closest("label, .vf-label, .option-card, tr, td, div");return c&&x(c.textContent).toLowerCase().includes(x(i).toLowerCase())});s&&(n=s)}}}else if(!(n instanceof HTMLInputElement)&&!(n instanceof HTMLSelectElement)&&!(n instanceof HTMLTextAreaElement)){let a=n.querySelector(`input[value="${R(i)}" i], [data-value="${R(i)}" i]`);if(a)n=a;else{let s=Array.from(n.querySelectorAll('input[type="radio"], input[type="checkbox"]')).find(r=>{let c=r.closest("label, .vf-label, .option-card, td, div");return c&&x(c.textContent).toLowerCase().includes(x(i).toLowerCase())});s&&(n=s)}}}if(n){let a=n.closest('.option-card, label, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, li')||n,l=n instanceof HTMLInputElement&&["radio","checkbox"].includes(n.type)?n:a.querySelector('input[type="radio"], input[type="checkbox"]')||(a.getAttribute("for")?a.ownerDocument.getElementById(a.getAttribute("for")):null),s=o.t==="chk"||o.c!==void 0?!!o.c:!0;if(te(l||a,s),l&&l.checked!==s){try{let r=l._valueTracker;r&&r.setValue(!s)}catch{}try{Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"checked")?.set?.call(l,s)}catch{}l.checked=s,l.dispatchEvent(new Event("input",{bubbles:!0,composed:!0})),l.dispatchEvent(new Event("change",{bubbles:!0,composed:!0}))}}return}if(o.t==="val"){let i=null;if(n&&(i=n instanceof HTMLInputElement||n instanceof HTMLTextAreaElement||n.isContentEditable?n:n.querySelector('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]')),!i){let a=document.body;try{a=O()||document.body}catch{}let l=Array.from(a.querySelectorAll('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]')),s=x(e).toLowerCase();i=l.find(r=>{let c=(r.getAttribute("placeholder")||"").toLowerCase(),d=(r.name||"").toLowerCase(),m=(r.id||"").toLowerCase(),u=(r.getAttribute("aria-label")||"").toLowerCase();return c.includes(s)||d.includes(s)||m.includes(s)||u.includes(s)})||(l.length>0?l[0]:null)}if(i){let a=String(o.v??"");try{if(i.focus?.(),i.type!=="number"){try{i.select?.()}catch{}document.execCommand?.("insertText",!1,a)}}catch{}Ne(i,a)}return}if(o.t==="sel"){if(!n&&e){let i=Array.from(document.querySelectorAll("select")),a=x(e).toLowerCase();n=i.find(l=>{let s=(l.name||"").toLowerCase(),r=(l.id||"").toLowerCase(),c=(l.getAttribute("aria-label")||"").toLowerCase();return s.includes(a)||r.includes(a)||c.includes(a)})||null}if(n){let i=Array.isArray(o.v)?o.v:[String(o.v)];Oe(n,i)}return}}function ge(o){try{if(o.t==="val"){let e=o.v!==void 0?o.v:o.value!==void 0?o.value:o.val!==void 0?o.val:o.text,t=String(e??"").trim(),n=t,i=o.id!==void 0&&o.id!==null?String(o.id):"";i||(i=o.target??o.name??o.selector??"1");let a=A(i,n,!0)||A(x(i),n,!0);if(!a){let m=document.body;try{m=O()||document.body}catch{}let u=Array.from(m.querySelectorAll('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="button"]), textarea, [contenteditable="true"]')).filter(h=>M(h)&&!$(h));u.length>0&&(a=u[0])}if(!a)return!1;let l=a instanceof HTMLInputElement&&a.type==="radio"?a:a.querySelector('input[type="radio"]');if(l&&l.name){let m=document.querySelector(`input[type="radio"][name="${R(l.name)}"]:checked`);if(!m)return!1;let u=x(m.value).toLowerCase(),h=x(t).toLowerCase(),f=x(m.closest("label, .vf-label, .option-card, tr, td, div")?.textContent||"").toLowerCase();return u===h||f===h||f.includes(h)}let s=a instanceof HTMLInputElement||a instanceof HTMLTextAreaElement||a.isContentEditable?a:a.querySelector('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]');if(!s){let u=a.closest('.form-group, .field, [class*="input" i], [class*="control" i], label, tr, td, li, p, div')?.querySelector('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]):not([type="submit"]):not([type="button"]), textarea, [contenteditable="true"]');u&&(s=u)}if(!s){let m=a.nextElementSibling;for(;m;){if(m instanceof HTMLInputElement&&!["hidden","button","submit","checkbox","radio"].includes(m.type)||m instanceof HTMLTextAreaElement||m instanceof HTMLElement&&m.isContentEditable){s=m;break}let u=m.querySelector('input:not([type="hidden"]):not([type="button"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]');if(u){s=u;break}m=m.nextElementSibling}}if(s instanceof HTMLSelectElement){let m=x(t).toLowerCase();return Array.from(s.options).some(u=>{if(!u.selected)return!1;let h=u.value.toLowerCase(),f=x(u.textContent).toLowerCase();return m===h||m===f||h.includes(m)||f.includes(m)})}let r=(s instanceof HTMLInputElement||s instanceof HTMLTextAreaElement?s.value:s?.textContent??a.textContent??"").trim();if(!r&&!t)return!0;if(!r&&t)return!1;let c=r.replace(",",".").replace(/\s+/g,"").toLowerCase(),d=t.replace(",",".").replace(/\s+/g,"").toLowerCase();return c===d||c.includes(d)||d.includes(c)||r.toLowerCase()===t.toLowerCase()}if(o.t==="sel"){let e=A(o.id,void 0,!0)||A(x(o.id),void 0,!0);if(!e){let a=document.body;try{a=O()||document.body}catch{}let l=Array.from(a.querySelectorAll('select, [role="combobox"], [role="listbox"]')).filter(c=>M(c)&&!$(c)),s=x(o.id).toLowerCase();e=l.find(c=>{let d=(c.id||"").toLowerCase(),m=(c.getAttribute("name")||"").toLowerCase(),u=(c.getAttribute("aria-label")||"").toLowerCase(),h=x(c.closest('.dropdown-row, [class*="dropdown" i], [class*="select" i], tr, label, div')?.textContent||"").toLowerCase();return d.includes(s)||m.includes(s)||u.includes(s)||s.length>=2&&h.includes(s)})||(l.length===1?l[0]:null)}if(!e)return!1;let t=e instanceof HTMLSelectElement?e:e.querySelector("select");if(!t){let a=e.matches?.('[role="combobox"], [role="listbox"], [class*="select" i], [class*="dropdown" i]')?e:e.closest('[role="combobox"], [role="listbox"], [class*="select" i], [class*="dropdown" i]');if(a){let s=(Array.isArray(o.v)?o.v:[String(o.v)]).map(c=>x(c).toLowerCase()),r=x(a.textContent).toLowerCase();return s.some(c=>r.includes(c)||c.includes(r))}return!1}let i=(Array.isArray(o.v)?o.v:[String(o.v)]).map(a=>x(a).toLowerCase());return Array.from(t.options).some(a=>{if(!a.selected)return!1;let l=a.value.toLowerCase(),s=x(a.textContent).toLowerCase();return i.some(r=>r===l||r===s||l.includes(r)||s.includes(r))})}if(o.t==="chk"||o.t==="clk"){let e=o.v!==void 0?String(o.v).trim():"",t=A(o.id,e)||A(x(o.id),e);if(!t)return!1;let n=t.closest('.option-card, label, [role="radio"], [role="checkbox"], [role="option"], .quiz-option, .answer, .choice, li')||t,i=t instanceof HTMLInputElement&&["checkbox","radio"].includes(t.type)?t:n.querySelector('input[type="checkbox"], input[type="radio"]')||(n.getAttribute("for")?n.ownerDocument.getElementById(n.getAttribute("for")):null),a=o.t==="chk"||o.c!==void 0?!!o.c:!0;if(i&&i.type==="radio"){if(i.checked===a)return!0;if(o.v&&i.name){let d=x(String(o.v)).toLowerCase(),m=document.querySelector(`input[type="radio"][name="${R(i.name)}"]:checked`);if(!m)return!1;if(m===i)return!0;let u=x(m.value).toLowerCase(),h=x(m.closest("label, .vf-label, .option-card, tr, td, div")?.textContent||"").toLowerCase();return u===d||h.includes(d)||d.includes(u)}}if(i&&["checkbox","radio"].includes(i.type))return i.checked===a;let l=n.getAttribute("aria-checked")===String(a)||n.getAttribute("aria-selected")===String(a)||n.getAttribute("aria-pressed")===String(a),s=a?n.getAttribute("data-selected")==="true"||n.getAttribute("data-checked")==="true"||n.getAttribute("data-active")==="true"||n.getAttribute("data-state")==="checked"||n.getAttribute("data-state")==="on":n.getAttribute("data-selected")==="false"||n.getAttribute("data-checked")==="false"||n.getAttribute("data-state")==="unchecked",r=a?/active|selected|checked|picked|correct|is-selected|choice-selected|selected-option|is-checked|chosen|current|highlight|ring|border-primary/i.test(n.className||""):!/active|selected|checked|picked|correct|is-selected|choice-selected|selected-option|is-checked|chosen|current|highlight|ring|border-primary/i.test(n.className||"");return!!(l||s||r||(n instanceof HTMLButtonElement||n.getAttribute("role")==="button")&&o.t==="clk"||o.t==="clk"&&!i)}if(o.t==="drag"){let e=A(o.from)||A(x(o.from)),t=A(o.to)||A(x(o.to));return!e||!t?!1:t.contains(e)?!0:/placed|dropped|assigned|matched|done|selected/i.test(e.className||"")||e.getAttribute("data-placed")==="true"}}catch{}return!1}async function ct(o,e,t=1,n=Te({engine:"smart",autoAdvance:e})){let i=o.actions.filter(p=>p.t!=="adv"),a=o.actions.filter(p=>p.t==="adv"),l=0,s=[],r=new Map,c=o.pageType==="question",d=i.filter(p=>p.t==="chk"||p.t==="clk"&&p.c!==void 0);for(let p of i){try{await po(p,t,n),l++}catch(b){r.set(p,b instanceof Error?b.message:String(b)),console.warn("[EasyQuiz] A\xE7\xE3o declarativa prim\xE1ria falhou com seguran\xE7a:",p,b)}await new Promise(b=>setTimeout(b,p.t==="drag"?300:100))}if(c&&d.length>0){let p=document.body;try{p=O()||document.body}catch{}let b=Array.from(p.querySelectorAll('input[type="checkbox"], [role="checkbox"]')).filter(q=>M(q)&&!$(q));if(b.length>1){let q=new Set;for(let w of d){let C=w.t==="chk"?!!w.c:!!(w.c??!0),T="id"in w&&typeof w.id=="string"?w.id:"";if(C&&T){let L=A(T,w.v);if(L){let P=L instanceof HTMLInputElement&&L.type==="checkbox"?L:L.querySelector('input[type="checkbox"]');q.add(P||L)}}}if(q.size>0)for(let w of b)q.has(w)||(w instanceof HTMLInputElement&&w.checked||w.getAttribute("aria-checked")==="true"||w.closest(".option-card, label")?.classList.contains("selected"))&&te(w,!1)}}await new Promise(p=>setTimeout(p,i.length>0?450:80));let m=0;for(let p of i){if(ge(p)){m++;continue}console.warn(`[EasyQuiz Auto-Cura] A\xE7\xE3o '${p.t}' no alvo '${p.id||p.from||""}' n\xE3o verificada no DOM. Disparando Passagem 2 de conting\xEAncia...`);try{et(p,n),await Dt(p)}catch(b){r.set(p,b instanceof Error?b.message:String(b)),console.warn("[EasyQuiz Auto-Cura] Rota alternativa falhou:",b)}await new Promise(b=>setTimeout(b,250)),ge(p)&&(console.log("[EasyQuiz Auto-Cura] \u2713 A\xE7\xE3o recuperada com sucesso pela rota de conting\xEAncia!"),m++)}if(m<i.length&&i.length>0){console.warn(`[EasyQuiz Auto-Cura] ${i.length-m} de ${i.length} a\xE7\xE3o(\xF5es) ainda n\xE3o verificadas. Disparando Passagem 3 final...`),await new Promise(p=>setTimeout(p,300));for(let p of i)if(!ge(p))try{await Dt(p)}catch(b){r.set(p,b instanceof Error?b.message:String(b))}await new Promise(p=>setTimeout(p,300)),m=0;for(let p of i)ge(p)&&m++}for(let p of i)ge(p)||s.push(p.t==="drag"?`${p.from} -> ${p.to}`:"id"in p?p.id:p.t);c&&i.length===0&&s.push("nenhuma a\xE7\xE3o de resposta prescrita");let u=i.map((p,b)=>{let q=p.t==="drag"?`${p.from} -> ${p.to}`:p.t==="js"?"$eq":p.id||p.t,w=p.t==="js"?!0:p.t==="drag"?!!(ae(p.from,"source")&&ae(p.to,"destination")):!!(A(p.id||"")||A(x(p.id||""))),C=ge(p);return{index:b,action:p,target:q,located:w,applied:!r.has(p),verified:C,strategy:p.t==="drag"?"drag-adaptive":p.t==="js"?"javascript":"declarative-dom",evidence:C?"estado do controle confirmado no DOM":"nenhuma evid\xEAncia suficiente ap\xF3s as tentativas",...r.has(p)?{error:r.get(p)}:{}}}),h=c?i.length>0&&s.length===0&&(m===i.length||l===i.length&&m>0):!0,f=!1,y=!1,v="Nenhuma a\xE7\xE3o de navega\xE7\xE3o solicitada.",g=l>0&&l>=i.length/2;if(e&&(h||!c||g)){await new Promise(C=>setTimeout(C,i.length>0?600:200));let p=!1;if(o.pageType!=="info"){let C=ho();C&&M(C)&&(await lt(C,1200),Q(C),p=!0,await new Promise(T=>setTimeout(T,800)))}let b=mo(),q=a.length>0?a[0].id:void 0,w=rt(q);if(!w&&p&&(await new Promise(C=>setTimeout(C,600)),w=rt(q)),w){await lt(w,1500);let C=q||w.textContent?.trim()||"";C&&Ue(window.location.hostname,{advanceSelector:C}),Q(w);let T=await go(b,2500);y=T.changed,v=T.evidence,f=T.changed||p,!T.changed&&!p&&console.warn("[EasyQuiz] O bot\xE3o de avan\xE7o foi acionado, mas a navega\xE7\xE3o ainda n\xE3o concluiu.")}else p?(f=!0,y=!0,v="Resposta confirmada via bot\xE3o de verifica\xE7\xE3o/envio."):console.warn("[EasyQuiz] Nenhum bot\xE3o de avan\xE7o encontrado na p\xE1gina.")}return{applied:l,verified:m,success:h,advanced:f,failed:s,reports:u,navigationVerified:y,navigationEvidence:v}}var fe=null,re=[],dt=[],ut=[],be=null,fo=`
...
`;function _t(){try{if(typeof document>"u"||!document.head)return;if(!document.getElementById("eq-image-pulse-style")){let o=document.createElement("style");o.id="eq-image-pulse-style",o.textContent=fo,document.head.appendChild(o)}}catch{}}function le(){fe&&(fe.style.removeProperty("outline"),fe.style.removeProperty("outline-offset"),fe.style.removeProperty("position"),fe=null);for(let o of re)o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("background-color"),o.style.removeProperty("box-shadow"),o.removeAttribute("data-easyquiz-highlight");re=[];for(let o of dt)o.style.removeProperty("animation"),o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("box-shadow"),o.removeAttribute("data-easyquiz-image-highlight");dt=[];for(let o of ut)try{o.remove()}catch{}if(ut=[],be){try{be.remove()}catch{}be=null}}function pt(o){_t();for(let e of o){if(!e||!(e instanceof(typeof HTMLElement<"u"?HTMLElement:e.constructor)))continue;let t=e;t.style.outline="3px solid #ffd600",t.style.outlineOffset="3px",t.style.animation="eq-image-pulse-yellow-white 1.2s ease-in-out infinite",t.setAttribute("data-easyquiz-image-highlight","true"),dt.push(t);try{let n=t.parentElement;if(n&&!n.querySelector("[data-easyquiz-capture-badge]")){window.getComputedStyle(n).position==="static"&&(n.style.position="relative");let a=document.createElement("div");a.setAttribute("data-easyquiz-capture-badge","true"),a.textContent="\u{1F4F7} Capturado pela IA",a.style.cssText=`
        ...
        `,n.appendChild(a),ut.push(a)}}catch{}}}function ht(o){le(),_t(),fe=o,o.style.outline="2px solid #00e5ff",o.style.outlineOffset="4px";try{window.getComputedStyle(o).position==="static"&&(o.style.position="relative");let t=document.createElement("div");t.style.cssText=`
    ...
    `,this.shadow.appendChild(this.element),this.element.querySelector("#eq-fah-pill").addEventListener("click",()=>this.restore()),this.element.querySelector("#eq-fah-min-btn").addEventListener("click",()=>this.minimize()),this.element.querySelector("#eq-fah-close-btn").addEventListener("click",()=>this.hide());let i=this.element.querySelector("#eq-fah-copy-md-btn");i.addEventListener("click",()=>this.copyMarkdownToClipboard(i));let a=this.element.querySelector("#eq-fah-copy-all-btn");a.addEventListener("click",()=>this.copyMarkdownToClipboard(a));let l=this.element.querySelector("#eq-fah-header");this.setupDraggable(l)}setupDraggable(e){let t=n=>{if(n.target.closest(".eq-fah-btn"))return;n.preventDefault(),this.isDragging=!0,this.dragStartX=n.clientX,this.dragStartY=n.clientY;let i=this.element.getBoundingClientRect();this.initialLeft=i.left,this.initialTop=i.top;let a=s=>{if(!this.isDragging||!this.element)return;let r=s.clientX-this.dragStartX,c=s.clientY-this.dragStartY,d=Math.max(10,window.innerWidth-this.element.offsetWidth-10),m=Math.max(10,window.innerHeight-this.element.offsetHeight-10),u=Math.min(Math.max(10,this.initialLeft+r),d),h=Math.min(Math.max(10,this.initialTop+c),m);this.element.style.left=`${u}px`,this.element.style.top=`${h}px`},l=()=>{this.isDragging=!1,window.removeEventListener("mousemove",a),window.removeEventListener("mouseup",l)};window.addEventListener("mousemove",a),window.addEventListener("mouseup",l)};e.addEventListener("mousedown",t)}renderContent(){if(!this.element||!this.currentPlan)return;let e=this.element.querySelector("#eq-fah-body"),t=this.element.querySelector("#eq-fah-pill-text"),n=this.element.querySelector("#eq-fah-pill-badge");e.innerHTML="";let i=this.currentPlan,a=i.actions.filter(h=>h.t==="drag"),l=i.actions.filter(h=>{if(h.t!=="val")return!1;let f=x(h.id||"").toLowerCase();return!/continu|avan[cç]|pr[oó]xim|submet|enviar|check|verific/i.test(f)}),s=i.actions.filter(h=>h.t==="clk"||h.t==="chk"),r=i.actions.filter(h=>h.t==="sel"),c=a.length||l.length||s.length||r.length,d=document.createElement("div");d.className="eq-fah-meta";let m=document.createElement("span");m.textContent=`Modo: ${i.mode.replace("_"," ")}`;let u=document.createElement("span");if(u.className="eq-fah-meta-badge",u.textContent=`${Math.round(i.confidence*100)}% Confian\xE7a`,d.append(m,u),e.appendChild(d),a.length>0||i.mode==="categorizacao"||i.mode==="arrastar_soltar"){t.textContent=`Categoriza\xE7\xE3o (${a.length} itens)`,n.textContent=String(a.length);let h={};for(let f of a){let y=x(f.to)||"Geral";h[y]||(h[y]=[]),h[y].push(x(f.from))}for(let[f,y]of Object.entries(h)){let v=document.createElement("div"),g=/fato|true|verdadeiro|sim/i.test(f),p=/opini[aã]o|false|falso|n[aã]o/i.test(f);v.className=`eq-fah-group ${g?"group-fato":p?"group-opiniao":""}`;let b=document.createElement("div");b.className="eq-fah-group-title",b.textContent=`\u{1F4C1} ${f} (${y.length})`,v.appendChild(b);let q=document.createElement("div");q.className="eq-fah-group-items";for(let w of y){let C=document.createElement("div");C.className="eq-fah-item";let T=document.createElement("span");T.className="eq-fah-item-text",T.textContent=w,C.appendChild(T);let L=document.createElement("button");L.className="eq-fah-copy-inline",L.textContent="Copiar",L.addEventListener("click",()=>{navigator.clipboard.writeText(w),L.textContent="\u2713 Copiado",setTimeout(()=>L.textContent="Copiar",1200)}),C.appendChild(L),q.appendChild(C)}v.appendChild(q),e.appendChild(v)}}else if(l.length>0){t.textContent=`Preenchimento (${l.length} campos)`,n.textContent=String(l.length);let h=document.createElement("div");h.className="eq-fah-group";let f=document.createElement("div");f.className="eq-fah-group-title",f.textContent="\u{1F4DD} Respostas para os Campos de Texto:",h.appendChild(f);let y=document.createElement("div");y.className="eq-fah-group-items";for(let v=0;v<l.length;v++){let g=l[v],p=document.createElement("div");p.className="eq-fah-item";let b=Me(g.id);(!b||/^[#\.\$]|input|mat-|cell|field|q[0-9]|eq-/i.test(b))&&(b=`Campo ${v+1}`);let q=String(g.v??""),w=document.createElement("div");w.className="eq-fah-field-box";let C=document.createElement("div");C.className="eq-fah-field-label",C.textContent=b,w.appendChild(C);let T=document.createElement("div");T.className="eq-fah-field-val",T.textContent=q,w.appendChild(T),p.appendChild(w);let L=document.createElement("button");L.className="eq-fah-copy-inline",L.textContent="Copiar",L.addEventListener("click",()=>{navigator.clipboard.writeText(q),L.textContent="\u2713 Copiado",setTimeout(()=>L.textContent="Copiar",1200)}),p.appendChild(L),y.appendChild(p)}h.appendChild(y),e.appendChild(h)}else if(s.length>0){t.textContent=`Op\xE7\xF5es (${s.length} marcadas)`,n.textContent=String(s.length);let h=document.createElement("div");h.className="eq-fah-group";let f=document.createElement("div");f.className="eq-fah-group-title",f.textContent="\u{1F3AF} Alternativa(s) Correta(s):",h.appendChild(f);let y=document.createElement("div");y.className="eq-fah-group-items";for(let v=0;v<s.length;v++){let g=s[v],p=document.createElement("div");p.className="eq-fah-item";let b=Me(g.id);(!b||/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(b))&&g.v&&(b=String(g.v)),b=x(b),/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(b)&&(b="");let q="",w=b.match(/^(\([A-Za-z0-9]\)|[A-Za-z0-9][\)\.\:\-])\s*(.*)$/);w?(q=w[1].replace(/[\(\)\.\:\-\s]/g,"").toUpperCase(),b=w[2].trim()||b):s.length>1&&(q=String.fromCharCode(65+v));let C=document.createElement("div");if(C.style.display="flex",C.style.alignItems="center",C.style.gap="8px",C.style.flex="1",q){let P=document.createElement("span");P.className="eq-fah-letter-badge",P.textContent=q,C.appendChild(P)}let T=document.createElement("span");T.className="eq-fah-item-text",T.textContent=b||(q?`Alternativa ${q}`:"Alternativa Selecionada"),C.appendChild(T),p.appendChild(C);let L=document.createElement("button");L.className="eq-fah-copy-inline",L.textContent="Copiar",L.addEventListener("click",()=>{navigator.clipboard.writeText(b||q),L.textContent="\u2713 Copiado",setTimeout(()=>L.textContent="Copiar",1200)}),p.appendChild(L),y.appendChild(p)}h.appendChild(y),e.appendChild(h)}else if(r.length>0){t.textContent=`Sele\xE7\xE3o (${r.length} listas)`,n.textContent=String(r.length);let h=document.createElement("div");h.className="eq-fah-group";let f=document.createElement("div");f.className="eq-fah-group-title",f.textContent="\u{1F4CB} Op\xE7\xF5es para Selecionar na Lista:",h.appendChild(f);let y=document.createElement("div");y.className="eq-fah-group-items";for(let v=0;v<r.length;v++){let g=r[v],p=document.createElement("div");p.className="eq-fah-item";let b=Me(g.id);(!b||/^[#\.\$]|select|input|mat-|cell|field|q[0-9]|eq-/i.test(b))&&(b=`Lista ${v+1}`);let C=(Array.isArray(g.v)?g.v:[String(g.v??"")]).map(W=>{let j=A(g.id,void 0,!0)||A(x(g.id),void 0,!0),V=j instanceof HTMLSelectElement?j:j?.querySelector("select");if(V){let ce=x(W).toLowerCase();for(let S=0;S<V.options.length;S++){let H=V.options[S];if(H.value.toLowerCase()===ce||x(H.textContent).toLowerCase()===ce){let z=x(H.textContent);if(z&&!z.toLowerCase().includes("selecione"))return z}}}return W}).join(", "),T=document.createElement("div");T.className="eq-fah-field-box";let L=document.createElement("div");L.className="eq-fah-field-label",L.textContent=b,T.appendChild(L);let P=document.createElement("div");P.className="eq-fah-field-val",P.textContent=C,T.appendChild(P),p.appendChild(T);let D=document.createElement("button");D.className="eq-fah-copy-inline",D.textContent="Copiar",D.addEventListener("click",()=>{navigator.clipboard.writeText(C),D.textContent="\u2713 Copiado",setTimeout(()=>D.textContent="Copiar",1200)}),p.appendChild(D),y.appendChild(p)}h.appendChild(y),e.appendChild(h)}else{t.textContent="Gabarito",n.textContent="0";let h=document.createElement("div");h.style.padding="10px",h.style.color="#888",h.textContent="Nenhuma resposta direta para exibir.",e.appendChild(h)}if(i.rationale){let h=document.createElement("div");h.className="eq-fah-rationale",h.textContent=`\u{1F4A1} Racioc\xEDnio da IA: ${i.rationale}`,e.appendChild(h)}}generateMarkdown(){if(!this.currentPlan)return"";let e=this.currentPlan,t=[];t.push("# Gabarito da Quest\xE3o \u2014 EasyQuiz Pro"),t.push(`- **Modo:** ${e.mode}`),t.push(`- **Confian\xE7a:** ${(e.confidence*100).toFixed(0)}%`),t.push("");let n=e.actions.filter(s=>s.t==="drag"),i=e.actions.filter(s=>s.t==="val"),a=e.actions.filter(s=>s.t==="clk"||s.t==="chk"),l=e.actions.filter(s=>s.t==="sel");if(n.length>0){t.push("## \u{1F4C2} Categoriza\xE7\xE3o:");let s={};for(let r of n){let c=x(r.to)||"Geral";s[c]||(s[c]=[]),s[c].push(x(r.from))}for(let[r,c]of Object.entries(s)){t.push(`### Categoria: ${r}`);for(let d of c)t.push(`- ${d}`);t.push("")}}else if(i.length>0){t.push("## \u270F\uFE0F Respostas para Preenchimento:");for(let s of i){let r=x(s.id);t.push(`- **${r||"Campo"}:** \`${s.v}\``)}t.push("")}else if(a.length>0){t.push("## \u2705 Alternativas Corretas:");for(let s=0;s<a.length;s++){let r=a[s],c=Me(r.id);(!c||/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(c))&&r.v&&(c=String(r.v)),c=x(c),/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(c)&&(c="");let d=a.length>1?`${String.fromCharCode(65+s)}) `:"";t.push(`- [x] ${d}${c||"Alternativa "+String.fromCharCode(65+s)}`)}t.push("")}else if(l.length>0){t.push("## \u{1F4CB} Op\xE7\xF5es Selecionadas em Lista:");for(let s of l){let r=x(s.id)||"Lista",c=Array.isArray(s.v)?s.v.join(", "):String(s.v??"");t.push(`- **${r}:** \`${c}\``)}t.push("")}return e.rationale&&(t.push("---"),t.push(`**\u{1F4A1} Racioc\xEDnio:** ${e.rationale}`)),t.join(`
`)}copyMarkdownToClipboard(e){let t=this.generateMarkdown();t&&navigator.clipboard.writeText(t).then(()=>{let n=e.innerHTML;e.id==="eq-fah-copy-md-btn"?e.innerHTML='<span style="font-size:10px; color:#00ffcc; font-weight:bold;">\u2713</span>':e.innerHTML="\u2713 Copiado!",setTimeout(()=>{e.innerHTML=n},1500)})}};var jt=`
          ...
          <nav class="eq-activity-bar" role="tablist" aria-label="Atalhos">
            <div class="eq-activity-top">
              ...
              <button class="eq-activity-btn" id="eq-tab-metrics" role="tab" title="M\xE9tricas & Cron\xF4metro (Tempo por Quest\xE3o e Hist\xF3rico)">
                <span class="eq-activity-indicator"></span>
                <span class="eq-activity-icon">${E.stopwatch}</span>
              </button>
                ...
                <div class="eq-field-group">
                  <div class="eq-section-title" id="eq-keys-section-header">
                    <span style="display:flex;align-items:center;gap:6px;">
                      <span id="eq-keys-chevron" style="display:inline-flex;transition:transform 0.2s;">${E.chevronRight}</span>
                  ...
                  <div class="eq-key-input-container">
                    <div class="eq-input-wrap">
                      ...
                      <button class="eq-icon-btn" id="eq-key-save" type="button" title="Adicionar Chave">${E.plus}</button>
                      <button class="eq-icon-btn" id="eq-key-more-btn" type="button" title="Mais Op\xE7\xF5es das Chaves">${E.moreVertical}</button>
                    </div>
                    ...
                    <div class="eq-context-menu" id="eq-key-context-menu" hidden>
                      ...
                      <button class="eq-context-item" id="eq-menu-bulk" type="button">
                        <span class="eq-item-icon">${E.listPlus}</span>
                      ...
                      <button class="eq-context-item" id="eq-menu-test" type="button">
                        <span class="eq-item-icon">${E.sparkles}</span>
