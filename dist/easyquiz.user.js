...
[RESPOSTAS]:
${(()=>{if(b.length===0)return"Nenhuma";let f=b.filter(L=>L.type==="checkbox"||L.type==="chk"),g=new Set(b.filter(L=>L.type==="radio").map(L=>L.name).filter(Boolean)),v=f.filter(L=>!L.name||!g.has(L.name)),C=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(o.questionText),A=v.length>=2||C,w=b.every(L=>L.type==="radio"||L.type==="chk")&&g.size>=1&&!A,E=b.filter(L=>L.type==="text"||L.type==="number"||L.type==="val"||L.tag==="input"||L.tag==="textarea"),I=E.length>=2;return(A?`[MULTI-SELE\xC7\xC3O: marque TODOS os corretos, pode ser 2 ou mais]
