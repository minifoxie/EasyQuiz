...
function multiCdnBookmarklet({ bundle, label, cdnBase, fastlyBase, rawBase }) {
  ...
  return `javascript:(function(){function L(f){var v='?v='+Date.now();return fetch('${cdnBase}/'+f+v,{cache:'no-store'}).then(function(r){if(!r.ok)throw 1;return r.text()}).catch(function(){return fetch('${fastlyBase}/'+f+v,{cache:'no-store'}).then(function(r){if(!r.ok)throw 2;return r.text()})}).catch(function(){return fetch('${rawBase}/'+f+v,{cache:'no-store'}).then(function(r){if(!r.ok)throw 3;return r.text()})}).then(function(t){if(!t||t.trim().charAt(0)==='<')throw new Error('Código indisponível');return t})}L('${bundle}').then(function(c){try{${cleanup}(0,eval)(c)}catch(e){alert('EasyQuiz ${label} erro: '+e)}}).catch(function(){var s=document.createElement('script');s.src='${cdnBase}/${bundle}?v='+Date.now();s.onload=function(){s.remove()};s.onerror=function(){s.remove();var s2=document.createElement('script');s2.src='${rawBase}/${bundle}?v='+Date.now();s2.onload=function(){s2.remove()};s2.onerror=function(){s2.remove();alert('EasyQuiz ${label}: nenhum método de carregamento funcionou neste site. Tente o Playground.')};document.head.appendChild(s2)};document.head.appendChild(s)})})();`
}
