...
[HTML]:
${t.htmlSnippet.slice(0,d).replace(/\s+/g," ")}`:"",f="";if(r&&typeof document<"u")try{let b=Array.from(document.querySelectorAll('input:not([type=hidden]), textarea, select, button, [role="button"], [role="radio"], [role="checkbox"], [role="option"], [onclick], [data-action], a[href]:not([href="#"]), [tabindex]:not([tabindex="-1"])')).filter(v=>{let w=v,x=w.getBoundingClientRect?.()||{width:0,height:0};return x.width>0&&x.height>0&&!w.closest("#easyquiz-shadow-root, .eq-sidebar")}).slice(0,40).map(v=>{let w=v,x=w.tagName.toLowerCase(),k=w.id?`#${w.id}`:"",P=w.className&&typeof w.className=="string"?`.${w.className.trim().split(/\s+/).slice(0,2).join(".")}`:"",B=(w.textContent||w.value||w.getAttribute("aria-label")||"").trim().slice(0,60),y=w.getAttribute("type")||w.getAttribute("role")||"";return`${x}${k}${P}[${y}] txt="${B}"`});b.length>0&&(f=`
[DOM-INTERATIVO]:
${b.join(`
...
[RESPOSTAS]:
${(()=>{if(m.length===0)return"Nenhuma";let b=m.filter(C=>C.type==="checkbox"||C.type==="chk"),v=new Set(m.filter(C=>C.type==="radio").map(C=>C.name).filter(Boolean)),w=b.filter(C=>!C.name||!v.has(C.name)),x=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(t.questionText),k=w.length>=2||x,P=v.size>1||/verdadeir|fals[oa]|\bv\s*\/\s*f\b|julgue|itens/i.test(t.questionText)&&v.size>=1,B=m.every(C=>C.type==="radio"||C.type==="chk")&&v.size===1&&!k&&!P,y=m.filter(C=>C.type==="text"||C.type==="number"||C.type==="val"||C.tag==="input"||C.tag==="textarea"),S=y.length>=2;return(P?`[GRADE VERDADEIRO/FALSO (${v.size||"m\xFAltiplas"} afirma\xE7\xF5es): voc\xEA DEVE julgar e marcar exatamente 1 op\xE7\xE3o (V ou F) para CADA uma das ${v.size} afirma\xE7\xF5es \u2014 emita ${v.size} a\xE7\xF5es chk separadas + adv]
...
`);navigator.clipboard.writeText(a).then(()=>{n.textContent="\u2713",setTimeout(()=>n.textContent="\u{1F4CB}",1e3)})}),this.el.querySelector("#__eq_dbg_btn_clear__")?.addEventListener("click",()=>{this.logs=[],this.renderConsoleLogs(),this.updateTabCounters()})}renderFlow(){let e=this.el?.querySelector(".__eq_dbg_body__");if(!e)return;if(!this.currentFlow||this.currentFlow.length===0){e.innerHTML=`
    ...
    `,this.el.querySelector(".__eq_dbg_header__").addEventListener("mousedown",this.onHeaderMouseDown.bind(this)),this.el.querySelectorAll(".__eq_dbg_tab__").forEach(n=>{n.addEventListener("click",o=>{let a=o.currentTarget.getAttribute("data-tab");this.activeTab=a||"console",this.el?.querySelectorAll(".__eq_dbg_tab__").forEach(i=>i.classList.remove("active")),o.currentTarget.classList.add("active"),this.render()})}),this.el.querySelector("#__eq_win_min__")?.addEventListener("click",()=>this.minimize()),this.el.querySelector("#__eq_win_close__")?.addEventListener("click",()=>this.close()),document.documentElement.appendChild(this.el),this.pillEl=document.createElement("div"),this.pillEl.id="__eq_dbg_pill__",this.pillEl.style.display="none",this.pillEl.innerHTML=`
      ...
      <span class="__eq_pill_text__"><img src="${we.canvasLogo}" style="width:14px;height:14px;vertical-align:middle;margin-right:4px;object-fit:contain;"/>EQ ${ge}: Pronto</span>
