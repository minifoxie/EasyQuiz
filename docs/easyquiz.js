...
[RESPOSTAS]:
${(()=>{if(p.length===0)return"Nenhuma";let v=p.filter(q=>q.type==="checkbox"||q.type==="chk"),y=new Set(p.filter(q=>q.type==="radio").map(q=>q.name).filter(Boolean)),C=v.filter(q=>!q.name||!y.has(q.name)),T=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(o.questionText),x=C.length>=2||T,E=y.size>1||/verdadeir|fals[oa]|\bv\s*\/\s*f\b|julgue|itens/i.test(o.questionText)&&y.size>=1,L=p.every(q=>q.type==="radio"||q.type==="chk")&&y.size===1&&!x&&!E,w=p.filter(q=>q.type==="text"||q.type==="number"||q.type==="val"||q.tag==="input"||q.tag==="textarea"),M=w.length>=2;return(E?`[GRADE VERDADEIRO/FALSO (${y.size||"m\xFAltiplas"} afirma\xE7\xF5es): voc\xEA DEVE julgar e marcar exatamente 1 op\xE7\xE3o (V ou F) para CADA uma das ${y.size} afirma\xE7\xF5es \u2014 emita ${y.size} a\xE7\xF5es chk separadas + adv]
...
`;function Kt(){try{if(typeof document>"u"||!document.head)return;if(!document.getElementById("eq-image-pulse-style")){let o=document.createElement("style");o.id="eq-image-pulse-style",o.textContent=qo,document.head.appendChild(o)}}catch{}}function ge(){ie&&(ie.style.removeProperty("outline"),ie.style.removeProperty("outline-offset"),ie.style.removeProperty("position"),ie=null);for(let o of ye)o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("background-color"),o.style.removeProperty("box-shadow"),o.removeAttribute("data-easyquiz-highlight");ye=[];for(let o of ft)o.style.removeProperty("animation"),o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("box-shadow"),o.style.removeProperty("filter"),o.removeAttribute("data-easyquiz-image-highlight");ft=[];for(let o of bt)try{o.remove()}catch{}bt=[],he&&typeof window<"u"&&(window.removeEventListener("scroll",he),window.removeEventListener("resize",he),he=null);for(let o of vt)try{o.remove()}catch{}if(vt=[],se){try{se.remove()}catch{}se=null}}function At(o){Kt();let e=[];for(let t of o){if(!t||typeof t.setAttribute!="function")continue;let n=t;try{n.style&&(n.style.outline="3px solid #ffd600",n.style.outlineOffset="4px",n.style.animation="eq-image-pulse-yellow-white 1.2s ease-in-out infinite",n.style.boxShadow="0 0 16px rgba(255, 214, 0, 0.7)",n.style.filter="drop-shadow(0 0 8px rgba(255, 214, 0, 0.8))"),n.setAttribute("data-easyquiz-image-highlight","true"),ft.push(n)}catch{}try{let r=t.getBoundingClientRect(),a=r.width||t.offsetWidth||280,s=r.height||t.offsetHeight||200;if(a>10&&s>10){let i=document.createElement("div");i.setAttribute("data-easyquiz-image-frame","true"),i.style.cssText=`
    ...
    `,this.shadow.appendChild(this.element),this.element.querySelector("#eq-fah-pill").addEventListener("click",()=>this.restore()),this.element.querySelector("#eq-fah-min-btn").addEventListener("click",()=>this.minimize()),this.element.querySelector("#eq-fah-close-btn").addEventListener("click",()=>this.hide());let r=this.element.querySelector("#eq-fah-copy-md-btn");r.addEventListener("click",()=>this.copyMarkdownToClipboard(r));let a=this.element.querySelector("#eq-fah-copy-all-btn");a.addEventListener("click",()=>this.copyMarkdownToClipboard(a));let s=this.element.querySelector("#eq-fah-header");this.setupDraggable(s)}setupDraggable(e){let t=n=>{if(n.target.closest(".eq-fah-btn"))return;n.preventDefault(),this.isDragging=!0,this.dragStartX=n.clientX,this.dragStartY=n.clientY;let r=this.element.getBoundingClientRect();this.initialLeft=r.left,this.initialTop=r.top;let a=i=>{if(!this.isDragging||!this.element)return;let d=i.clientX-this.dragStartX,l=i.clientY-this.dragStartY,u=Math.max(10,window.innerWidth-this.element.offsetWidth-10),f=Math.max(10,window.innerHeight-this.element.offsetHeight-10),c=Math.min(Math.max(10,this.initialLeft+d),u),m=Math.min(Math.max(10,this.initialTop+l),f);this.element.style.left=`${c}px`,this.element.style.top=`${m}px`},s=()=>{this.isDragging=!1,window.removeEventListener("mousemove",a),window.removeEventListener("mouseup",s)};window.addEventListener("mousemove",a),window.addEventListener("mouseup",s)};e.addEventListener("mousedown",t)}renderContent(){if(!this.element||!this.currentPlan)return;let e=this.element.querySelector("#eq-fah-body"),t=this.element.querySelector("#eq-fah-pill-text"),n=this.element.querySelector("#eq-fah-pill-badge");e.innerHTML="";let r=this.currentPlan,a=r.actions.filter(m=>m.t==="drag"),s=r.actions.filter(m=>{if(m.t!=="val")return!1;let p=k(m.id||"").toLowerCase();return!/continu|avan[cç]|pr[oó]xim|submet|enviar|check|verific/i.test(p)}),i=r.actions.filter(m=>m.t==="clk"||m.t==="chk"),d=r.actions.filter(m=>m.t==="sel"),l=a.length||s.length||i.length||d.length,u=document.createElement("div");u.className="eq-fah-meta";let f=document.createElement("span");f.textContent=`Modo: ${r.mode.replace("_"," ")}`;let c=document.createElement("span");if(c.className="eq-fah-meta-badge",c.textContent=`${Math.round(r.confidence*100)}% Confian\xE7a`,u.append(f,c),e.appendChild(u),a.length>0||r.mode==="categorizacao"||r.mode==="arrastar_soltar"){t.textContent=`Categoriza\xE7\xE3o (${a.length} itens)`,n.textContent=String(a.length);let m={};for(let p of a){let h=k(p.to)||"Geral";m[h]||(m[h]=[]),m[h].push(k(p.from))}for(let[p,h]of Object.entries(m)){let b=document.createElement("div"),A=/fato|true|verdadeiro|sim/i.test(p),v=/opini[aã]o|false|falso|n[aã]o/i.test(p);b.className=`eq-fah-group ${A?"group-fato":v?"group-opiniao":""}`;let y=document.createElement("div");y.className="eq-fah-group-title",y.textContent=`\u{1F4C1} ${p} (${h.length})`,b.appendChild(y);let C=document.createElement("div");C.className="eq-fah-group-items";for(let T of h){let x=document.createElement("div");x.className="eq-fah-item";let E=document.createElement("span");E.className="eq-fah-item-text",E.textContent=T,x.appendChild(E);let L=document.createElement("button");L.className="eq-fah-copy-inline",L.textContent="Copiar",L.addEventListener("click",()=>{navigator.clipboard.writeText(T),L.textContent="\u2713 Copiado",setTimeout(()=>L.textContent="Copiar",1200)}),x.appendChild(L),C.appendChild(x)}b.appendChild(C),e.appendChild(b)}}else if(s.length>0){t.textContent=`Preenchimento (${s.length} campos)`,n.textContent=String(s.length);let m=document.createElement("div");m.className="eq-fah-group";let p=document.createElement("div");p.className="eq-fah-group-title",p.textContent="\u{1F4DD} Respostas para os Campos de Texto:",m.appendChild(p);let h=document.createElement("div");h.className="eq-fah-group-items";for(let b=0;b<s.length;b++){let A=s[b],v=document.createElement("div");v.className="eq-fah-item";let y=Ie(A.id);(!y||/^[#\.\$]|input|mat-|cell|field|q[0-9]|eq-/i.test(y))&&(y=`Campo ${b+1}`);let C=String(A.v??""),T=document.createElement("div");T.className="eq-fah-field-box";let x=document.createElement("div");x.className="eq-fah-field-label",x.textContent=y,T.appendChild(x);let E=document.createElement("div");E.className="eq-fah-field-val",E.textContent=C,T.appendChild(E),v.appendChild(T);let L=document.createElement("button");L.className="eq-fah-copy-inline",L.textContent="Copiar",L.addEventListener("click",()=>{navigator.clipboard.writeText(C),L.textContent="\u2713 Copiado",setTimeout(()=>L.textContent="Copiar",1200)}),v.appendChild(L),h.appendChild(v)}m.appendChild(h),e.appendChild(m)}else if(i.length>0){t.textContent=`Op\xE7\xF5es (${i.length} marcadas)`,n.textContent=String(i.length);let m=document.createElement("div");m.className="eq-fah-group";let p=document.createElement("div");p.className="eq-fah-group-title",p.textContent="\u{1F3AF} Alternativa(s) Correta(s):",m.appendChild(p);let h=document.createElement("div");h.className="eq-fah-group-items";for(let b=0;b<i.length;b++){let A=i[b],v=document.createElement("div");v.className="eq-fah-item";let y=Ie(A.id);(!y||/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(y))&&A.v&&(y=String(A.v)),y=k(y),/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(y)&&(y="");let C="",T=y.match(/^(\([A-Za-z0-9]\)|[A-Za-z0-9][\)\.\:\-])\s*(.*)$/);T?(C=T[1].replace(/[\(\)\.\:\-\s]/g,"").toUpperCase(),y=T[2].trim()||y):i.length>1&&(C=String.fromCharCode(65+b));let x=document.createElement("div");if(x.style.display="flex",x.style.alignItems="center",x.style.gap="8px",x.style.flex="1",C){let w=document.createElement("span");w.className="eq-fah-letter-badge",w.textContent=C,x.appendChild(w)}let E=document.createElement("span");E.className="eq-fah-item-text",E.textContent=y||(C?`Alternativa ${C}`:"Alternativa Selecionada"),x.appendChild(E),v.appendChild(x);let L=document.createElement("button");L.className="eq-fah-copy-inline",L.textContent="Copiar",L.addEventListener("click",()=>{navigator.clipboard.writeText(y||C),L.textContent="\u2713 Copiado",setTimeout(()=>L.textContent="Copiar",1200)}),v.appendChild(L),h.appendChild(v)}m.appendChild(h),e.appendChild(m)}else if(d.length>0){t.textContent=`Sele\xE7\xE3o (${d.length} listas)`,n.textContent=String(d.length);let m=document.createElement("div");m.className="eq-fah-group";let p=document.createElement("div");p.className="eq-fah-group-title",p.textContent="\u{1F4CB} Op\xE7\xF5es para Selecionar na Lista:",m.appendChild(p);let h=document.createElement("div");h.className="eq-fah-group-items";for(let b=0;b<d.length;b++){let A=d[b],v=document.createElement("div");v.className="eq-fah-item";let y=Ie(A.id);(!y||/^[#\.\$]|select|input|mat-|cell|field|q[0-9]|eq-/i.test(y))&&(y=`Lista ${b+1}`);let x=(Array.isArray(A.v)?A.v:[String(A.v??"")]).map(I=>{let q=P(A.id,void 0,!0)||P(k(A.id),void 0,!0),H=q instanceof HTMLSelectElement?q:q?.querySelector("select");if(H){let O=k(I).toLowerCase();for(let Q=0;Q<H.options.length;Q++){let N=H.options[Q];if(N.value.toLowerCase()===O||k(N.textContent).toLowerCase()===O){let U=k(N.textContent);if(U&&!U.toLowerCase().includes("selecione"))return U}}}return I}).join(", "),E=document.createElement("div");E.className="eq-fah-field-box";let L=document.createElement("div");L.className="eq-fah-field-label",L.textContent=y,E.appendChild(L);let w=document.createElement("div");w.className="eq-fah-field-val",w.textContent=x,E.appendChild(w),v.appendChild(E);let M=document.createElement("button");M.className="eq-fah-copy-inline",M.textContent="Copiar",M.addEventListener("click",()=>{navigator.clipboard.writeText(x),M.textContent="\u2713 Copiado",setTimeout(()=>M.textContent="Copiar",1200)}),v.appendChild(M),h.appendChild(v)}m.appendChild(h),e.appendChild(m)}else{t.textContent="Gabarito",n.textContent="0";let m=document.createElement("div");m.style.padding="10px",m.style.color="#888",m.textContent="Nenhuma resposta direta para exibir.",e.appendChild(m)}if(r.rationale){let m=document.createElement("div");m.className="eq-fah-rationale",m.textContent=`\u{1F4A1} Racioc\xEDnio da IA: ${r.rationale}`,e.appendChild(m)}}generateMarkdown(){if(!this.currentPlan)return"";let e=this.currentPlan,t=[];t.push("# Gabarito da Quest\xE3o \u2014 EasyQuiz Pro"),t.push(`- **Modo:** ${e.mode}`),t.push(`- **Confian\xE7a:** ${(e.confidence*100).toFixed(0)}%`),t.push("");let n=e.actions.filter(i=>i.t==="drag"),r=e.actions.filter(i=>i.t==="val"),a=e.actions.filter(i=>i.t==="clk"||i.t==="chk"),s=e.actions.filter(i=>i.t==="sel");if(n.length>0){t.push("## \u{1F4C2} Categoriza\xE7\xE3o:");let i={};for(let d of n){let l=k(d.to)||"Geral";i[l]||(i[l]=[]),i[l].push(k(d.from))}for(let[d,l]of Object.entries(i)){t.push(`### Categoria: ${d}`);for(let u of l)t.push(`- ${u}`);t.push("")}}else if(r.length>0){t.push("## \u270F\uFE0F Respostas para Preenchimento:");for(let i of r){let d=k(i.id);t.push(`- **${d||"Campo"}:** \`${i.v}\``)}t.push("")}else if(a.length>0){t.push("## \u2705 Alternativas Corretas:");for(let i=0;i<a.length;i++){let d=a[i],l=Ie(d.id);(!l||/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(l))&&d.v&&(l=String(d.v)),l=k(l),/^(eq-|#|\$|\.|input_|mat-|choice_|radio_|chk_)/i.test(l)&&(l="");let u=a.length>1?`${String.fromCharCode(65+i)}) `:"";t.push(`- [x] ${u}${l||"Alternativa "+String.fromCharCode(65+i)}`)}t.push("")}else if(s.length>0){t.push("## \u{1F4CB} Op\xE7\xF5es Selecionadas em Lista:");for(let i of s){let d=k(i.id)||"Lista",l=Array.isArray(i.v)?i.v.join(", "):String(i.v??"");t.push(`- **${d}:** \`${l}\``)}t.push("")}return e.rationale&&(t.push("---"),t.push(`**\u{1F4A1} Racioc\xEDnio:** ${e.rationale}`)),t.join(`
