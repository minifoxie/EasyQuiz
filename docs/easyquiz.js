...
[HTML]:
${o.htmlSnippet.slice(0,l).replace(/\s+/g," ")}`:"",f="";if(i&&typeof document<"u")try{let v=Array.from(document.querySelectorAll('input:not([type=hidden]), textarea, select, button, [role="button"], [role="radio"], [role="checkbox"], [role="option"], [onclick], [data-action], a[href]:not([href="#"]), [tabindex]:not([tabindex="-1"])')).filter(y=>{let T=y,k=T.getBoundingClientRect?.()||{width:0,height:0};return k.width>0&&k.height>0&&!T.closest("#easyquiz-shadow-root, .eq-sidebar")}).slice(0,40).map(y=>{let T=y,k=T.tagName.toLowerCase(),x=T.id?`#${T.id}`:"",E=T.className&&typeof T.className=="string"?`.${T.className.trim().split(/\s+/).slice(0,2).join(".")}`:"",S=(T.textContent||T.value||T.getAttribute("aria-label")||"").trim().slice(0,60),w=T.getAttribute("type")||T.getAttribute("role")||"";return`${k}${x}${E}[${w}] txt="${S}"`});v.length>0&&(f=`
[DOM-INTERATIVO]:
${v.join(`
...
[RESPOSTAS]:
${(()=>{if(p.length===0)return"Nenhuma";let v=p.filter(C=>C.type==="checkbox"||C.type==="chk"),y=new Set(p.filter(C=>C.type==="radio").map(C=>C.name).filter(Boolean)),T=v.filter(C=>!C.name||!y.has(C.name)),k=/selecione as|assinale as|quais das|todas as|marque as|escolha as|quais dessas|quais dos/i.test(o.questionText),x=T.length>=2||k,E=y.size>1||/verdadeir|fals[oa]|\bv\s*\/\s*f\b|julgue|itens/i.test(o.questionText)&&y.size>=1,S=p.every(C=>C.type==="radio"||C.type==="chk")&&y.size===1&&!x&&!E,w=p.filter(C=>C.type==="text"||C.type==="number"||C.type==="val"||C.tag==="input"||C.tag==="textarea"),I=w.length>=2;return(E?`[GRADE VERDADEIRO/FALSO (${y.size||"m\xFAltiplas"} afirma\xE7\xF5es): voc\xEA DEVE julgar e marcar exatamente 1 op\xE7\xE3o (V ou F) para CADA uma das ${y.size} afirma\xE7\xF5es \u2014 emita ${y.size} a\xE7\xF5es chk separadas + adv]
...
`;function Kt(){try{if(typeof document>"u"||!document.head)return;if(!document.getElementById("eq-image-pulse-style")){let o=document.createElement("style");o.id="eq-image-pulse-style",o.textContent=qo,document.head.appendChild(o)}}catch{}}function he(){ie&&(ie.style.removeProperty("outline"),ie.style.removeProperty("outline-offset"),ie.style.removeProperty("position"),ie=null);for(let o of ye)o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("background-color"),o.style.removeProperty("box-shadow"),o.removeAttribute("data-easyquiz-highlight");ye=[];for(let o of ft)o.style.removeProperty("animation"),o.style.removeProperty("outline"),o.style.removeProperty("outline-offset"),o.style.removeProperty("box-shadow"),o.style.removeProperty("filter"),o.removeAttribute("data-easyquiz-image-highlight");ft=[];for(let o of bt)try{o.remove()}catch{}bt=[],ge&&typeof window<"u"&&(window.removeEventListener("scroll",ge),window.removeEventListener("resize",ge),ge=null);for(let o of At)try{o.remove()}catch{}if(At=[],se){try{se.remove()}catch{}se=null}}function vt(o){Kt();let e=[];for(let t of o){if(!t||typeof t.setAttribute!="function")continue;let a=t;try{a.style&&(a.style.outline="3px solid #ffd600",a.style.outlineOffset="4px",a.style.animation="eq-image-pulse-yellow-white 1.2s ease-in-out infinite",a.style.boxShadow="0 0 16px rgba(255, 214, 0, 0.7)",a.style.filter="drop-shadow(0 0 8px rgba(255, 214, 0, 0.8))"),a.setAttribute("data-easyquiz-image-highlight","true"),ft.push(a)}catch{}try{let r=t.getBoundingClientRect(),n=r.width||t.offsetWidth||280,s=r.height||t.offsetHeight||200;if(n>10&&s>10){let i=document.createElement("div");i.setAttribute("data-easyquiz-image-frame","true"),i.style.cssText=`
          position: absolute;
        ...
        `,i.appendChild(d),document.body.appendChild(i),bt.push(i),At.push(d),e.push({element:t,frame:i})}}catch{}}e.length>0&&!ge&&typeof window<"u"&&(ge=()=>{for(let t of e)try{let a=t.element.getBoundingClientRect();a.width>0&&a.height>0&&(t.frame.style.top=`${a.top+window.scrollY-3}px`,t.frame.style.left=`${a.left+window.scrollX-3}px`,t.frame.style.width=`${a.width+6}px`,t.frame.style.height=`${a.height+6}px`)}catch{}},window.addEventListener("scroll",ge,{passive:!0}),window.addEventListener("resize",ge,{passive:!0}))}function Pe(o){ie&&ie!==o&&(ie.style.removeProperty("outline"),ie.style.removeProperty("outline-offset")),Kt(),ie=o,o.style.outline="2px solid #00e5ff",o.style.outlineOffset="4px";try{if(se){try{se.remove()}catch{}se=null}window.getComputedStyle(o).position==="static"&&(o.style.position="relative");let t=document.createElement("div");t.style.cssText=`
...
HTML do alvo: ${v}`:""}`,k=m.strategiesAttempted?[m.strategiesAttempted].flat().concat(["alternative-path","injectScript","keyboard","vue-react-internals"]):["alternative-path","injectScript","keyboard","vue-react-internals"],x=`[REPLANEJAMENTO URGENTE \u2014 TENTATIVA FINAL]
A seguinte a\xE7\xE3o falhou ap\xF3s ${k.length} estrat\xE9gias autom\xE1ticas: ${k.join(", ")}.
